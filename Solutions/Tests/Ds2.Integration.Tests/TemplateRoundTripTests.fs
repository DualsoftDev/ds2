...
// =============================================================================
// v10 ApiDef ActionType / SensingType round-trip 검증.
// Real(SignalMode, TimePolicy option) + Virtual(TimePolicy option) 모든 case 가
// AASX JSON 직렬화 통해 보존되는지. spec §3.2 / §11 의 dispatch case 망라.
// =============================================================================

let private addApiDef (store: DsStore) (sys: DsSystem) name action sensing =
    let def = ApiDef(name, sys.Id)
    def.ActionType <- action
    def.SensingType <- sensing
    store.DirectWrite(store.ApiDefs, def)
    def.Id

let private exportImportStore (store: DsStore) : DsStore =
    let path = tempPath "rt-v10"
    AasxExporter.exportFromStore store path "https://dualsoft.com/" false false |> ignore
    let imported = DsStore()
    AasxImporter.importIntoStoreOrRaise imported path
    try File.Delete path with _ -> ()
    try Directory.Delete(Path.GetDirectoryName path, true) with _ -> ()
    imported

[<Fact>]
let ``v10 ApiDef Real Level None round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let id = addApiDef store sys "Normal" (ActionType.Real (Level, None)) (SensingType.Real (Level, None))
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(ActionType.Real (Level, None), def.ActionType)
    Assert.Equal(SensingType.Real (Level, None), def.SensingType)

[<Fact>]
let ``v10 ApiDef Real OneShot None round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let id = addApiDef store sys "Pulse" (ActionType.Real (OneShot, None)) (SensingType.Real (OneShot, None))
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(ActionType.Real (OneShot, None), def.ActionType)
    Assert.Equal(SensingType.Real (OneShot, None), def.SensingType)

[<Fact>]
let ``v10 ApiDef Real Latched None round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let id = addApiDef store sys "Set" (ActionType.Real (Latched, None)) (SensingType.Real (Latched, None))
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(ActionType.Real (Latched, None), def.ActionType)
    Assert.Equal(SensingType.Real (Latched, None), def.SensingType)

[<Fact>]
let ``v10 ApiDef Real Level with Append round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let action  = ActionType.Real  (Level, Some (Append 500))
    let sensing = SensingType.Real (Level, Some (Append 20))
    let id = addApiDef store sys "TimeAppend" action sensing
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(action,  def.ActionType)
    Assert.Equal(sensing, def.SensingType)

[<Fact>]
let ``v10 ApiDef Real OneShot with Append round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let action  = ActionType.Real  (OneShot, Some (Append 200))
    let sensing = SensingType.Real (OneShot, Some (Append 50))
    let id = addApiDef store sys "PulseHold" action sensing
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(action,  def.ActionType)
    Assert.Equal(sensing, def.SensingType)
...
[<Fact>]
let ``v10 ApiDef Virtual None round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let id = addApiDef store sys "Virt" (ActionType.Virtual None) (SensingType.Virtual None)
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(ActionType.Virtual None,  def.ActionType)
    Assert.Equal(SensingType.Virtual None, def.SensingType)

[<Fact>]
let ``v10 ApiDef Virtual with Append round-trip`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let action  = ActionType.Virtual  (Some (Append 1000))
    let sensing = SensingType.Virtual (Some (Append 100))
    let id = addApiDef store sys "VirtPlus" action sensing
    let imported = exportImportStore store
    let def = imported.ApiDefs.[id]
    Assert.Equal(action,  def.ActionType)
    Assert.Equal(sensing, def.SensingType)
...
[<Fact>]
let ``v10 ApiDef multiple defs preserve distinct types`` () =
    let store, _ = buildMinimalProject()
    let sys = store.Systems.Values |> Seq.head
    let id1 = addApiDef store sys "Adv" (ActionType.Real (Level, None))           (SensingType.Real (Level, None))
    let id2 = addApiDef store sys "Pls" (ActionType.Real (OneShot, Some (Append 100))) (SensingType.Real (OneShot, Some (Append 30)))
    let id3 = addApiDef store sys "Vrt" (ActionType.Virtual (Some (Append 500))) (SensingType.Virtual None)
    let imported = exportImportStore store
    Assert.Equal(ActionType.Real (Level, None), imported.ApiDefs.[id1].ActionType)
    Assert.Equal(ActionType.Real (OneShot, Some (Append 100)), imported.ApiDefs.[id2].ActionType)
    Assert.Equal(SensingType.Real (OneShot, Some (Append 30)), imported.ApiDefs.[id2].SensingType)
    Assert.Equal(ActionType.Virtual (Some (Append 500)), imported.ApiDefs.[id3].ActionType)
    Assert.Equal(SensingType.Virtual None, imported.ApiDefs.[id3].SensingType)
