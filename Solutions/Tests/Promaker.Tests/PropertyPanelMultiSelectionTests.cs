...
public sealed class PropertyPanelMultiSelectionTests
{
    ...
    public void Multi_selected_calls_apply_timeout_to_all_selected_items()
    {
        StaTestRunner.Run(() =>
        {
            ...
            store.AddCallsWithDevice(projectId, work1Id, ["Dev.Api1"], true);
            store.AddCallsWithDevice(projectId, work2Id, ["Dev.Api2"], true);
            ...
            Assert.Equal(1800.0, store.Calls[call1.Id].Properties.Timeout!.Value.TotalMilliseconds);
            Assert.Equal(1800.0, store.Calls[call2.Id].Properties.Timeout!.Value.TotalMilliseconds);
        });
    }
