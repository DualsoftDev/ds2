...
///
/// Phase 6 chunk-3: 호출 entry 가 `validateModelByGuid` → `validateModelByPath` 로 격상.
/// GUID 입출력 폐기 정책 (SSOT yaml-protocol-v0.md §2.5.1) 정합. None = global / Some "" = global /
/// Some ".Project.Sys" = SystemScope / Some ".Project.Sys.Flow" = FlowScope. 매칭 실패 path =
/// VALIDATION_ERROR (RunRead 의 INTERNAL_ERROR 분기 회피).

[<Fact>]
let ``빈 store None scope 는 no issues 메시지 (global 동등)`` () =
    let store = DsStore()
    let result = ToolOperations.validateModelByPath store None
    Assert.Equal("(no issues; scope=global)", result)

[<Fact>]
let ``빈 store empty path 는 None 과 동등 (global)`` () =
    let store = DsStore()
    let result = ToolOperations.validateModelByPath store (Some "")
    Assert.Equal("(no issues; scope=global)", result)

[<Fact>]
let ``존재하지 않는 path scope 는 VALIDATION_ERROR`` () =
    let store = DsStore()
    let result = ToolOperations.validateModelByPath store (Some ".NoSuchProject")
    Assert.StartsWith("VALIDATION_ERROR:", result)
    Assert.Contains(".NoSuchProject", result)
...
[<Fact>]
let ``Project path 1-segment 는 GlobalScope 동등 (path 깊이 → kind 룰 N=1)`` () =
    // SSOT §2.5.1 / resolveValidationScope: 1-segment path → GlobalScope (PoC N=1 가정).
    let store = DsStore()
    let _projectId = store.AddProject("Project")
    let result = ToolOperations.validateModelByPath store (Some ".Project")
    Assert.Equal("(no issues; scope=global)", result)
...
[<Fact>]
let ``System path scope 는 Orphan check skipped footer`` () =
    let store = DsStore()
    let projectId = store.AddProject("Project")
    let _sysId = store.AddSystem("Sys", projectId, true)
    let result = ToolOperations.validateModelByPath store (Some ".Project.Sys")
...
[<Fact>]
let ``Flow path scope 는 sibling DuplicateName / ApiDef / ArrowBetweenWorks skip footer`` () =
    let store = DsStore()
    let projectId = store.AddProject("Project")
    let sysId = store.AddSystem("Sys", projectId, true)
    let _flowId = store.AddFlow("Flow", sysId)
    let result = ToolOperations.validateModelByPath store (Some ".Project.Sys.Flow")
    Assert.Contains("Orphan / sibling-flow DuplicateName / ApiDef / ArrowBetweenWorks checks skipped", result)

[<Fact>]
let ``4-segment 이상 path 는 scope-unsupported VALIDATION_ERROR`` () =
    // resolveValidationScope: segs.Length > 3 → None → VALIDATION_ERROR.
    // Work/Call scope 는 validate_model 미지원 (sibling 검사 의미 없음).
    let store = DsStore()
    let projectId = store.AddProject("Project")
    let sysId = store.AddSystem("Sys", projectId, true)
    let flowId = store.AddFlow("Flow", sysId)
    let _workId = store.AddWork("W", flowId)
    let result = ToolOperations.validateModelByPath store (Some ".Project.Sys.Flow.W")
    Assert.StartsWith("VALIDATION_ERROR:", result)
