...
[<Fact>]
let ``현재 tool 풀세트 sanity = 10개 (doc-level 4 + read 6)`` () =
    let csNames = File.ReadAllText promakerToolNamesPath
    let listed = extractPromakerToolNames csNames
    // Phase 5 cleanup (SSOT: yaml-protocol-v0.md §2): op-layer 15종 (apply_operations + add_* + remove_entity +
    // rename_entity) 일소. 남는 풀세트 = doc-level 4 (apply_model_doc / validate_model_doc / export_model_doc /
    // json_to_yaml) + read 6 (list_projects / list_systems / describe_system / describe_subtree / find_by_name /
    // validate_model). yaml_to_json LLM 비노출 (todo §3.4) 결정 유지.
    Assert.Equal(10, listed.Count)

/// `[McpServerTool, Description("...")]` 의 string literal 안에 Phase 5 cleanup 으로 일소된 op-layer 어휘가
/// 잔재하면 fail. 도구 이름 자체는 PromakerToolNames 와 Sync 가 위에서 검증되지만, description 본문이 LLM 의
/// mental model 을 직접 형성하므로 stale 어휘가 회귀 fence 대상.
...
[<Fact>]
let ``snake_case 변환 단위 동작 — Describe/ApiDef 등 핵심 케이스`` () =
