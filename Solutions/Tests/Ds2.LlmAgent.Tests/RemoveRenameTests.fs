...
/// active 가 없는 빈 project — active path 의 add → remove cycle 회귀 방어용 (M-B).
let private buildFixtureBare () =
    let store = DsStore()
    store.AddProject("Proj") |> ignore
    store
...
[<Fact>]
let ``같은 turn 의 add_passive 직후 remove_entity 는 명확한 진단 메시지로 거부`` () =
    let store, _, _, _, _ = buildFixture ()
    let plan = ImportPlanBuilder()
    // 같은 turn 안에서 add_passive_system 을 plan 에 누적 — store 는 아직 미반영 (extend-mcp L2 분리).
    let newSysId = ToolOperations.queueAddPassiveSystem plan store "TempSys" "Unit"
    Assert.False(store.Systems.ContainsKey(newSysId))   // store 는 모름 (turn end 까지)
    let countAfterAdd = plan.Count   // queueAddPassiveSystem 은 AddSystem + LinkSystemToProject 누적
...
[<Fact>]
let ``같은 turn 의 add_active 직후 remove_entity 는 명확한 진단 메시지로 거부 (M-B active path)`` () =
    let store = buildFixtureBare ()   // active 없는 빈 project — PLC 1 controller fail 회피
    let plan = ImportPlanBuilder()
    let newSysId = ToolOperations.queueAddActiveSystem plan store "TempSys"
    Assert.False(store.Systems.ContainsKey(newSysId))
    let countAfterAdd = plan.Count
    let ex = Assert.Throws<InvalidOperationException>(fun () ->
        ToolOperations.queueRemoveEntity plan store newSysId |> ignore)
    Assert.Contains("같은 apply_model_doc turn", ex.Message)
    Assert.Contains("patch.remove", ex.Message)
