...
[<Fact>]
let ``commit-6b: buildWith useStreamJsonInput=false → --input-format 미전달 (기존 text 경로)`` () =
    let args = ClaudeCliArgs.buildWith baseOptions None None false
    Assert.DoesNotContain("--input-format", args)

[<Fact>]
let ``commit-6b: buildWith useStreamJsonInput=true → --input-format stream-json 노출`` () =
    let args = ClaudeCliArgs.buildWith baseOptions None None true
    let i = args |> List.findIndex ((=) "--input-format")
    Assert.Equal("stream-json", args.[i + 1])
