...
///
/// 검증 시나리오:
///   1. synthetic xlsx fixture (***REDACTED***2 IO List 의 4-block layout 모사) — strategy 매치 + markdown 구조 회귀.
...
// ── synthetic xlsx fixture — ***REDACTED***2 IO List 4-block layout 모사 ────────────────
...
[<Fact>]
let ``IoListStrategy signature — ***REDACTED***2 fixture 는 매치 (score >= threshold)`` () =
...
[<Fact>]
let ``IoListStrategy Build — 매치 시 markdown 반환 + 머리말 6행 강제`` () =
    withTempPath ".xlsx" (fun path ->
        makeIoListFixture path
        let extracted = extractDocument path
        let strategy = IoListStrategy() :> IXlsxStrategy
        ...
        | Built markdown ->
            // 머리말 6행 (`documents-based-gfm.md` §8.5.5 — 2026-05-27 canary 1행 patch 박제 baseline).
            let lines = markdown.Split([| '\n' |], 9)
            Assert.True(lines.Length >= 6, "최소 6 행 머리말 필요 (canary + 기존 5행)")
            // 1행: canary (Prompts/*.md 정합 — pong: summary/<Filename>)
            Assert.Contains("canary:", lines.[0])
            Assert.Contains("pong: summary/", lines.[0])
            // 2행: generated by IoListStrategy vX.Y.Z at <ISO>
            Assert.Contains("generated by IoListStrategy v", lines.[1])
            // 3행: source: <filename> (docId: <8-char>)
            Assert.Contains("source:", lines.[2])
            Assert.Contains("docId:", lines.[2])
            // 4행: signature: IoListStrategy:N/9
            Assert.Contains("signature: IoListStrategy:", lines.[3])
            // 5행: estimated-tokens
            Assert.Contains("estimated-tokens:", lines.[4])
            // 6행: strategy-version: IoListStrategy vX.Y.Z (M11 — version 박제 stale 감지 용)
            Assert.Contains("strategy-version: IoListStrategy v", lines.[5]))
...
[<Fact>]
let ``XlsxSignatureClassifier — strategy list = IoListStrategy 단독 (2026-06-04 WorkOrder 제거)`` () =
    // WorkOrderStrategy 제거 — IoList 만 specialized. WorkOrder 는 일반 KB 자료.
    let names =
        XlsxSignatureClassifier.strategies
        |> List.map (fun s -> s.Name)
    Assert.Equal(1, List.length names)
    Assert.Equal<string list>([ "IoListStrategy" ], names)

// ── WorkOrderStrategy fixture + signature + build (PR-I2) ────────────────

/// ***REDACTED***2 조립작업서 signature 충족 xlsx fixture.
...
[<Fact>]
let ``WorkOrderStrategy Build — 매치 시 markdown 반환 + 머리말 6행 강제`` () =
    withTempPath ".xlsx" (fun path ->
        makeWorkOrderFixture path
        let extracted = extractDocument path
        let strategy = WorkOrderStrategy() :> IXlsxStrategy
        ...
        | Rejected entry -> Assert.Fail(sprintf "Build 가 Rejected 반환 — reason=%s" entry.Reason)
        | Built markdown ->
            // 머리말 6행 — 2026-05-27 canary 1행 patch 박제 baseline.
            let lines = markdown.Split([| '\n' |], 9)
            Assert.True(lines.Length >= 6, "최소 6 행 머리말 필요 (canary + 기존 5행)")
            Assert.Contains("canary:", lines.[0])
            Assert.Contains("pong: summary/", lines.[0])
            Assert.Contains("generated by WorkOrderStrategy v", lines.[1])
            Assert.Contains("source:", lines.[2])
            Assert.Contains("docId:", lines.[2])
            Assert.Contains("signature: WorkOrderStrategy:", lines.[3])
            Assert.Contains("estimated-tokens:", lines.[4])
            Assert.Contains("strategy-version: WorkOrderStrategy v", lines.[5]))
...
[<Fact>]
let ``XlsxSignatureClassifier — WorkOrder fixture 는 더 이상 dispatch 안 됨 (2026-06-04 IoList 단독)`` () =
    // WorkOrderStrategy 제거 — WorkOrder fixture 는 IoList signature 미매치 → Unmatched/NearMiss (specialized 아님).
    withTempPath ".xlsx" (fun path ->
        makeWorkOrderFixture path
        let extracted = extractDocument path
        let result = XlsxSignatureClassifier.classify path extracted
        match result with
        | Matched (name, _) -> Assert.Fail(sprintf "WorkOrder 가 specialized 매치되면 안 됨 (일반 자료) — %s" name)
        | NearMiss entries -> Assert.All(entries, fun e -> Assert.True(e.Score < e.Threshold))
        | Unmatched | RejectedByStrategy _ -> ())
...
[<Fact>]
let ``자료 C 실파일 — 존재 시 specialized 미매치 (2026-06-04 WorkOrder 일반 자료 전환)`` () =
    // WorkOrderStrategy 제거 — 자료 C(조립작업서)는 classifier 에서 더 이상 specialized 매치 안 됨.
    // 일반 KB 자료로 색인 (text dump + summary + 검색). WorkOrderStrategy 코드 자체 회귀는
    // 위 strategy-단위 테스트(signature/Build)가 보존 (재등록 대비).
    if File.Exists sampleCPath then
        ...
        // IoList 단독 classifier — 자료 C(작업서)는 IoList signature 미매치. Matched 면 실패, 그 외는 정상.
        // RejectedByStrategy 는 IoListStrategy.Build 가 항상 Built 라 도달 불가지만 패턴 완전성 위해 명시(재등록 대비).
        match result with
        | Matched (name, _) -> Assert.Fail(sprintf "자료 C 가 specialized 매치되면 안 됨 (일반 자료) — %s" name)
        | NearMiss entries -> Assert.All(entries, fun e -> Assert.True(e.Score < e.Threshold))
        | Unmatched | RejectedByStrategy _ -> ()
...
let private sampleAPath =
    @"F:/Git/dualsoft/secrets/KBSamples/core/4-3. ***REDACTED***2 SIDE OUTER SV IO LIST STD MAP.xlsx"
...
let ``자료 A 실파일 — 존재 시 IoListStrategy 매치 + markdown 박제 e2e`` () =
    if not (File.Exists sampleAPath) then
        ...
        | Matched (name, markdown) ->
            Assert.Equal("IoListStrategy", name)
            // 머리말 6행 — 2026-05-27 canary patch 박제 baseline (line 0 = canary, line 1~5 = 기존 5행).
            let headLines = markdown.Split([| '\n' |], 7)
            Assert.True(headLines.Length >= 6)
            Assert.Contains("canary:", headLines.[0])
            Assert.Contains("generated by IoListStrategy v", headLines.[1])
            Assert.Contains("strategy-version: IoListStrategy v", headLines.[5])
            // 6 컬럼 표 헤더.
            Assert.Contains("| Word | Direction | Tag | DataType | Address | Symbol |", markdown)
