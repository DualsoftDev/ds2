...
[<Fact>]
let ``XlsxSignatureClassifier — PR-I1 시점 strategy list = IoListStrategy 단독`` () =
    // todo §2.1 의 "strategy list 에 IoList 외 추가 0" 가드.
    Assert.Equal(1, List.length XlsxSignatureClassifier.strategies)
    Assert.Equal("IoListStrategy", XlsxSignatureClassifier.strategies.Head.Name)
