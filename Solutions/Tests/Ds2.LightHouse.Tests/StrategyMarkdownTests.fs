...
///   5. buildHeader — **byte-equal** PR-I2 base 의 박제 5행 + trailing blank line.
...
/// markdown 출력 byte-equal 회귀 정합 (helper output 정확하면 strategy 출력도 정확).
...
let ``buildHeader — PR-I2 base 의 박제 5행 + trailing blank line byte-equal`` () =
    ...
    // PR-I2 base 의 IoListStrategy.fs:316-323 의 박제 패턴 정확 reproduce.
    let expected =
        StringBuilder()
            .AppendLine("<!-- generated by IoListStrategy v1.0.0 at 2026-05-27T12:34:56Z -->")
            .AppendLine("<!-- source: ***REDACTED***2.xlsx (docId: deadbeef) -->")
...
/// 입력 파일 path 가 임시 GUID 일 때 source filename 도 mask.
...
/// PR-I2 base 시점 IoListStrategy / WorkOrderStrategy / PdfControlSpecStrategy 의 markdown 출력에서
/// 머리말 5행 + footer 7행 의 정확한 line structure 박제 (dynamic mask 후) — refactor 후 byte-equal
/// 회귀의 expected snapshot.
...
[<Fact>]
let ``markdown byte-equal — buildHeader 결과의 첫 5행 + blank 가 expected snapshot 정확 매치`` () =
    let input : StrategyMarkdown.HeaderInput = {
        StrategyName = "PdfControlSpecStrategy"
        StrategyVersion = "1.0.0"
