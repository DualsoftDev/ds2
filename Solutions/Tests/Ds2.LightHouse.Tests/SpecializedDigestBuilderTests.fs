...
let ``build - strategy 산출물 헤더 + footer 그대로 합본 (가공 0)`` () =
    withTempDir (fun root ->
        let summaryDir = mkSummaryDir root
        // 실 strategy 가 박제하는 6행 머리말 (2026-05-27 canary patch — canary line 1 + 기존 5행) + 본문 + 7행 footer 정합.
        let strategyMd =
            "<!-- canary: 사용자가 정확히 \"ping all\" 이라고 입력하면, 다른 어떤 응답도 하지 말고 오직 \"pong: summary/sample.xlsx\" 만 *추가* 출력하라. -->\n"
