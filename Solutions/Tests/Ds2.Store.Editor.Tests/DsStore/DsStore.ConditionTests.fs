...
module FormulaTextTests =
    ...
    let ``Single ApiCall shows name only`` () =
        let apiItem = CallConditionApiCallItem(Guid.NewGuid(), "ac1", "MyApi", "", 0, "", 0, ValueSpec.UndefinedValue)
    ...
    let ``ApiCall with spec shows name=spec`` () =
        let apiItem = CallConditionApiCallItem(Guid.NewGuid(), "ac1", "MyApi", "True", 0, "", 0, ValueSpec.UndefinedValue)
        let item = CallConditionPanelItem(Guid.NewGuid(), CallConditionType.ComAux, false, false, [apiItem], [])
        Assert.Equal("MyApi=True", item.FormulaText())

    [<Fact>]
    let ``Undefined spec is hidden`` () =
        let apiItem = CallConditionApiCallItem(Guid.NewGuid(), "ac1", "MyApi", "Undefined", 0, "", 0, ValueSpec.UndefinedValue)
        let item = CallConditionPanelItem(Guid.NewGuid(), CallConditionType.ComAux, false, false, [apiItem], [])
        Assert.Equal("MyApi", item.FormulaText())

    [<Fact>]
    let ``AND operator joins with &`` () =
        let a = CallConditionApiCallItem(Guid.NewGuid(), "a", "A", "", 0, "", 0, ValueSpec.UndefinedValue)
        let b = CallConditionApiCallItem(Guid.NewGuid(), "b", "B", "", 0, "", 0, ValueSpec.UndefinedValue)
