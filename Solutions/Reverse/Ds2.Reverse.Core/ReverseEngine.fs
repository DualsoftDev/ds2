                    ...
                    if sId <> tId && processedPairs.Add(sId, tId) then
                        ...
                        if pwS = pwT then
                            ...
                            let logicStr =
                                ...
                                | true, v -> Some v
                                | _ -> None
                            // B5 Logic-Hybrid recovery: capture 가 약함 (suff 0.4-0.85)
                            // + logic strength 가 충분히 강함 (>= 0.7) → emit 인정.
                            // logic 이 더 강하면 capture threshold 더 완화 (>= 0.8 logic, 0.3 suff).
                            let finalDecision =
                                match finalDecision with
                                | Dropped(_, s) ->
                                    let suffOk =
                                        match logicStr with
                                        | Some ls when ls >= 0.8 -> s.Sufficiency >= 0.3
                                        | Some ls when ls >= 0.7 -> s.Sufficiency >= 0.4
                                        | _ -> false
                                    let neccOk =
                                        match logicStr with
                                        | Some ls when ls >= 0.7 -> s.Necessity >= 0.4
                                        | _ -> false
                                    if suffOk && neccOk && s.LagMean > 0.0 then
                                        let augmented = { s with PassesSeq = true }
                                        EmitSequential(1, augmented)
                                    else finalDecision
                                | other -> other
                            match finalDecision with
