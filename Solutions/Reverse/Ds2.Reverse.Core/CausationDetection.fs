...
module CausationDetection =
    ...
    let score (cfg: CausationConfig) (aTimes: int64 seq) (bTimes: int64 seq) : CausationScore =
        ...
        if nA < cfg.MinFires || nB < cfg.MinFires then
            ...
            // B1.2 Multi-modal k-means stability — 3~6 peaks 의 다양한 분리도 인정.
            //
            // 방법: 1-D k-means with k=3..6.
            // 인정 조건 (각 k 별 통과 시):
            //   • 모든 cluster std < 50ms                   (mode 내부 안정)
            //   • 최소 cluster size >= max(8%, 5 elements)   (outlier 분포 아님)
            //   • 최소 cluster 간 separation >= 80ms        (분리 명확)
            //
            // separation: 인접 centroid 거리 의 최소값.
            let kmeansStable =
                // modeCount 검사 제거 — k-means 자체로 분류 가능.
                if lags.Count < 15 then false
                else
                    ...
                    let runKmeans (k: int) =
                        ...
                        // 각 cluster size + std + centroid
                        let clusters =
                            [| for j in 0 .. k - 1 ->
                                let mems =
                                    [| for i in 0 .. n - 1 do
                                        if assign.[i] = j then yield sorted.[i] |]
                                if mems.Length = 0 then 0, 999.0, 0.0
                                else
                                    let m = Array.average mems
                                    let s = sqrt (Array.averageBy (fun x -> (x - m) ** 2.0) mems)
                                    mems.Length, s, m |]
                        clusters
                    // try k=3..6. 최소 cluster size >= max(8%, 5), 모든 std < 50, sep >= 80.
                    let kCandidates = [ 3; 4; 5; 6 ]
                    let minSizeThreshold = max 5 (n * 8 / 100)
                    kCandidates
                    |> List.exists (fun k ->
                        if k * minSizeThreshold > n then false
                        else
                            let clusters = runKmeans k
                            let minSize = clusters |> Array.map (fun (sz, _, _) -> sz) |> Array.min
                            let maxStd = clusters |> Array.map (fun (_, s, _) -> s) |> Array.max
                            let sortedC =
                                clusters
                                |> Array.map (fun (_, _, c) -> c)
                                |> Array.sort
                            let minSep =
                                if sortedC.Length < 2 then 9999.0
                                else
                                    [| for i in 0 .. sortedC.Length - 2 ->
                                        sortedC.[i + 1] - sortedC.[i] |]
                                    |> Array.min
                            // separation 이 within-cluster std 의 2배 이상 — uniform spread 거부.
            ...
            let stable =
                ...
                || kmeansStable
            // Polling detector — 세 단서로 판정.
            //   1) Rate ratio: A/B >= 5x → polling 강한 의심.
            //   2) Inter-arrival CV: A 의 간격 cv < 0.10 + ratio >= 2.0 → polling.
            //   3) FFT-based periodicity: A 의 binned histogram 의 peak ratio > 5 → polling.
            let rateRatio =
                if nB > 0 then float nA / float nB else 0.0
            let isPollingByRate = rateRatio >= 5.0
            let isPollingByCV =
                if a.Length < 5 || rateRatio < 2.0 then false
                else
                    let cvIntervals = SignalAnalysis.interArrivalCV a
                    cvIntervals < 0.10
            // FFT — 매우 엄격: rate ratio >= 2.5 AND a.Length >= 12.
            let isPollingByFFT =
                if a.Length < 12 || rateRatio < 2.5 then false
                else
                    let binMs =
                        match cfg.CycleHintMs with
                        | Some c -> max 10L (c / 20L)
                        | None -> 100L
                    let isPoll, _, _ = SignalAnalysis.detectPollingFromTimes a binMs
                    isPoll
            // Domain library — 알려진 polling 패턴 + rate ratio gating.
            let isPollingByDomain =
                if a.Length < 8 || rateRatio < 2.0 then false
                else PollingPatterns.isKnownPolling a
            let isPolling =
                isPollingByRate || isPollingByCV || isPollingByFFT
                || isPollingByDomain
            let passesSeq =
                not isPolling
    ...
    /// Phase 9C Bayesian Prior Confidence — prior(logic, source-type) + likelihood(capture).
    ///
    /// Prior 구성:
    ///   - logic strength → prior probability (0.5 default if None, else strength)
    ///   - polling 의심 (high rate ratio) → prior 0.2 (낮은 prior)
    ///   - High N → tighter posterior
    ///
    /// Likelihood: capture evidence quality.
    ///   P(observed | true causation) = suff * necc * stability_factor
    ///   P(observed | not causal) = low (constant 0.3)
    ///
    /// Posterior = Bayes update:
    ///   P(causal | obs) = P(obs | causal) * P(causal) /
    ///                     [P(obs | causal) * P(causal) + P(obs | ~causal) * P(~causal)]
    let bayesianConfidence
            (sco: CausationScore)
            (logicStrength: float option)
            (suspectPolling: bool) : ArrowConfidence =
        // Prior
        let logicPrior = logicStrength |> Option.defaultValue 0.5
        let pollPenalty = if suspectPolling then 0.3 else 1.0
        let prior = max 0.05 (min 0.95 (logicPrior * pollPenalty))
        // Likelihood given causal
        let stabilityFactor =
            if sco.PassesSeq || sco.PassesGrp then 1.0
            elif sco.LagCv < 0.5 then 0.7
            else 0.4
        let pObsCausal =
            max 0.05 (min 0.99 (sco.Sufficiency * sco.Necessity * stabilityFactor))
        // Likelihood given not causal (random / spurious)
        let pObsNotCausal = 0.30
        // Bayes update
        let numerator = pObsCausal * prior
        let denominator = numerator + pObsNotCausal * (1.0 - prior)
        let posterior =
            if denominator < 1e-9 then 0.5
            else max 0.0 (min 1.0 (numerator / denominator))
        // N reliability multiplier
        let nReliability =
            if sco.NA < 10 then 0.7
            elif sco.NA < 30 then 0.9
            else 1.0
        let scaled = posterior * nReliability
        let tier =
        ...
        let evidence =
            [
                ...
                if suspectPolling then "polling_suspect"
                sprintf "prior=%.2f" prior
                sprintf "lik=%.2f" pObsCausal
                sprintf "n=%d" sco.NA
            ]
