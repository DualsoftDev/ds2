...
- [ ] **Codex `cd:` 인자 + sandbox 격리 (보안)** — 현재 `CreateCodexProvider` 가 `cd: None` + `sandbox_mode=danger-full-access` 로 spawn. codex 가 Promaker bin 디렉토리에서 실행 + file system / network 자유 접근. 우리 use case 는 codex 가 Promaker MCP tool 만 호출하는 것이 의도 — codex 의 file/shell/web 도구는 미사용. **격리 방법**: (a) `cd` 를 `%TEMP%/Promaker/codex-workspace-<sessionId>/` 같은 빈 폴더로, (b) codex 0.125 의 sandbox bug 가 future 버전에서 fix 되면 workspace-write 로 격상, (c) codex 측 tool 화이트리스트 (mcp__promaker__* 만 활성) 검토. 사용자 신뢰 향상 + codex 가 의도치 않은 시스템 변경 회피
- [ ] **Codex session cleanup 정책** — `--ephemeral` 을 default off 로 변경 (multi-turn resume 보장 — `no rollout found for thread id` fix). 부작용: 사용자 `~/.codex/sessions/` 에 thread rollout 누적. 후속 처리 옵션: (a) `CODEX_HOME` 환경변수로 인스턴스별 임시 디렉토리 격리 + 종료 시 삭제, (b) `LlmChatViewModel.Reset()` 시 마지막 sessionId 의 rollout 파일만 cleanup, (c) `--ephemeral` 토글 UI 노출 후 사용자 선택. ChatGPT subscription 사용량 청구 영향은 없음 (rollout 은 local file)
- [ ] **MCP HTTP ready 신호 wait** — Codex 는 mcp_servers init 상태 stdout 미노출. Kestrel startup 후 HTTP GET / retry 또는 첫 tool call 실패 시 retry 정책. McpHostService 에 ready 신호 export 필요
...
- 현 단계: **Phase 2 — Codex production 통합 완료 (mutation 시나리오 e2e 통과)** (2026-05-06, 빌드 통과 + 90/90). Codex provider 가 ChatPanel ComboBox 에서 선택 가능 + multi-turn chat + MCP tool 호출 + Promaker 모델 mutation 까지 동작 확인. 핵심 정착 포인트 5종 (config override `mcp_servers.promaker.url` + `http_headers` inline table + `approval_policy="never"` + `sandbox_mode="danger-full-access"` + `experimental_instructions_file=<temp Phase1c>`) + 인자 정착 (`--ephemeral` off, PATH 정규화, OS 분기 PATHEXT 검색). 다음: MCP HTTP ready 신호 wait, codex session cleanup 정책, Codex `cd:` 보안 격리, B-3 (rename cascade), m1 (LlmTurnContext AsyncLocal), provider fallback 자동 정책.
- 결정 상태:
