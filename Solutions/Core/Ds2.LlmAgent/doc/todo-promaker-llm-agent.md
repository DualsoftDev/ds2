...
- [x] Codex CLI provider — concrete + parser + skeleton 모두 완료. **C-1** (인자 inventory + CodexCliOptions/CodexCliArgs.build), **C-2** (실제 spike — 4 packet 종류 / stderr 분리 / MCP HTTP inline 등록 / connect 실패 graceful), **C-3** (`CodexStreamJsonParser` 4 packet → LlmEvent 매핑 + `CodexCliProvider` Send/EnsureCli/SessionId/ClearSession — ClaudeCliProvider 와 동일 패턴, single `item.completed` → `AssistantDelta` 1회). 인터페이스 재설계 인정 (review M5) — ClaudeCli 와 별개 type. **남은 후속**: Promaker UI 측 provider 선택 (ClaudeCliProvider vs CodexCliProvider dispatch), Codex MCP HTTP ready 신호 별도 wait (in-process Kestrel HTTP GET retry), `instructions` override 정책 (12k default system prompt 사용 vs Promaker 측 SystemPrompt.Phase1c 로 완전 override). production 통합 검증은 Promaker UI 측 dispatch 추가 후 사용자 e2e
...
- 현 단계: **Phase 2 첫 사이클 + Codex provider concrete 완료** (2026-05-06, 빌드 통과 + unit test 80/80 통과). 진척: (A) `list_projects` / (B-1) `RemoveEntity` cascade / (B-2) `RenameEntity` System+ApiDef / (C-1) `CodexCliOptions`+`CodexCliArgs.build` skeleton / (C-2) 실제 codex exec spike (4 packet + MCP HTTP 양립성) / (C-3) `CodexStreamJsonParser` + `CodexCliProvider` concrete + 13 parser test. 다음: Promaker UI 측 provider 선택 (Claude vs Codex dispatch), production MCP HTTP 통합 검증, B-3 (Flow/Work/Call rename child cascade), m1 (LlmTurnContext AsyncLocal — 다중 dock 시점), provider fallback 정책.
- 결정 상태:
