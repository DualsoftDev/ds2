...
- [ ] **API key 입력 UI** — 다이얼로그 또는 chat panel header 안 입력 박스. 현재는 LlmApiConfig 의 SetApiKey 가 노출되어 있으나 UI 진입점 미연결. Anthropic / OpenAI 두 provider 만 필요 (Ollama 는 무필요)
- [ ] **모델명 / Ollama base URL 입력 UI** — LlmApiConfig.AnthropicModel / OpenAiModel / OllamaModel / OllamaBaseUrl. 현재는 default 값 + 코드 직접 수정만
...
- 현 단계: **Phase 2 — API providers 3종 통합 + review 반영** (2026-05-07, 빌드 통과 + 92/92 회귀 통과). Microsoft.Extensions.AI `IChatClient` 추상 채택. 단일 `ApiChatProvider` (C#) 가 Anthropic / OpenAI / Ollama 공통. `LlmProviderKind` 5종 (Claude / Codex / AnthropicApi / OpenAiApi / Ollama). `LlmApiConfig` DPAPI 암호화. **review 반영 (2026-05-07)**: (1) `ApiChatProvider.SendImpl` firstTurn 순서 — `ListToolsAsync` 실패 시 sessionId/history silent corruption 방지. tools 먼저 로드 → 성공 후 state 확정. (2) `ApiProviderFactory.CreateInternalAsync` 도입 — 3 public 메소드 (Anthropic/OpenAI/Ollama) 의 builder + mcp 결선 + ctor 7줄 중복 제거. (3) `AnthropicModel` default `claude-sonnet-4-5-20250929` → `claude-sonnet-4-6`. (4) `AnthropicKey`/`OpenAiKey` 우회 패턴 (private const + public property) → `public const string` 단순화. **보류**: (5) Send 도중 cancel/throw 시 history user→user 연속 가능 — reviewer 도 코드 변경 비권장 (CLAUDE.md 예외 처리 자제 부합). e2e 검증 권장 사항. **다음**: API key 입력 UI / 모델 선택 UI / API provider 단위 테스트 / B-3 (rename cascade) / provider fallback 정책.
- 결정 상태:
