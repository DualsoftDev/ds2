...
- 현 단계: **Phase 2 — Codex 보안/lifecycle 정착 완료** (2026-05-06, 빌드 통과 + 90/90). Codex `cd:` 워크스페이스 격리 + `CODEX_HOME` 인스턴스별 격리 + MCP HTTP self-probe ready 신호 wait 모두 적용. 워크스페이스 디렉토리 (`%TEMP%/Promaker/codex-workspace-<guid>/`) 가 instructions.md / codex-home 서브디렉토리를 모두 holding → DisposeAsync 재귀 삭제 1회로 일괄 cleanup. McpHostService 가 startup 마지막에 routing pipeline 까지 self-probe (50ms backoff/2초 timeout) → silent fallback (Hot-fix-4 시나리오) 회피. **사용자 e2e 검증 권장 항목**: (1) Codex 가 격리 워크스페이스에서 mutation 정상 동작 확인, (2) `CODEX_HOME` 격리로 사용자 `~/.codex/sessions/` 청결 유지 확인 — 미동작 시 후속 패스에서 (b) Reset() rollout 직접 삭제로 전환. 다음: B-3 (rename cascade), m1 (LlmTurnContext AsyncLocal), provider fallback 자동 정책.
- 결정 상태:
