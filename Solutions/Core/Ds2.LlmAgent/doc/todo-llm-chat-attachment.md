...
- **deferred C-2**: PDF 미지원 provider (Codex/Ollama) 의 fallback 정책 — 자동 텍스트 추출 vs 확인 dialog
- **별도 phase**: Ollama vision 모델 동적 capability 갱신 — `EnsureCli` 시점 `/api/show` 조회 (정책 7), 현재 정적 placeholder TextOnly
- **UX 개선**: outer `catch (Exception ex)` single-file failure 의 user-visible 안내 (자가 검열 M-2 잔여)
