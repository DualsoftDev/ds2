...
- 본 디렉토리 (`Solutions/Core/Ds2.LlmAgent/`) 는 Phase 2 까지 완료된 상태 — LLM Chat 패널이 dock UserControl 로 통합되어 있고 6종 provider (Claude CLI / Codex CLI / Anthropic API / OpenAI API / Ollama / Groq F-1 spike) dispatch 가 동작 중.
...
관련 코드 위치 (rev 19 line 재측정 — `--inspect-diff 7` review 에서 line drift 확인 후 sweep):
- `Apps/Promaker/Promaker/Controls/Llm/LlmChatPanel.xaml(.cs)` — dock UserControl, 첨부 UI 추가 지점
- `Apps/Promaker/Promaker/ViewModels/LlmChatViewModel.cs:437` — `SendAsync` 진입점. `:445` `var snapshotProvider = _provider`, `:523` `snapshotProvider.Send(msg, _cts.Token)`
- `Apps/Promaker/Promaker/LlmAgent/Api/ApiChatProvider.cs:63` — `_history: List<ChatMessage>` 누적, `:113` Send 시그니처, `:162` `_history.Add(new ChatMessage(ChatRole.User, ...))`. `:200-261` rev 19 try/finally + partial collected flush (cancel 시 user-only 회피)
- `Apps/Promaker/Promaker/MainWindow.xaml.cs:188-218` — `Window_DragOver` / `Window_Drop` (단일 파일 import — bubble 차단 필요)
- `Solutions/Core/Ds2.LlmAgent/LlmProvider.fs:33` — `ILlmProvider.Send` 시그니처 (`LlmUserMessage * CancellationToken -> IAsyncEnumerable<LlmEvent>`)
- `Solutions/Core/Ds2.LlmAgent/ClaudeCliProvider.fs:70` — `let stdinBody = ClaudeStreamJsonInput.encode prompt nonTextAttachments`, `:95` `Stdin = Some stdinBody`
- `Solutions/Core/Ds2.LlmAgent/CodexCliProvider.fs:87` — `member this.Send` 진입, `:113-129` rev 19 spool try/with (partial leak 방어), `:155` `Stdin = None`, `:159-170` rev 19 `static member SweepStale ()` (App.OnStartup 1회 호출)
...
| OpenAI API | gpt-4o vision | 20MB | **O** (V-1 closed rev 15) | 32MB / 100p | `{type:"file", file:{filename, file_data:"data:application/pdf;base64,..."}}` content block — Microsoft.Extensions.AI.OpenAI 10.5.2 자동 wire |
...
- [x] **S-4** (2026-05-09 closed, rev 15): 5종 SDK 의 multimodal wire raw source 직접 검증 — Anthropic 공식 12.20.0 (R5 ambiguity 해결: tghamm 아닌 anthropics/anthropic-sdk-csharp) `AnthropicClientExtensions.cs:890-916` image/pdf/text 자동 매핑 / Microsoft.Extensions.AI.OpenAI 10.5.2 `application/pdf` → `CreateFilePart` 자동 / OllamaSharp 5.4.25 `MicrosoftAi/AbstractionMapper.cs:232` image-only (PDF silent drop, EnforceCapabilityOrFail 가 차단). **raw HttpClient 우회 어댑터 = 모두 불필요**. **V-1 RESOLVED** — OpenAI Chat Completions PDF native (32MB / 100p) + SDK 자동 wire
...
### Phase 3b — PDF (rev 15, 2026-05-09 완료)

- [x] **S-4** spike 완료 후 진입 (rev 15)
- [x] `Attachment` DU 의 PDF case 활성 — UI chip + capability + size cap + 페이지 cap 검증 추가
- [x] PDF 첨부 chip + size cap 32MB (`caps.MaxPdfBytes`) 검증 — `LlmChatViewModel.Attachments.cs:ClassifyPathSync` PDF 분기
- [x] **PDF 페이지 cap 검증** (정책 11) — `PdfPig` 0.1.14 (Apache 2.0) 도입. `using var doc = UglyToad.PdfPig.PdfDocument.Open(bytes); doc.NumberOfPages` 로 페이지 수 추출 → `caps.MaxPdfPages` 비교. **현재 100p 단일 적용** (Anthropic 200K context Opus 4.7 / Sonnet 4.6 + OpenAI 100p 명시값 기준 보수). 모델별 200K vs 그 외 분기 (= 600p) 는 v-next — 모델 분류 SSOT 결정 후 진입
...
1. **Phase 3a-pre spike S-1 / S-2 / S-3** ✅ rev 4 — Claude/Codex CLI 채널 확정
2. **Phase 3a commit-1** ✅ rev 5 — `LlmMessage.fs` 신설 (dead code)
3. **Phase 3a commit-2** ✅ rev 5 — `Send` 시그니처 마이그레이션 (5종 provider + ViewModel)
4. **Phase 3a commit-3** ✅ rev 6 — Validator + TokenEstimator + System prompt
5. **Phase 3a commit-4..6b** ✅ rev 7~13 — UI 점증 + 회귀 fix
6. **Phase 3a-pre S-4 spike + Phase 3b PDF wire** ✅ rev 15 — V-1 RESOLVED + PdfPig 도입 + capability schema 확장 + UI 활성
...
- [x] **V-1** (2026-05-09 closed, rev 15): OpenAI API PDF 직접 입력 — `{"type":"file", "file":{"filename":..., "file_data":"data:application/pdf;base64,..."}}` content block 으로 native 지원, 32MB / 100p / request. Microsoft.Extensions.AI.OpenAI 10.5.2 의 `OpenAIChatClient.ToChatMessageContentPart` 가 `application/pdf` MIME 분기 → `ChatMessageContentPart.CreateFilePart(...)` 자동 wire. `CapabilityPresets.OpenAiApiWire` = `ImagesAndPdf(20MB, 32MB, 100p)` 갱신
...
- Anthropic .NET SDK NuGet ID **R5 ambiguity 해결** (rev 15 / S-4 spike) — 사용 중인 `Anthropic` 12.20.0 = 공식 `anthropics/anthropic-sdk-csharp` (NuGet `id=Anthropic`, repository url 검증 완료). tghamm/Anthropic.SDK 아님. CLAUDE.md 갱신 권장
