...
> - 다음 리비전 trigger: 사용자가 결정 1~5 변경 / 본 todo 가 phase 진입 / `2.modeling.md` 영속화 후 / WithCyl.json SSOT 옵션 결정 후 / Core ensureSystem/ensurePendingWork 시그니처 변경 caller 영향 최종 확정 후 / **C5 Active/Passive 단일 vs 분리 비교 결정 후** / **rev 8 의 "본 phase 도구 범위 — 후속 확장 예정 entity" 목록의 IOTag / CallCondition / SubmodelProperty 등을 다룰 후속 todo 진입 시점** / **rev 10 의 §1 op 수 감소 실측치 — 실제 helper 도입 후 LLM 발행 op 수 측정 (Pass 5 baseline) 으로 추정치 → 측정치 갱신 시점** / **L3 (C# Promaker) 진입 후 D8 quota cascade C# 측 mechanism 확정 시점**.
