...
> - 다음 리비전 trigger: 사용자가 결정 1~5 변경 / 본 todo 가 phase 진입 / `2.Model.md` 영속화 후 / WithCyl.json SSOT 옵션 결정 후 / Core ensureSystem/ensurePendingWork 시그니처 변경 caller 영향 최종 확정 후.
...
| **8** | ⚙️ | **helper cascade op 의 quota 합산 정책 (rev 3 신설, rev 4·5 정정)**: 현행 `LlmTurnContext.cs:24` `MutationQuota = 50` + `MutationCallCount` 누적. **`IncrementMutationCount(int delta = 1)` 가 `LlmTurnContext.cs:50` 에 *이미 존재*** (review C1 주석에 batch 우회 DoS 표면 명시). batch tool 은 `ModelTools.cs:163` 에서 `inputs.Length - 1` delta 추가 차감 (RunMutation 이 +1 한 후) — 동일 패턴을 helper 에 적용. <br/>**rev 5 정정 (Critical 1)**: F# `ToolOperations.fs` 가 C# `Promaker.LlmAgent.LlmTurnContext` 를 *참조 불가* (assembly 의존 방향: Promaker → Ds2.LlmAgent). 따라서 quota delta 차감은 **C# `ModelTools.cs` 의 helper 메서드 안에서** 처리해야 함 (F# 측 ToolOperations 함수는 quota 무관). <br/>**채택 정책**: helper 진입점 (예: `ModelTools.AddCylinder` 등) 이 *RunMutation 진입 직후* helper 종류별 cascade op 수 사전 계산 → `ctx.IncrementMutationCount(cascadeOpCount - 1)` 추가 차감 (RunMutation 이 +1 했으므로). single helper / batch 안 helper 모두 동일. cascade op 수 산식 = §4.3 의 `3 + 2N + C(N,2) (all-pairs)` 또는 `3 + 2N + (N-1) (chain)`. 초과 시 `QuotaExceededException` 자동 throw → BATCH\_ERROR. |
...
## 4. ImportPlan / DU / Editor 측 검증 결과 — *추가 작업 불필요*

초안의 §4 "구현 진입 전 확인 필요" 였던 두 항목은 **모두 이미 충족됨** (review C1).
...
> **caller 영향 점검 의무 (M17)**: `linkCallsToDevicesMultiFlow` 시그니처를 *변경* 하는 경우 (예: `WiringMode` 인자 추가) 기존 caller 인 `Solutions/Core/Ds2.Core/Store/Mermaid/MapperTargetPlanning.fs:108-120` / `MapperTargets.fs:91,132,180` / CSV `CsvMapper.fs:95` 의 호출부 모두 영향. **권장**: 시그니처 변경 대신 `buildWorkArrows` 만 별도 함수 (`buildWorkArrowsAllPairs` 등) 로 신설하여 기존 caller 무수정 유지 — D7 의 all-pairs 옵션은 helper 측에서 분기.
...
| `Solutions/Core/Ds2.Core/Store/ImportPlan.Device.fs` | (rev 5 재정의) (1) `[<InternalsVisibleTo("Ds2.LlmAgent")>]` 는 `Ds2.Core/AssemblyInfo.fs:11` 에 **이미 존재** — 추가 작업 불필요. (2) **`ensureSystem` 시그니처 변경**: `systemType: string option` 인자 추가 → `system.SystemType <- systemType` 설정 (기존 line 88 직후). (3) **`ensurePendingWork` 시그니처 변경**: `workDuration: TimeSpan option` 인자 추가 → 500ms hardcoded 제거. (4) **`buildPassiveDeviceCascade` 신설 (internal)** — §4.2 의 시그니처. caller 가 PassiveSystemId + (apiName \* ApiDefId) list 받음. (5) **`buildWorkArrowsAllPairs` 신설** — D7 의 all-pairs 분기. (6) `WiringMode` DU 신설 (Chain / AllPairs / NoneMode). |
| `Solutions/Core/Ds2.Core/Store/Mermaid/MapperTargetPlanning.fs:108-120` / `MapperTargets.fs:91,132,180` | (rev 5) **caller 갱신 의무**: `linkCallsToDevices` / `linkCallsToDevicesMultiFlow` 가 내부적으로 `ensureSystem` / `ensurePendingWork` 를 호출하므로, `linkCallsToDevices*` 의 시그니처에도 `systemType?` / `workDuration?` 인자가 옵션으로 추가되거나, *기존 caller 호출부에서 default* (`None` / `Some 500ms`) 를 명시적으로 넘기도록 갱신. **권장**: `linkCallsToDevices*` 가 default 값을 내부적으로 채우는 형태로 시그니처 무변경 + Core 의 ensureSystem/ensurePendingWork 만 시그니처 변경 → caller 무영향. |
| `Solutions/Core/Ds2.Core/Store/Csv/CsvMapper.fs:95` | 동상 — `linkCallsToDevices*` 가 default 채우는 정책이면 호출부 무수정. |
...
| 파일 | 변경 |
|---|---|
| `Apps/Promaker/Promaker/LlmAgent/Prompts/1.SystemPrompt.md` | 도구 목록 갱신. canary 보존. (본문에 "16" 직접 hardcoded 는 검증 결과 없음) |
| `Apps/Promaker/Promaker/LlmAgent/Prompts/2.Model.md` | §1 표 / §2 ✅예시 / §3.3 표 / §4 모든 예시 helper 사용 형태로 재작성. deviceType 표기 정렬 (cylinder/clamp/lifter→"Unit", robot→"Robot", conveyor→"Conveyor"). canary 보존 |
...
| `Solutions/Core/Ds2.LlmAgent/llm-samples/**/*.md` (6개 파일) | sample 의 add\_system / add\_call 4-인자 호출 형태 갱신. golden test fixture 활용 시 필수. |
...
| `HelperCascadeTests.fs` (**신규 1**) | helper 4종 cascade 결과 검증. **expected = PassiveSystem + Flow + Work + ApiDef + ResetReset Arrow 갯수/관계** (rev 4: ApiCall 제외 — helper 책임 범위 외). cylinder N=2 = 1 System + 1 Link + 1 Flow + 2 Work + 2 ApiDef + 1 Arrow = 7 op. |
...
- queueAdd\* 함수: `Solutions/Core/Ds2.LlmAgent/ToolOperations.fs:188 queueAddSystem`, `:230 queueAddCall`, `:242 queueAddApiDef`, `:322 queueAddArrow`, **`:849` `dispatchBatchOp`** (rev 3 정정 — 직전 revision 의 `:861-` 은 dispatcher 안 `add_system` case 첫 줄 위치였음). `add_system` 허용 op 메시지 위치: `:168`, `:183`, `:191`, `:312`, **`:913` (runtime LLM 노출)**.
...
현재 `module internal ImportPlanDeviceOps` in `Ds2.Core/Store/ImportPlan.Device.fs`. LlmAgent 가 직접 호출하려면:
- 옵션 a) `module internal` → `module` (= public). visibility 광역 노출이라 비권장.
- 옵션 b) `[<InternalsVisibleTo("Ds2.LlmAgent")>]` AssemblyInfo 추가 (Ds2.Core 측). 단 LlmAgent 는 `Ds2.Editor` 만 ProjectReference 라 transitive 가능.
- 옵션 c) Editor 측에 thin wrapper 만들고 LlmAgent 가 그것 호출.

**권장**: **옵션 b** (visibility 조정 최소). Editor 의 Authoring 무수정 (CLAUDE.md 결정 7 정책).
