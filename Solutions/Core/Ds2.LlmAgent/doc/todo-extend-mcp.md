...
> - **rev 12 (2026-05-08)**: L1 (F# Core) 작업 진입 + 외부 reviewer 4건 review 처리 반영. (1) **L1.a `Ds2.LlmAgent.fsproj` 에 `Ds2.Core` 직접 ProjectReference 추가** — InternalsVisibleTo 효력화. CLAUDE.md L19 "ProjectReference = Ds2.Editor only" 표기 동시 갱신. (2) **L1.b `ImportPlan.Device.fs` 변경 4건 적용** — `WiringMode` DU 신설, `ensureSystem`/`ensurePendingWork` 시그니처 변경 (systemType / workDuration option 인자), `buildWorkArrowsBy` 공통화 (`buildWorkArrows` + `buildWorkArrowsAllPairs` 가 wrapper, reviewer #1 권고 수용 — 30 line 중복 제거), `buildPassiveDeviceCascade` 신설 (`let internal`). (3) Mermaid/CSV caller 무수정 정책 단일화 — `linkCallsToDevicesWithState` 내부에서 default 값 (None / Some 500ms) 채워 전달. (4) **D9 신설 (사용자 confirm)** — helper 의 동명 PassiveSystem 충돌 시 `invalidOp` (BATCH_ERROR). reviewer #2 의 silent ignore 우려 차단. ensureSystem 자체는 변경 안 함 → Mermaid/CSV idempotent 호환 보존, strict 검사는 `buildPassiveDeviceCascade` 진입점 한정. (5) `NoneMode` 명명은 reviewer #4 권고 거부 — F# Option.None 패턴 매칭 ambiguity 회피 + opposing `"none"` 1:1 대응 + todo §5.2 합의 명명. (6) **L2 미진입** — 다음 세션이 §7.3 의 L2 의존성 그래프 (queueAddActiveSystem/queueAddPassiveSystem 분리 → queueAddCall 시그니처 변경 → helper 4종 → dispatchBatchOp 갱신 → invalidOp 메시지 → D8 quota mechanism) 부터 진입. (7) **L1 자가 검열 결과 반영** (sub-agent 위임 review 후 사용자 confirm) — §3.1 D9 본문에 cross-project 한정 표기 (`passiveSystemsOf projectId` 한정, project 격리) + §3.2 Tier 1 helper 본문에 Flow 명명 의도 (`{name}_Flow` 의 implementation detail 성격, caller 미노출 forward-compat 보류) 보강.
> - **rev 13 (2026-05-08)**: L2 (F# LlmAgent) 작업 진입 + 자가 검열 4건 반영. (1) **L2 완료** — `Solutions/Core/Ds2.LlmAgent/ToolOperations.fs` 단일 파일 변경 (~270 line). queueAddSystem → queueAddActiveSystem / queueAddPassiveSystem 분리 (resolveFirstProjectId private helper 추출), queueAddCall 시그니처 (workId, apiDefId) 2-인자 + ApiDef→ParentSystem 역추적 + 룰 C 런타임 검증 + ApiCall cascade (`OriginFlowId = work.ParentId`), queueAddApiDef 에 txWorkId/rxWorkId 인자 추가, helper 4종 신설 (queueAddCylinder/Clamp/Robot/Device — runDeviceCascade 공통 wrapper), dispatchBatchOp 반환 시그니처 `(Guid option * string)` → `((string * Guid) list * string)` + 새 op 6종 추가 + add_system 제거 + helper 의 apiDef\*Ref 다중 ref 매핑, queueBatch 의 ref 등록 list-walk 화 (helper sub-ref ↔ main ref 충돌 시 invalidOp), validateRefName / getStringArrayArg / getIntArgOpt 헬퍼 분리. F# 빌드 0 warn / 0 error 통과. C# Promaker 측은 *예상대로* L3 미작업이라 ModelTools.cs 3건 에러 (L3 commit 시 회복). (2) **자가 검열 4건 반영** (사용자 직접 review): ① **Issue 1 (Major)** queueAddCall 의 룰 C 검증 match 분기에 `(None, None)` 통과 분기 존재 → 4-way 분기로 확장 + `(None, None)` = orphan 또는 LinkSystemToProject 누락 fail-safe invalidOp. ② **Issue 2 (Minor)** runDeviceCascade 의 apiNames 중복 미검사 → `HashSet<string>` 으로 빈 항목 검사와 통합 (중복 시 buildPassiveDeviceCascade 의 ensureApiDef 가 dedupe 하면서 결과 ApiDef 수가 입력 수와 어긋나 list_zip 깨지는 silent miscompile 차단). ③ **Issue 3 (Minor)** `ImportPlanDeviceOps.buildPassiveDeviceCascade` 의 반환 list 가 입력 apiNames 순서를 보존한다는 contract 가 docstring 에 미명시 → docstring 에 "반환 list 의 순서는 입력 apiNames 순서를 그대로 보존 — silent miscompile 회피 contract" 명시. 코드 자체 (`List.fold + List.rev`) 는 순서 보장됨. **본 docstring 보강은 L1 영역 (`Solutions/Core/Ds2.Core/Store/ImportPlan.Device.fs`) 변경이지만 doc-only 라 본 commit 범위에 포함**. ④ **Issue 4 (Minor)** quota 50 magic number drift 위험 → `[<Literal>] let MutationQuotaSync = 50` module-level 상수 신설 + `LlmTurnContext.cs:24` 와 sync 주석 명시 + queueAddRobot/queueAddDevice 두 곳 모두 상수 참조로 교체. (3) **글로벌 `~/CLAUDE.md` 자가 검열 지침 강화** — 본 작업 진행 중 자가 검열 step 누락 발생 (빌드 성공 직후 commit 결정 질문으로 진입) → 사용자가 root cause 분석 요청. 지침을 5 sub-bullet 으로 확장 (Trigger OR 결합 / 수행 시점 4-step 차단 / 차단 규칙 / 위임 mechanism / 리포트 형식). 본 commit 범위 외 (글로벌 user file). (4) **잔여 우려** — Issue 3 의 contract 위반 회귀 방어용 unit test 부재 (todo §5.6 의 HelperGuiParityTests / LlmTurnContextQuotaTests 작성 시점에 추가 권장). MutationQuotaSync drift 방어는 현재 주석만 — cross-project 상수 단일화는 변경 빈도 낮아 미적용. (5) **다음 세션 진입점** = §7.3 의 **L3 (C# Promaker)** — `ModelTools.cs` (16개 메서드 → Active/Passive 분리 + AddCall 시그니처 + AddApiDef 인자 추가 + helper 4종 신설 + D8 quota cascade single/batch 양 경로) + `PromakerToolNames.cs` (16 → 21) + `LlmChatViewModel.cs:35` xmldoc + `LlmTurnContext.cs:24` MutationQuota 와 F# `MutationQuotaSync` sync 검증.
> - **rev 14 (2026-05-08)**: L3 (C# Promaker) 작업 + 외부 reviewer (`--inspect-diff` 종합 리포트) 처리 + 회귀 fix 통합 commit. (1) **L3 완료** — `Apps/Promaker/Promaker/LlmAgent/Tools/ModelTools.cs` (대부분의 변경, +338/-19 → 통합 후 +315/-19), `PromakerToolNames.cs` (16→21 — `add_system` 제거 + active/passive + helper 4종), `ViewModels/LlmChatViewModel.cs:35` xmldoc 갱신. ModelTools 변경: AddSystem → AddActiveSystem / AddPassiveSystem(deviceType) 분리, AddCall(workId, apiDefId) 2-인자 + 룰 C 런타임 검증 안내 (single=VALIDATION_ERROR / batch=BATCH_ERROR wrap), AddApiDef 에 `txWorkId?`/`rxWorkId?` 인자 노출 (Microsoft.FSharp.Core.FSharpOption 변환), helper 4종 신설 (AddCylinder/Clamp/Robot/Device — `RunPairedDeviceCascadeWork` + `RunListDeviceCascadeWork` 통합 본문 2개로 ~150 line 중복 제거 + M2/M3 적용), D8 quota cascade single/batch 양 경로 (single = `RunMutation` work delegate 안에서 `IncrementMutationCount(cascadeOpCount-1)` 추가 차감, batch = `ApplyOperations` inputs walk 시 helper op 별 사전 합산 차감), `IsHelperOp`/`CalcCascadeOpCount` 헬퍼 추가 (CalcCascadeOpCount 는 F# `ToolOperations.cascadeOpCount` SSOT 호출 — M3), `ParseStringArrayArg`/`ToFSharpList`/`ParseDurationMs`/`FormatApiDefMapping` 헬퍼 분리. (2) **외부 reviewer (`--inspect-diff` 종합 리포트) 처리** — Critical 2건 (C1 prompt drift / C2 BatchTests stale) + Major 7건 검증 후 적용. C1 = Prompt md 3 파일 (1.entities/2.modeling/3.tooling) 갱신 — helper 사용 형태로 매핑표 / 룰 / §3.5 / §4 예시 / §5 self-check 의미 반전 (사용자 confirm) + GUI canonical 절을 helper 권고로 전환 + 11-op → 9-op cylinder 예시. C2 = BatchTests.fs 5 fixture cutover. M1 = cylinder/clamp Sanitize/quota 순서 (refactor 본문에 흡수). M2 + M3 = helper 4종 통합 + cascade 산식 SSOT (`ToolOperations.cascadeOpCount` F# 단일 함수, queueAddRobot/Device 가 호출). M4 = `Solutions/Core/Ds2.LlmAgent/CLAUDE.md` "16개" 4곳 → 21 (drift 동시 commit 정책 §7.3). M5 = single helper validateRefName 보강 = 후속 phase 보류. M7 = `AddPassiveSystem` / `AddDevice` 의 deviceType `SanitizeOrThrow`. opposing trim normalize (AddRobot/Device). AddCall Description "BATCH_ERROR" wording → "거부 (single=VALIDATION_ERROR / batch=BATCH_ERROR wrap)" 보강. (3) **회귀 fix 2건** (외부 reviewer 가 별개 issue 로 분리했으나 본 PR 의 결과 회귀로 재분류) — ① `dispatchBatchOp` 반환 시그니처를 `(Guid option * (string * Guid) list * string)` 로 변경 + queueBatch 가 primaryId 별도 필드로 BatchOpResult.Id 노출 → ref 없는 add_project 의 Id.IsSome 검증 회복. ② ValidateModelTests `queueAddApiDef plan store "TODO" sysId |> ignore` 의 partial application silent skip — `None None` 인자 추가 (`RemoveRenameTests` 도 동일 cutover 적용 완료 — rev 13 작업). (4) **Tests 보강** — `PromakerToolNamesDriftTests`: 16→21 sanity + snake_case 케이스 (add_active_system/add_passive_system/add_cylinder). `BatchTests.fs`: 5 fixture cutover (add_passive_system + add_call 2-인자 + add_active_system) + 풀세트 plan op 11→15 (Active+Passive 각각 LinkSystemToProject + AddCall 마다 AddApiCall). (5) **빌드 / 테스트 결과** — F# `Ds2.LlmAgent.fsproj` 0 warn / 0 error, C# `Promaker.csproj` (CoreCompile, no-incremental) 0 error (분석기 버전 경고 2개 = pre-existing OllamaSharp), Tests 147/147 PASS. (6) **commit 단위** — Layer commit 정의 (todo §7.1) 의 L3 + Drift/Prompt + Tests 묶음을 *원자 commit* 으로 처리 (todo §7.3 의 "Drift 동시 갱신 = 반드시 동시 commit (false-green 회피)" 정책 + reviewer C1 의 "다음 turn LLM 이 stale 학습 회피" 우려). (7) **잔여 우려 / 후속** — todo §5.6 의 helper 단위 테스트 (HelperCascadeTests / HelperGuiParityTests / LlmTurnContextQuotaTests / ImportPlanApplyApiCallTests / PromptCanaryTests / DeviceTypeDriftTests) 신규 6종 작성은 후속 phase. M5 (single helper validateRefName) / M6 (MutationCallCount rollback docstring) 도 후속 phase. WithCyl.json SSOT 영속화 (§0.2) 미해결 (옵션 B `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` 권장 — 사용자 결정 후 진입). 본 commit 후 **본 todo 의 §5 영향 파일 체크리스트 대부분 PASS** — 미PASS 영역은 §5.6 신규 6 tests + WithCyl.json SSOT 만.
> - **rev 15 (2026-05-08)**: §5.6 신규 6 tests 일괄 작성 + §0.2 옵션 B 채택 (WithCyl.json SSOT 영속화) + done-extend-mcp.md 신규 분리 (todo §7.5 권장). (1) **§0.2 옵션 B 처리** — `Solutions/Core/Ds2.LlmAgent/doc/WithCyl.json` (tracked) → `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` 으로 mv (commit 시점에 git rename detection 으로 history 보존). `F:\tmp\WithCyl.json` 절대경로 의존 제거. (2) **§5.6 신규 6 tests 작성 완료** — `HelperCascadeTests.fs` (13 facts, ~210 line — cylinder/clamp/robot/device cascade 갯수 + SystemType + ArrowType.ResetReset + Duration 500ms + apiNames sanitize + D9 동명 충돌), `HelperGuiParityTests.fs` (2 facts, ~165 line — Fixtures/WithCyl.json 과 helper cylinder 의 *구조 parity* 8 항목 비교 — naming 외), `PromptCanaryTests.fs` (4 facts — 4 prompt md 첫 줄 canary 회귀 방어), `DeviceTypeDriftTests.fs` (3 facts — KnownNames 19종 + helper deviceType Unit/Robot/Conveyor sync, 텍스트 regex 파싱 / ProjectReference 부담 회피), `LlmTurnContextQuotaTests.fs` (Theory 11 + Fact 6 = 17 cases — `cascadeOpCount` 산식 회귀 + helper 사전 reject N=8/9/10 임계 검증 + `MutationQuotaSync = 50` literal sync), `ImportPlanApplyApiCallTests.fs` (4 facts — `queueAddCall` ApiCall cascade ApiDefId/OriginFlowId binding + helper + 후속 add_call 통합 시퀀스로 §9.2 silent miscompile 실 검출 + 룰 C 런타임 차단). 신규 44 tests / 합계 191 PASS (147→191). (3) **fsproj 갱신** — `Ds2.LlmAgent.Tests.fsproj` 에 `<Compile Include>` 6 추가 + `<Content Include="Fixtures\WithCyl.json"><CopyToOutputDirectory>PreserveNewest</CopyToOutputDirectory></Content>`. (4) **자가 검열** (sub-agent general-purpose 위임) — Critical 0건 / Major 0건 / Minor 5건. Minor: ① `newStoreWithProject ()` 가 HelperCascade/Quota 2 파일 중복 (다음 helper test 추가 시 `module TestHelpers` 통합 권고) / ② DeviceTypeDriftTests regex fragile (단 댓글에 ProjectReference 부담 회피 trade-off 의식적 명시) / ③ `Path.Combine(__SOURCE_DIRECTORY__, "..", "..", "..")` path traversal 패턴 (PromakerToolNamesDriftTests 와 동일 — 향후 `findRepoRoot` 헬퍼 통합 권고) / ④ HelperGuiParityTests 의 `helperShape.PassiveSysCount = 1` hardcode (helper 측만 — 미래 N>1 false-negative 방어로 plan walk 권고) / ⑤ D9 "이미 존재" 메시지 의존 (번역 시 fragile, 단 회복 단서 contract 으로 수용). 모두 즉시 수정 필수도 아니라 본 commit 차단 사유 없음. (5) **§5.6 영향 체크리스트 PASS** — 본 commit 후 §5 영향 파일 체크리스트 *모두 완료*. 잔여 후속은 todo 와 무관한 phase 외 영역 (M5/M6 single helper validateRefName / rollback docstring / rev 10 op 수 실측 / C5 Active/Passive 단일 vs 분리 결정 / IOTag·CallCondition·SubmodelProperty 등 entity 확장 후속 todo). (6) **본 todo 종결 시점** — extend-mcp phase 전 4 stage (L1/L2/L3 + Tests) 완료. 추가 phase 진입 시점에 별도 todo 신설. done-extend-mcp.md 가 historical record 보존 (todo §7.5 분리 권장 채택).
> - 다음 리비전 trigger: M5 single helper validateRefName 보강 / M6 MutationCallCount rollback docstring / rev 10 §1 op 수 감소 실측치 측정 / **C5 Active/Passive 단일 vs 분리 비교 결정** / 후속 phase entity (IOTag / CallCondition / SubmodelProperty) 진입 시.
...
### 0.2 `HelperGuiParityTests` fixture 의 repo 내 이전 — ✅ rev 15 옵션 B 채택 + 적용 완료

- **현재 SSOT** (rev 15): `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` (commit `983426b` 시점에 git rename detection 으로 history 보존). `HelperGuiParityTests.fs` 의 fixture 경로 = `Path.Combine(AppContext.BaseDirectory, "Fixtures", "WithCyl.json")`. fsproj 의 `<Content Include="Fixtures\WithCyl.json"><CopyToOutputDirectory>PreserveNewest</CopyToOutputDirectory></Content>` directive 가 빌드 출력에 copy.
- **history (rev 4~14)**: 본 fixture 는 작성자 머신 `F:\tmp\WithCyl.json` 에서 시작 → rev 4 시점에 `Solutions/Core/Ds2.LlmAgent/doc/WithCyl.json` 로 untracked 보관 → rev 15 옵션 B 채택으로 `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` 로 git mv (`git log --follow` 로 추적 가능). 옵션 A (doc 폴더 그대로 track) 는 rev 15 시점에 사용자 결정으로 비채택.
- **회복 시 SSOT**: 위 새 path. 누락 시 §10.4 회복 가이드 참조.
...
### 2.3 `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` 분석 — Promaker GUI 의 cylinder default cascade
...
| **9** | 🔒 | **helper 의 동명 PassiveSystem 충돌 정책 (rev 12 신설 — 사용자 confirm)**: `buildPassiveDeviceCascade` 진입 시 `Queries.passiveSystemsOf projectId store` 에서 동명 매칭이 *나타나면* `invalidOp` (LlmAgent 측 BATCH_ERROR 로 변환). <br/>**의도**: helper 는 *신규 device 생성* 책임만 짐. 기존 device 재사용 시나리오는 LLM 이 사전에 `find_by_name`/`list_systems` 로 조회 후 primitive `add_call` 로 기존 ApiDef.Id 참조 — `2.modeling.md` self-check rev 10 wording (사용자 confirm) 의 *런타임 강제*. <br/>**범위 제한**: ensureSystem 자체는 변경하지 않음 → Mermaid/CSV import 의 idempotent 재실행 (같은 device 가 여러 행에 등장 시 기존 인스턴스 재사용) 호환 보존. strict 검사는 `buildPassiveDeviceCascade` (helper 진입점) 안에서만 작동. <br/>**cross-project 한정 (rev 12 보강)**: 검사 query = `Queries.passiveSystemsOf projectId store` — *현재 project 한정*. 다른 project 에 동명 PassiveSystem 이 존재해도 통과 (project 격리). 같은 project 안 동명 충돌만 reject — 사용자 confirm 시 cross-project 검사 확장 여지. <br/>**reviewer #2 (rev 11 review) 의 silent ignore 우려 차단** — existing 분기에서 SystemType 덮어쓰지 않는 동작이 LLM helper context 에서는 silent miscompile 위험이었음. 본 결정으로 helper context 만 strict, Mermaid/CSV 무영향. |
...
- **Flow 명명 의도 (rev 12 보강)**: Passive 내부 Flow 의 이름은 helper 가 자동 결정 (`{name}_Flow`) — caller (LLM) 에 인자로 노출하지 않음. 이유 = Passive 내부 Flow 는 도메인 룰상 *device cascade 의 implementation detail* (같은 device 안 Work 들의 grouping). LLM 이 직접 명명할 필요 없고, GUI canonical (WithCyl.json §2.3) 의 `{devAlias}_Flow` 패턴 (devAlias=name) 과 동등 결과. caller 가 Flow 이름을 결정해야 할 시나리오 미발견 → forward-compat 인자 노출 보류.
...
| `HelperGuiParityTests.fs` (**신규 2**) | fixture = `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` (§0.2 옵션 B 채택, rev 15). 비교 범위 = *naming 외 cascade 구조 (Work/ApiDef/Arrow 갯수·관계) parity*. naming 자체는 helper `name` ≠ GUI `{flow}_{devAlias}` 라 비교 제외 (M11). GUI fixture 가 ApiCall 까지 가지고 있더라도 비교 시 ApiCall 부분은 제외 (rev 4 helper 책임 일관화). |
...
- 핵심 sample: `Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` — Promaker GUI 의 cylinder default cascade 결과 (rev 15 옵션 B 채택. 옛 path `F:\tmp\WithCyl.json` / `doc/WithCyl.json` 은 git history 로 추적 가능)
...
`Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` (rev 15 옵션 B 채택 SSOT) 누락 시:
- `git log --follow Solutions/Tests/Ds2.LlmAgent.Tests/Fixtures/WithCyl.json` 으로 history 추적 → `git checkout <commit> -- <path>` 복원. rev 15 (`983426b`) 이전 history 는 옛 path `Solutions/Core/Ds2.LlmAgent/doc/WithCyl.json` 형태로 보존.
- git history 에서 복원 불가 시 Promaker GUI 에서 `add_cylinder` 동작 (PassiveSystem cascade 자동 생성) 후 export 로 재생성. 재생성 시 `HelperGuiParityTests` 의 expected 도 동시 갱신.
