...
| **Pass 3** (c) F# / C# 구현 | ⬜ 미진입 | — | 다음 작업 |
| **Pass 4** Negative test + SystemPrompt 풀세트 | ⬜ 미진입 | — | |
...
5. **Pass 3 — (c) Task C-1 + C-2 구현** ⬜ 미진입 (다음 작업): F# `VarCache (ConcurrentDictionary)` + `resolveGuidOrVar` + `registerVar` + `sanitizeVarName` + `@`/`$` prefix reject + `LlmTurnContext.SignalCascadeFailure (volatile + Plan.Clear())`. 모든 mutation tool 의 `assignVar` 인자 (add_* 한정). **SemaphoreSlim gate 는 미도입** (Pass 2 결정).
