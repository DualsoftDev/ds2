...
다른 Claude Code 세션이 본 todo 를 이어받을 때:

1. 본 절 + "## 진행 상태 (현재 위치)" + "## 작업 순서 권장" 의 ✅ / 미진입 표시 확인
2. 가장 최근 commit log 확인:
   ```
   git log --oneline -6
   # 기대: e7e8025 Pass 2 spike 까지 완료된 상태 (또는 이후)
   ```
3. 미진입 단계 확인:
   - Pass 3 미진입 = (c) F# / C# 구현 시작 — 본 todo 의 "## (c) Variable binding 안" + "구현 task 분해" 절 참조
   - Pass 3 진입 중 = 변경 파일 (`ToolOperations.fs`, `ImportPlanBuilder.fs`, `LlmTurnContext.cs`, `ModelTools.cs`) 의 git status 확인
4. 측정 인프라 (Pass 1.5/2 자동화) 의 영구성 / 정리 시점 = "## 측정 인프라 cleanup 항목" 절

---

## 진행 상태 (현재 위치)

| Pass | 상태 | commit | 핵심 결과 |
|---|---|---|---|
| **Pass 0** PoC | ✅ 완료 | (이전 phase) | LLM ≥99% + 자발 묶음 확인 |
| **Pass 1** (a) SystemPrompt batching 절 | ✅ 완료 | `5e80af3` | 가이드 추가, 효과는 Pass 1.5 에서 검증 |
| **Pass 1.5** 측정 자동화 + 정량 | ✅ 완료 (한계 인정) | `08d9fed` | (a) 단독 효과 미미 / inconclusive — chat hooks/CLAUDE.md noise + n=3 분산 큼 |
...
| 결정 | 결과 |
|---|---|
| (a) batching 가이드 | 적용 완료, 효과 미미하나 부작용 없어 유지 |
| (c) SemaphoreSlim race gate | **제거** (Pass 2 결과로 SDK 자체 직렬화 확인) |
| (c) VarCache | `ConcurrentDictionary` 유지 (방어적) |
| (c) Cascade flag | `volatile bool` + `Plan.Clear()` 유지 |
| (b) batch tool | Appendix A 격리 (변동 없음) |
| AskUserQuestion 도구 | SystemPrompt rule 7 로 chat 자연어 redirect (양 commit 모두) |
...
3. ~~**Pass 1.5 — (a) 단독 효과 측정 gate**~~ ✅ **완료 (2026-05-07)**. 결과는 "## Pass 1.5 결과" 절 참조. (a) 단독 효과 미미 / inconclusive — chat hooks/CLAUDE.md noise + n=3 분산 큼. 가이드 유지 + Pass 2 진입.
4. ~~**Pass 2 — McpHostService 동시성 spike**~~ ✅ **완료 (2026-05-07)**. 결과는 "## Pass 2 결과" 절 참조. **SDK 자체 직렬 dispatch 확인** (Δms 311~395) → SemaphoreSlim gate 제거. (c) race 처리 부담 없음.
5. ~~**Pass 3 — (c) Task C-1 + C-2 구현**~~ ✅ **완료 (2026-05-07)**. 결과는 "## Pass 3 결과" 절 참조. F# 측 SSOT (`ImportPlanBuilder.VarCache`/`CascadeFailureFlag`/`SignalCascadeFailure` + `ToolOperations.sanitizeVarName`/`resolveGuidOrVar`/`registerVar` + `sanitizeName` 의 `@`/`$` prefix reject), C# 측 add_* 6종 `assignVar` + dispatcher work 안 sanitize/resolve 통일 + RunMutation cascade 단락. SemaphoreSlim gate 는 Pass 2 결정대로 미도입.
6. **Pass 4 — Task C-4 + Task C-5**:
   - ~~Task C-4: SystemPrompt 풀세트~~ ✅ **완료 (2026-05-07)**. `Apps/Promaker/Promaker/LlmAgent/Prompts/1.SystemPrompt.md` 의 "Mutation batching" 절을 chain pattern 까지 풀세트로 확장 (실린더 8 op 압축 예시 + assignVar rule + `@`/`$` reject + BATCH_ABORTED). mutation tools 시그니처 절에도 assignVar 1줄 추가. 빌드 통과
   - prompt cache breakpoint 영향 확인 (M-12) — `cache_creation_input_tokens` / `cache_read_input_tokens` metadata 비교 — Pass 5 측정 시 함께
   - ~~Task C-5 — Negative test~~ ✅ **완료 (2026-05-07)**. `Solutions/Tests/Ds2.LlmAgent.Tests/VarBindingTests.fs` 신규 (31 케이스: sanitizeVarName 11 / resolveGuidOrVar 9 / registerVar 6 / cascade flag 5) + `SanitizeNameTests.fs` 의 `@`/`$` prefix reject 6 케이스. **129 테스트 모두 통과**. 미커버: C# RunMutation 의 BATCH_ABORTED 단락 흐름 (LlmTurnContext / IUiDispatcher 의존성 — Promaker 통합 테스트 영역, Pass 5 의 실측 시나리오로 확인)
...
## Pass 3 결과 (2026-05-07) — (c) Variable binding 구현

### 산출물 (file:line)

| 파일 | 변경 |
|---|---|
| `Solutions/Core/Ds2.LlmAgent/ImportPlanBuilder.fs` | `VarCache: ConcurrentDictionary<string,Guid>` 멤버 / `[<VolatileField>] cascadeFailed` + `CascadeFailureFlag` / `SignalCascadeFailure()` (set + Plan.Clear) 추가. 기존 멤버 무영향 |
| `Solutions/Core/Ds2.LlmAgent/ToolOperations.fs` | `sanitizeName` 본문에 `@`/`$` prefix reject 1줄 추가. 신규 함수 — `VarNameMaxLength=32` / `VarCacheMaxCount=50` / `sanitizeVarName` (echo 회피 codepoint 메시지) / `resolveGuidOrVar` (`$xxx` lookup or GUID parse, 실패 = invalidOp) / `registerVar` (assignVar = null/empty no-op, 중복/cap 초과 = invalidOp) |
| `Apps/Promaker/Promaker/LlmAgent/Tools/ModelTools.cs` | 헬퍼 재구성 — `SanitizeOrThrow` / `ResolveGuidOrThrow` / `ParseGuidOrThrow` / `VarSuffix` / `EnsureErrorPrefix`. `RunMutation` 변경 = 진입 시 `CascadeFailureFlag` 검사 → `BATCH_ABORTED` 단락, `IncrementMutationCount` 를 try 안으로, catch 에서 `SignalCascadeFailure()` + prefix 중복 회피. add_* 6종 (system/flow/work/call/api_def/arrow) 에 `assignVar: string? = null` 인자 + sanitize/parse 검사를 dispatcher work delegate 안으로 이동 |

### Pass 2 결정 반영

- **SemaphoreSlim gate 미도입** (Pass 2 SDK 직렬 dispatch 확인). `LlmTurnContext` 에 SemaphoreSlim 추가 X
- **`ConcurrentDictionary<string,Guid>` 유지** — F# `ImportPlanBuilder.VarCache` 멤버. dispatcher 안 R/W 라 race 없으나 향후 SDK upgrade / 다른 transport 변경 cheap insurance
- **`[<VolatileField>] cascadeFailed`** — `RunMutation` 진입 thread 와 dispatcher work thread 가 다를 수 있어 read 측 안전성

### Critical-2 fail-fast 통일 적용

- mutation tool 의 throw 는 모두 `RunMutation` catch 가 흡수
- catch 시점: `SignalCascadeFailure()` 호출 → `cascadeFailed = true` + `Plan.Clear()` 동반 → 같은 turn 의 후속 mutation tool 진입 시점에 `BATCH_ABORTED` 단락
- 빈 plan 의 turn end 처리: `IsEmpty` 검사로 `ApplyImportPlan` 호출 자체 skip 가능 (호출자 측 책임 — `LlmChatViewModel` 의 turn end 흐름은 Pass 5 까지 유지)

### Trigger 카테고리 (M-15 prefix 표 적용)

| 메시지 prefix | 발생 시점 | 예시 |
|---|---|---|
| `VALIDATION_ERROR` | dispatcher work 안 invalidOp / sanitize fail / `$xxx` lookup miss / 중복 assignVar | `VALIDATION_ERROR: name 가 '@' 또는 '$' 로 시작할 수 없습니다 (예약 prefix).` |
| `BATCH_ABORTED` | `cascadeFailed=true` 후 후속 mutation 진입 시점 단락 | `BATCH_ABORTED: 같은 turn 의 이전 mutation 이 실패하여 후속 호출이 중단되었습니다 (add_call skipped).` |
| `QUOTA_EXCEEDED` | `ctx.IncrementMutationCount` (turn 당 50회 초과) | `QUOTA_EXCEEDED: turn 당 mutation tool 호출이 50 회를 초과했습니다.` |
| `INTERNAL_ERROR` | RunRead 의 dispatcher exception (RunMutation 은 VALIDATION_ERROR 로 흡수) | RunRead catch path 만 |

### 검증

- F# DLL 단독 빌드 통과 (`dotnet build Solutions/Core/Ds2.LlmAgent`)
- Promaker.sln 전체 빌드 통과 (warning 2 = OllamaSharp 분석기 버전 mismatch, 기존부터 있던 무관)
- `Ds2.LlmAgent.Tests` 92 테스트 모두 통과 (sanitizeName 의 `@`/`$` reject 추가가 기존 테스트 케이스 깨뜨리지 않음)

### 남은 일 (Pass 4 진입 전 점검 필요)

- Negative test (Pass 4 Task C-5):
  - `$xxx` 미정의 → `VALIDATION_ERROR`
  - 잘못된 변수명 형식 (예: `$1abc`, `$ab-cd`) → `VALIDATION_ERROR`
  - 중복 `assignVar` → `VALIDATION_ERROR`
  - 중간 op 실패 → 후속 op `BATCH_ABORTED`
  - turn end 에 부분 plan 이 1 undo step 으로 적용 vs 빈 plan skip
  - `name="@x"` / `name="$y"` reject
- SystemPrompt 풀세트 (Pass 4 Task C-4): chain pattern + assignVar 가이드를 (a) 의 단독 안전 케이스와 결합

### Pass 3 산출물

(별도 측정 자동화 산출물 없음 — 구현만)

---

## 측정 인프라 cleanup 항목 (Pass 5 종료 후 처리)

본 todo 의 Pass 1.5 / Pass 2 자동화 위해 production 코드에 측정 전용 hook + trace 가 영구 commit 됨. **opt-in flag (`--autostart-llm` / `--measure-prompt` / `--measure-then-exit`) 가 모두 gate 라 default false → production 동작 영향 없음**. 단 코드 cleanliness 위해 Pass 5 final 측정 종료 후 결정 필요.

### Cleanup 분류 표

| 항목 | 위치 | 분류 | Pass 5 후 처리 후보 |
|---|---|---|---|
| `ModelTools.RunMutation` enter trace 로그 | `Apps/Promaker/Promaker/LlmAgent/Tools/ModelTools.cs:50-65` (`_toolCallSeq` + `enter seq= t= nanos=` 라인) | **측정 전용 noise** — 매 mcp call 마다 Debug 라인 1줄 추가 | **제거 권장** (Pass 2 spike 산출 완료, 더 이상 불필요). 또는 `if (Log.IsDebugEnabled)` gate |
| `ModelTools.RunRead` enter trace 로그 | 위와 동일 파일 RunRead 메서드 | 위와 동일 | 위와 동일 |
| `_toolCallSeq` static field | `ModelTools.cs` | trace 로그 의존 | trace 로그 제거 시 같이 |
| `App.StartupAutoOpenLlm` flag + 인자 처리 | `App.xaml.cs` | 측정 도구 | 회귀 측정용 보존 또는 별 namespace 분리 |
| `App.StartupMeasurePrompt` / `StartupMeasureThenExit` | 위와 동일 | 측정 도구 | 위와 동일 |
| `MainViewModel.ScheduleMeasurePrompt` private 메서드 | `Apps/Promaker/Promaker/ViewModels/Shell/MainViewModel.cs` ctor 끝 + private 메서드 | 측정 도구 (autostart flag gate) | 회귀 측정용 보존 |
| `MainWindow.Window_Closing` 의 dirty check skip | `Apps/Promaker/Promaker/MainWindow.xaml.cs:81-87` (autostart 모드 분기) | risk 항목 — autostart flag 가 활성일 때만 skip | 보존 OK (gate 안전) |
| `doc/run-pass15.fsx` / `run-pass15.ps1` / `run-pass2-spike.fsx` | doc 내부 | 측정 helper | 회귀 측정용 보존 |
| `doc/poc-roundtrip-analysis.ps1` | doc 내부 | 측정 분석 | 보존 |
| `doc/pass15-results-*.md` | doc 내부 (자동 생성물) | 측정 raw 결과 | gitignore 추가 또는 정리 후 done 으로 통합 이전 |

### Cleanup 결정 — Pass 5 종료 직후 일괄 진행

Pass 5 의 final 측정이 끝나고 done 문서 이전 시:

1. **즉시 제거** — `ModelTools` 의 `enter` trace 로그 + `_toolCallSeq` (Pass 2 산출 종료 = 더 이상 필요 없음)
2. **보존 (회귀 측정용)** — measure-* 인자 / `ScheduleMeasurePrompt` / fsx 스크립트 (향후 모델 / SDK 변경 시 재측정 가능)
3. **gitignore** — `doc/pass15-results-*.md` 자동 생성 raw 결과 (의미 있는 결과만 done 문서에 통합 후 raw 삭제)
4. **분리 namespace 검토** — `App.cs` / `MainViewModel.cs` / `MainWindow.xaml.cs` 의 측정 분기를 별 partial class 또는 `MeasurementHooks.cs` 로 분리하면 production 코드와의 visual 분리

### 결정 미정 — Pass 5 종료 시 사용자 확정

옵션 A (보존) vs 옵션 B (제거):
- A — production 코드에 measurement opt-in 영구. 향후 회귀 측정 / 새 모델 비교 즉시 가능. 단 코드량 증가
- B — 측정 끝나면 모두 revert. production clean. 단 향후 회귀 측정 시 재구축 필요
