...
| **Pass 3** (c) F# / C# 구현 | ✅ 완료 | (HEAD) | F# `VarCache`/`SignalCascadeFailure` + `sanitizeVarName`/`resolveGuidOrVar`/`registerVar` + `@`/`$` reject. C# add_* 5종 `assignVar` + cascade 단락. 솔루션 빌드 통과. 기존 92 테스트 통과 |
| **Pass 4** Negative test + SystemPrompt 풀세트 | ⬜ 미진입 | — | 다음 작업 |
...
5. ~~**Pass 3 — (c) Task C-1 + C-2 구현**~~ ✅ **완료 (2026-05-07)**. 결과는 "## Pass 3 결과" 절 참조. F# 측 SSOT (`ImportPlanBuilder.VarCache`/`CascadeFailureFlag`/`SignalCascadeFailure` + `ToolOperations.sanitizeVarName`/`resolveGuidOrVar`/`registerVar` + `sanitizeName` 의 `@`/`$` prefix reject), C# 측 add_* 5종 `assignVar` + dispatcher work 안 sanitize/resolve 통일 + RunMutation cascade 단락. SemaphoreSlim gate 는 Pass 2 결정대로 미도입.
