...
- 현 단계: **Phase 1a 완료** (2026-05-06). 결정 9개 모두 확정. Phase 1b-c (HTTP MCP transport 채널) 진입 가능
- 결정 상태:
...
3. ✅ **Phase 1a** — scaffold + ClaudeCliProvider concrete + 최소 chat panel (PR 1, internal) — 2026-05-06 완료. 발견 사항 + 작업 산출물은 `done-promaker-llm-agent.md` 참조
4. **Phase 1b-c** (HTTP) — Promaker in-process Kestrel + `ModelContextProtocol.AspNetCore` 1.2.0 + loopback bind + ephemeral port + handshake nonce + `IUiDispatcher` 추상 + Tool registry 공통 invoker (PR 2)
