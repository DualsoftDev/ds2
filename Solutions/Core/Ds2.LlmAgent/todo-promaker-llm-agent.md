...
#### Phase 1a — Scaffold + Claude CLI echo (PR 1, internal)
- [ ] `./Ds2.LlmAgent.fsproj` 생성 (이 문서와 같은 디렉토리; `../Ds2.Core/` / `../Ds2.Editor/` 와 동일 SDK / 패키지 props 상속). **ProjectReference: `Ds2.Editor` only** (review 2차 R3 M5 — `Ds2.Core` 는 transitive)
- [ ] `../../Apps/Promaker/Promaker.sln` 에 명시적 추가 (review 1차 M2)
- [ ] `../../Apps/Promaker/Promaker/Promaker.csproj` ProjectReference 추가 (csproj 기준 상대경로 `..\..\..\Solutions\Core\Ds2.LlmAgent\Ds2.LlmAgent.fsproj`)
- [ ] **`App.xaml` 의 `StartupUri` 제거** (review 1차 C2) + 진입점 분리:
  - 1차 안: `App.OnStartup` argv 분기 (MCP 모드면 WPF 진입 skip)
  - 2차 안 (review 2차 R5 M5): `<DisableWinExeOutputInference>` + 별도 `static int Main` 분리 — designer / Hot Reload / Theme / TimerPeriod hook 의 MCP 모드 영향 회피
  - 사전 실증 0 sub-task 결과로 1차 vs 2차 결정
- [ ] `ClaudeCliProvider` 구현 (**concrete 1종 only, ILlmProvider 인터페이스 미도입** — review 2차 R3 M1 / R1 M3):
  - phase 1 = ClaudeCli concrete struct/record + 공통 `LlmEvent` union 만. `ILlmProvider` 인터페이스 / `Capabilities` flag 는 phase 2 Codex 실증 후 패턴 추출. 사용자 철학 "신규 추가 10점" 부합
  - 기동: 사전 실증 2 결과에 따라 인자 조합 결정. 1차 후보 = `claude -p <msg> --output-format stream-json --verbose --resume <session-id>`. 양립 불가 시 `--input-format stream-json` stdin 연속 모드 (사전 실증 2a fallback)
  - **session_id 캡처 FSM** (review 1차 M7 + 2차 R2 Critical): 첫 stream 의 **`{"type":"system","subtype":"init","session_id":"..."}`** 도착 후에만 다음 Send 활성화. parser 는 `subtype` 으로 분기 (Builder PoC `parse_stream_line` 의 `system` 타입 무시 패턴 재사용 X). SessionEnd 이벤트 로 종료 표시.
  - 이후 턴: `--resume <session-id>`
  - Windows: Job Object attach + `CREATE_NO_WINDOW` + hidden STARTUPINFO
  - **Claude CLI 버전 핀 enforce** (review 2차 R5 M2): provider 기동 시 `claude --version` 으로 SemVer 체크 → 미달 시 `ProviderError` fail-fast. 정확한 minimum version 은 사전 실증 단계에서 결정.
  - stream-json 줄 단위 파싱 → 공통 `LlmEvent` 정규화
  - **stream backpressure** (review 2차 R4 M3): `Channel.CreateBounded<LlmEvent>` (capacity = e.g. 256) + AssistantDelta merge throttle (50ms aggregation). ListBox `VirtualizingStackPanel` ChatPanel 측에서.
- [ ] **Audit log 인프라** (review 2차 R5 M4 — log4net 글로벌 원칙): `Promaker.LlmAgent.ToolCall` logger + `Promaker.LlmAgent.RawStream` (verbose, default OFF) appender 등록
- [ ] 최소 chat panel: 메시지 보내고 AssistantDelta 표시만 (mutation tool 없음)

#### Phase 1b-c — HTTP MCP transport 채널 (PR 2, 결정 4 (c) 채택 시)
> 결정 4 가 사전 실증 3 결과로 (c) 확정 시 진행. (b) 채택 시 1b-b 로 분기.

- [ ] Promaker in-process Kestrel + `ModelContextProtocol` C# SDK HTTP transport 어댑터
- [ ] **loopback-only bind** (`127.0.0.1`) + OS-assigned ephemeral port
- [ ] `.mcp-config` JSON 에 transport=http + url + handshake nonce 기록 (ACL 명세는 결정 5 동일 적용)
- [ ] **`IUiDispatcher` 추상 주입** (`InvokeAsync<'T>` Background priority — 결정 8) + Tool handler 진입 marshalling
- [ ] **Tool registry 공통 invoker** (review 2차 R3 M4 + R5 통합 — 아래 "Tool registry 공통 invoker 사양" 참조)
...
- 현 단계: **설계 논의(--plan) + cross-validated 3 reviewers `--review` 1차 + 5 reviewers `--review` 2차 반영 완료**, 구현 미착수
- 결정 상태:
  - 확정 8개: 결정 1 (통합 형태) / 결정 2 (언어 + tool registry, ILlmProvider 인터페이스 phase 2 로 미룸) / 결정 3 (Provider 우선순위) / 결정 5 (인스턴스 격리 + IPC 보안, (c) HTTP 시 5.0 / (b) stdio 시 5.1~5.6) / 결정 7 ((d) ImportPlan 활용 — 2차 R1 로 (c) 잠정에서 격상 확정) / 결정 8 (Thread 모델, InvokeAsync Background priority) / 결정 9 (비동기 표현 — provider stream `IAsyncEnumerable`, EditorEvent `IObservable`) / 결정 6 흡수 완료
  - **잠정 1개**: 결정 4 (Tool 채널 — 사전 실증 3 후 (c) 또는 (b) 확정)
- **다음 작업 진입 시 권장 순서**:
  1. **Phase 0 사전 실증 동시 진행** (~수 시간): 실증 3 (HTTP MCP, 우선순위 1번) + 0 (WinExe stdio + 진입점 분리) + 1 (MCP SDK F# 호출) + 2 (CLI 인자 양립성 + spawn 시점 + latency, 4 sub-task) + 4 (Codex spike) + 5 (Editor surface inventory) + 7 (session_id subtype:init) + (결정 4 (b) 시) 6 (손자 Job)
  2. 결정 4 잠정 → 확정 전환 (HTTP 가능 시 (c), 아니면 (b)). Phase 1b → 1b-c 또는 1b-b 분기 결정
  3. **Phase 1a** scaffold + ClaudeCliProvider concrete + 최소 chat panel (PR 1, internal)
  4. **Phase 1b-c (HTTP) 또는 Phase 1b-b (stdio)** sibling 중 하나 — MCP 채널 + Tool registry 공통 invoker + IUiDispatcher (PR 2)
  5. **Phase 1c** 최소 system prompt + `add_system` end-to-end + ImportPlan turn end apply (PR 3)
  6. **Phase 1d** Tool 풀세트 + UI 완성 + system prompt 보강 + golden scenario (token 회귀 / prompt injection / RDP 격리 포함) (PR 4)
