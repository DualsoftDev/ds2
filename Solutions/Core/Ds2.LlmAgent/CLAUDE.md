...
| 첨부 분류 (정책 19) | `Solutions/Core/Ds2.LlmAgent/AttachmentClassifier.fs:38-86` | `textExtensions` (코드/문서 화이트리스트, line 38) / `rejectedExtensions` (실행파일/미디어/압축 + `.env` 비밀정보, line 61) / `extensionlessTextNames` (Dockerfile 등, line 80) 3 set SSOT — UI Drop/Paste handler 는 `classify(path)` 한 함수만 호출. `detectEncoding` (line 115) 은 `Ds2.LightHouse.TextEncoding` shim (todo §4.2b) |
