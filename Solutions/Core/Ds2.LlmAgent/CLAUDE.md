...
AttachmentClassifier.fs   첨부 분류 SSOT (정책 19) — `Classification` DU (AcceptImage/Text/Pdf/RejectExtension/Unknown) + `textExtensions` / `rejectedExtensions` (`.env` 비밀정보 차단 포함) / `extensionlessTextNames` set. `detectEncoding` 은 `Ds2.LightHouse.TextEncoding.detectEncoding` shim (todo §4.2b 이전 — BOM → strict UTF-8 → CP949 → UTF-8 replacement 본체는 LightHouse SSOT)
...
| 첨부 분류 (정책 19) | `Solutions/Core/Ds2.LlmAgent/AttachmentClassifier.fs` | `textExtensions` / `rejectedExtensions` (`.env` 비밀정보 차단) / `extensionlessTextNames` (Dockerfile 등) 3 set SSOT — UI Drop/Paste handler 는 `AttachmentClassifier.classify(path)` 한 함수만 호출. `AttachmentClassifier.detectEncoding` 은 `Ds2.LightHouse.TextEncoding.detectEncoding` 의 영구 호환 shim (todo §4.2b — return type `TextEncoding.TextEncodingDetect`). line 박제는 review m3 권고에 따라 함수명 anchor 로 약화 |
