...
### Promaker 통합 (Phase 1d 완료 상태)

```
Apps/Promaker/Promaker/
├── Controls/Llm/LlmChatPanel.xaml(.cs)         dock UserControl (1d-4 D — 기존 Windows/LlmChatWindow 폐기 후 이전)
├── ViewModels/LlmChatViewModel.cs              [ObservableProperty]/[RelayCommand] + AssistantDelta throttle + LlmTurnLabelPrefix 상수
...
└── LlmAgent/
    ├── McpHostService.cs                       in-process Kestrel + ModelContextProtocol.AspNetCore 1.2.0
...
| 결정 8 | `Apps/Promaker/Promaker/ViewModels/Shell/MainViewModel.cs:683-` | 기존 `RequestRebuildAll` 이 `BeginInvoke` + `DispatcherPriority.Background` 패턴 — sync `Invoke` 는 coalescing 깨뜨림 |
