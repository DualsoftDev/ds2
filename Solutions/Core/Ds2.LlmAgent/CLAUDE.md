...
Logging.fs                log4net `Ds2.LlmAgent.Provider` / `Promaker.LlmAgent.RawStream` (verbose, default OFF). 텍스트 인코딩 추정 warn 은 todo §4.2b 이후 `Ds2.LightHouse.TextEncoding` logger 로 분기 — `<logger name="Ds2.LlmAgent.Provider">` 사용자 config 시 인지 (review m1)
...
LlmMessage.fs             첨부 도메인 SSOT — `Attachment` DU (Image/Pdf/TextFile) + `Attachment.mimeOf` / `Attachment.extOf` (rev 18 m3 일원화 — ImageFormat 자체는 `Ds2.LightHouse` 로 이전, todo §4.2a) + `Capabilities` record + `CapabilityPresets` (AnthropicWire/OpenAiApiWire/CodexCliWire SSOT) + `AttachmentRendering` (toInlineString fence escalate / summarize / formatBytes) + `LlmUserMessageOps.EnforceCapabilityOrFail`
AttachmentClassifier.fs   첨부 분류 SSOT (정책 19) — `Classification` DU (AcceptImage/Text/Pdf/Xlsx/RejectExtension/Unknown) + `textExtensions` / `rejectedExtensions` (`.env` 비밀정보 차단 포함) / `extensionlessTextNames` set. `AcceptXlsx` 는 Promaker UI 가 Excel COM 으로 PDF 변환 후 기존 PDF 첨부 경로에 태움 (DEBUG build 는 변환 PDF 유지, PDF 미지원 provider 는 텍스트 fallback). `detectEncoding` 은 `Ds2.LightHouse.TextEncoding.detectEncoding` shim (todo §4.2b 이전 — BOM → strict UTF-8 → CP949 → UTF-8 replacement 본체는 LightHouse SSOT)
...
ToolOperations.fs         queueAdd* helper (Project/ActiveSystem/PassiveSystem/Flow/Work/Call(+AllowDup)/ApiDef/Arrow/Cylinder/Clamp/Robot/Device + queueRemoveEntity/queueRenameEntity) + read 6종 (listProjects/listSystems/describeSystem/describeSubtree/findByName/validateModelByGuid) + format helper 3종 + cascadeOpCount + sanitizeName (NameMaxLength=128, Cc/Cf + @/$ prefix 차단). Phase 5 cleanup 후 doc-level dispatcher (`ModelProtocol.applyPatch` / `applyDoc`) 만 호출 — op-layer [McpServerTool] surface 일소
```
...
└── LlmAgent/
    ...
    ├── PromakerToolNames.cs                    6개 fully-qualified mcp__promaker__* (allowlist SSOT) — read 2종 (find_by_name/validate_model) + doc-level 4종 (apply_model_doc/validate_model_doc/export_model_doc/json_to_yaml). Phase 5 cleanup 으로 op-layer 15종 (apply_operations + add_* + remove_entity + rename_entity) 일소 + Phase 6 cleanup 으로 read list/describe 4종 (list_projects/list_systems/describe_system/describe_subtree) 일소 — 모두 `export_model_doc(path?, depth?)` 으로 흡수, GUID 입출력 폐지
    ...
    └── Tools/ModelTools.cs                     [McpServerToolType] + Sanitize → ToolOperations.sanitizeName 위임 + RunMutation/RunRead 헬퍼 + 10 tool method = 4 doc-level (ApplyModelDoc / ValidateModelDoc / ExportModelDoc / JsonToYaml) + 6 read (ListProjects/ListSystems/DescribeSystem/DescribeSubtree/FindByName/ValidateModel). Phase 5 cleanup 으로 op-layer 15종 일소
```
...
| 1d-4 tool 화이트리스트 | `Apps/Promaker/Promaker/LlmAgent/PromakerToolNames.cs:16-39` | 10개 fully-qualified `mcp__promaker__*` 이름 (read 6 + doc-level 4 — Phase 5 cleanup 후). drift 시 LLM 측 차단 → `PromakerToolNamesDriftTests` 가 회귀 검출 |
...
| 첨부 분류 (정책 19) | `Solutions/Core/Ds2.LlmAgent/AttachmentClassifier.fs` | `textExtensions` / `rejectedExtensions` (`.env` 비밀정보 차단) / `extensionlessTextNames` (Dockerfile 등) 3 set SSOT — UI Drop/Paste handler 는 `AttachmentClassifier.classify(path)` 한 함수만 호출. `.xlsx` 는 `AcceptXlsx` 로 분류되고 Promaker UI 가 Excel COM PDF 변환 후 기존 PDF 첨부 처리 재사용. DEBUG build 는 변환 PDF 를 유지하고, PDF 미지원 provider 는 PdfPig 텍스트 fallback 이라 xlsx 이미지/차트는 보존되지 않음. `AttachmentClassifier.detectEncoding` 은 `Ds2.LightHouse.TextEncoding.detectEncoding` 의 영구 호환 shim (todo §4.2b — return type `TextEncoding.TextEncodingDetect`). line 박제는 review m3 권고에 따라 함수명 anchor 로 약화 |
...
| F3 size cap 검증 (rev 20) | `Solutions/Core/Ds2.LlmAgent/LlmMessage.fs:268-296` + `Apps/Promaker/Promaker/ViewModels/LlmChatViewModel.Attachments.cs:191-244` | provider 전환 시 `MaxImageBytes` / `MaxPdfBytes` cap 도 검사 — 10MB OpenAI 이미지 → Claude (5MB) 전환 silent 통과 회귀 차단. F# strict throw + UI 사전 흡수 dual-layer |
| F2 Codex 32K mitigation (rev 20) | `Solutions/Core/Ds2.LlmAgent/CodexCliArgs.fs:135-160` + `CodexCliProvider.fs:131-141` | `measureArgsBytes` (UTF-8 byte 합 + 토큰당 separator 3 over-estimate) + `[<Literal>] CodexArgsByteCap = 24576` (Windows lpCommandLine 32,767 wide-char 한도의 ASCII 75% 마진). 1MB 텍스트 첨부 inline 시 process spawn 깨짐 회피. 정석 Stream stdin 시그니처는 Phase D 보류 |
| F1 consent 첨부 명시 (rev 20) | `Apps/Promaker/Promaker/LlmAgent/LlmConfig.cs:153-164` | 동의 다이얼로그 본문에 첨부 송신 사실 (텍스트 fenced inline / 이미지·PDF multimodal content block) + `.env` 등 비밀정보 차단 명시 — 첨부 phase 정착 후 consent 동기화 누락 회귀 |
