module Ds2.Editor.ArrowPathCalculator
...
module private FaceGeometry =
    ...
    let boundaryPointToward (r: Xywh) (towardX, towardY) =
    ...
    let determineFaces (source: Xywh) (target: Xywh) =
    ...
    let dockPointOnFace (r: Xywh) face (towardX, towardY) perpOffset =
        let x, y, w, h = rectOf r
...
module private RouteProfiles =

    let private classifyRouteCategory (source: Xywh) (target: Xywh) =
        let srcCx, srcCy = FaceGeometry.centerOf source
        let tgtCx, tgtCy = FaceGeometry.centerOf target
        let _, _, srcW, srcH = FaceGeometry.rectOf source
        let _, _, tgtW, tgtH = FaceGeometry.rectOf target
    ...
    let buildRouteProfile (source: Xywh) (target: Xywh) (srcOff: float) (tgtOff: float) =
        let srcCx, srcCy = FaceGeometry.centerOf source
        let tgtCx, tgtCy = FaceGeometry.centerOf target
        let dx = tgtCx - srcCx
        let dy = tgtCy - srcCy
            ...
            if isCloseRange then
                {
                    StartPadding = 0.0
                    EndPadding = 0.0
                    HandleLength = FaceGeometry.clamp 10.0 20.0 (dist * 0.10)
                    BaseShift = 0.0
                    CandidateShifts = [ 0.0 ]
                    CandidateScales = [ 1.0 ]
                }
                ...
                let liftBase = liftDir * FaceGeometry.clamp 24.0 72.0 (dist * 0.10) + laneBias
                {
                    StartPadding = 0.0
                    EndPadding = 0.0
                    HandleLength = FaceGeometry.clamp 32.0 120.0 (dist * 0.34)
                    BaseShift = 0.0
        ...
        | DiagonalBridge ->
            ...
            let liftBase = liftDir * FaceGeometry.clamp 10.0 34.0 (dist * 0.05) + laneBias * 0.85
            {
                StartPadding = 0.0
                EndPadding = 0.0
                HandleLength = FaceGeometry.clamp 26.0 90.0 (dist * 0.28)
                BaseShift = 0.0
                CandidateShifts = [ 0.0; liftBase; -liftBase; liftBase * 1.5; -liftBase * 1.5 ]
                CandidateScales = [ 1.0; 1.2; 1.45 ]
            }

module private BezierRouting =
    ...
    let private pointInInflatedRect (px, py) (r: Xywh) =
        let x, y, w, h = FaceGeometry.rectOf r
        px >= x - obstacleMargin
    ...
    let computeBezierPoints srcFace tgtFace (srcPos: Xywh) (tgtPos: Xywh) srcOff tgtOff (obstacles: Xywh list) =
        let profile = RouteProfiles.buildRouteProfile srcPos tgtPos srcOff tgtOff
        let tgtCenter = FaceGeometry.centerOf tgtPos
        let srcCenter = FaceGeometry.centerOf srcPos
        let rawSx, rawSy = FaceGeometry.dockPointOnFace srcPos srcFace tgtCenter srcOff
        let rawTx, rawTy = FaceGeometry.dockPointOnFace tgtPos tgtFace srcCenter tgtOff
        let srcNx, srcNy = FaceGeometry.faceVector srcFace
        let tgtNx, tgtNy = FaceGeometry.faceVector tgtFace
...
let chooseDockPoints (source: Xywh) (target: Xywh) =
    let tgtCenter = FaceGeometry.centerOf target
    let srcCenter = FaceGeometry.centerOf source
    let srcFace, _ = FaceGeometry.boundaryPointToward source tgtCenter
    let tgtFace, _ = FaceGeometry.boundaryPointToward target srcCenter
    FaceGeometry.dockPointOnFace source srcFace tgtCenter 0.0,
    FaceGeometry.dockPointOnFace target tgtFace srcCenter 0.0

let computePathWithObstacles (source: Xywh) (target: Xywh) (obstacles: Xywh list) : ArrowVisual =
    let srcFace, tgtFace = FaceGeometry.determineFaces source target
    { Points = BezierRouting.computeBezierPoints srcFace tgtFace source target 0.0 0.0 obstacles }
...
let computeFlowArrowPaths (store: DsStore) (flowId: Guid) : Map<Guid, ArrowVisual> =
    ...
    for arrow in arrows do
        ...
        | Some srcPos, Some tgtPos ->
            let srcFace, tgtFace = FaceGeometry.determineFaces srcPos tgtPos
