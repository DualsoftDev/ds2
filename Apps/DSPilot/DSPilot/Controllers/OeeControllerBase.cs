...
public abstract class OeeControllerBase : ControllerBase
{
    ...
    /// </summary>
    protected async Task<PlannedAutoPatternDto> ComputeAndCacheAutoPatternAsync(string? flowName, CancellationToken ct)
    {
        ...
        try
        {
            ...
            foreach (var f in targetFlows)
            {
                ...
                foreach (var r in rows)
                {
                    ...
                    {
                        plannedCtMs += cMs;
                        nonProdIntervals.Add((startMs, rec));
                    ...
                    double repair = r.Mt is long mtL ? (mtL - thr) : (cMs - thr);
