...
{
    // ════════════════════════════════════════════════════════════════════════
    //  판정 규칙 (2026-09-11 두 규칙 모델, doc/28). 사이클 = 동작(MT) + 대기(WT). 판정·손실·표시의 단위는
    ...
    //  고장 규칙을 넘긴 행이라도 길이가 비생산 경계 이상이면 비생산이다(doc/30 §12-⑧ 비생산 우선 — 주말 60시간은
    //  고장이 아니다). 종전 "정지(비가동)" 중간 밴드와 신호 기반 유발자·형제 판별(doc/25)은 폐기.
    //  사용자가 움직이는 것은 고장·비생산 배수 2개뿐이다 — 행 단위 수동 라벨은 없다(doc/30 §11.1).
    // ════════════════════════════════════════════════════════════════════════
    ...
    /// <summary>
    /// 행↔구간 <b>과반</b> 겹침 비율(doc/28 §2.6) — 저장된 유지보수 이벤트 구간을 재도출된 행에 다시 붙일 때의
    /// 내부 조인 허용치. 한 행이 서로 겹치지 않는 두 구간에 동시에 과반일 수 없어 이중 분류가 불가능하다.
    /// </summary>
    ...
    /// 비생산으로 판정돼도 TEEP 는 변하지 않는다(달력 손실 그대로) — 화면 ⓘ 한 줄(doc/28 §2.7).
    /// </summary>
