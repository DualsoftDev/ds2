...
///
/// 대상 IP/Port 는 공유 <see cref="SharedPaths.PlcConnectionFilePath"/>(PlcConnection.json — Promaker 가
/// 쓰는 SSOT)에서 읽는다. 핑은 ICMP 가 아니라 PLC 제어 포트로의 <b>TCP connect</b> — 게이트웨이가 실제로
...
public sealed class PlcPingService
{
    ...
    public PlcPingService(ILogger<PlcPingService> logger) => _logger = logger;
    ...
    public async Task<IReadOnlyList<PlcPingResult>> ProbeAsync(CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var results = new List<PlcPingResult>(endpoints.Count);
            foreach (var ep in endpoints)
                results.Add(await ProbeOneAsync(ep, ct));
    ...
    /// <summary>PlcConnection.json 에서 현재 활성 PLC 엔드포인트를 읽는다. 현재는 단일 활성 연결.</summary>
