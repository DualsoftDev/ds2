...
public class OeeManualSettings
{
    ...
    /// 고장 규칙을 넘긴 행이라도 <b>행 전체 길이가 이 경계 이상이면 비생산</b>이다(doc/30 §12-⑧ 비생산 우선 —
    /// 주말 방치를 고장으로 세지 않는다). 낮출수록 대기가 분모 밖으로 빠져 가용성이 후해지므로 주의.
    /// 판정을 옮기는 수단은 이 배수 하나다 — 행 단위 수동 확정은 2026-09-18 폐기(doc/30 §11.1).
