...
; ── 시크릿(dsp.conf) 자동 배치 ──
; 고객 설치본이 첫 부팅부터 메일 발송(BriefingRelay)·클라우드 로그인(CloudAuth) 등에 연결되도록 시크릿을 배치한다.
;   dsp.conf = 정본 파일명(런타임이 직접 읽음, Program.cs). "Secrets" 를 배포 타깃에 광고하지 않는 무해한 이름.
; ▸ 기본(권장): 정본 dsp.conf 파일이 있으면 build-installer.bat 가 publish 에 넣어 [Files] 로 {app}\dsp.conf 배치.
;               전 섹션(Briefing+CloudAuth+ExternalAccess) 통째 반영. iss 는 설치 후 icacls 로 권한만 잠근다.
; ▸ 폴백: 정본 파일이 없고 브리핑 API 키만 주입되면(/DBriefingApiKey) BriefingRelay 섹션만 합성해 기록.
;   어느 경우든 SYSTEM(서비스 계정)+Administrators 만 접근하도록 상속 제거 후 잠근다(일반 Users 읽기 차단).
...
begin
  ...
  begin
    ...
    // ── 시크릿(dsp.conf) 배치 + 권한 잠금 ──
    // 서비스 시작 *전*(ssPostInstall)에 처리해 첫 부팅부터 연동이 살아있게 한다. 사용자 설정(Production.json)과 별개 파일.
    SecretsPath := ExpandConstant('{app}\dsp.conf');
    // 폴백 경로: 정본 dsp.conf 파일이 publish 로 실리지 않았고(=파일 부재) 브리핑 키만 주입된 빌드 → BriefingRelay 만 합성.
    // (정본 파일이 실린 기본 경로면 [Files] 가 이미 {app}\dsp.conf 를 배치했으므로 덮어쓰지 않고 그대로 둔다 — CloudAuth 등 보존.)
    if (not FileExists(SecretsPath)) and ('{#BriefingApiKey}' <> '') then
      SaveStringToFile(SecretsPath,
        ...
        '  }' + #13#10 +
        '}' + #13#10, False);
    // 정본이 실렸거나 폴백 합성으로 파일이 생겼으면 권한 잠금: 상속 제거(/inheritance:r) 후
    // SYSTEM(S-1-5-18, LocalSystem 서비스 계정)+Administrators(S-1-5-32-544) 만 전체 권한 → 일반 사용자 읽기 차단.
    // (SID 사용 → 로케일 무관. 매 설치/업그레이드마다 재적용해 중앙 관리 값·권한을 갱신.)
