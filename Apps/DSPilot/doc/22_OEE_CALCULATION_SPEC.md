...
> **2026-09-11 대체**: 판정 규칙은 [doc/28](28_TWO_RULE_STOP_MODEL_PLAN.md) §1(두 규칙 · 사이클 행 단위 — 완료 행 mt/wt, 불인정 행 ct)로 대체됐다. 아래는 이력.

> **2026-09-09 갱신 — 판정 축 CT → WT/MT 2축 전환 ([doc/27](27_WT_MT_TWO_AXIS_STOP_CLASSIFICATION_SPEC.md) 정본).** 아래 ①②·§3.3 의 `평균 CT × 배수` 경계는 `중앙 WT × 배수`(정지 기본 5× / 비생산 30×, 중앙 CT 1·10 사이클 하한)로 바뀌었고, MT 과주행(고장)은 길이 무관 비생산 승격 대상이 아니다. 설정 키 `IdleWtMultiplier / NonProdWtMultiplier`(구 CT 키 미이관). 본 절의 CT 축 서술은 WT 기준선이 없는 flow(tail 미정의)의 폴백으로만 남는다.

> **2026-07-13 갱신 — 판정 배수 사용자 설정화.** ①② 의 판정 경계가 `CT이상치 × 비가동 배수`(기본 **2.5×**,
> `OeeManualSettings.IdleCtMultiplier`)로 바뀌었다(종전 1×). 경계 아래의 느린 사이클은 **정상**으로 Σ실측CT 에
> 편입돼 속도 손실이 성능 P 로 재배분된다(A↑·P↓) — 성능 표준치(§4)와 onset/repair 는 여전히 **1×CT이상치**.
> §3.3 의 비생산 배수(기본 10×, `NonProdCtMultiplier`)도 함께 설정화. 두 배수는 설비효율 현황의
> '정지·비생산 판정 기준' 카드에서 조절(분류 밴드 + flow 임계 환산 + 저장 전 재분류 미리보기),
> API = `GET/PUT /api/oee/ct-multipliers`, `GET ct-multipliers/preview`(오버라이드 계산 — 감지로그 materialize 억제).
> 제약 = 비가동 배수 < 비생산 배수(역전 시 저장 거부, 읽기 경로는 `ResolveCtMultipliers` 가 방어 보정).
> 저장 시 사전계산(P2) 저장본 동기 폐기 + 학습기(§3.5) 24h 캐시 폐기. 무사이클 **감지** 임계(doc/23 3×gap' 체인)는
> 별개 — 이 배수의 영향을 받지 않는다. 기존 `oeeNonProdDetectionLog` 행은 감지 당시 배수 스냅샷대로 유지(감사·재현성).

한 사이클이 아래 중 하나면 **그 사이클 CT 전체가 비가동 CT** (P5 §①: 인식지연 + 고장 + going후 정상회복 모두 포함).

1. **MT 과주행**: `MT > CT이상치 × 비가동배수` — 동작이 판정 경계보다 길게 늘어진 정지(완료가 늦게 발화).
2. **CT 폭주(완료 실패)**: `complete = null AND CT > CT이상치 × 비가동배수` — 끝내 완료 못 한 정지 사이클(§1 의 MT=null 케이스 보강).
...
>
> ⚠ **2026-09-18 — 사용자 오버라이드 축 폐기.** 위 문단의 '비생산↔비가동 보내기'(`reclassify`)와 고장/유지보수
> 수동 체크는 API·저장소·화면에서 전부 삭제됐다. 자동 판정의 오심은 행 라벨이 아니라 **배수(κ)** 로 교정한다
> (doc/30 §11.1 · §12-⑨ κ 변경 시 자동 재라벨 · §13 2.5차). 당일 10×CT 판정과 2-패스 카빙은 그대로다.
...
- **3단 분류**(배수는 2026-07-13 부터 사용자 설정 — 기본 비가동 2.5×·비생산 10×): CT ≤ 비가동배수×평균 = 정상 /
  비가동배수× < (무변화 정지) < 비생산배수×평균 = 비가동(A 깎임) / ≥ 비생산배수×평균 = **비생산**(분모 밖).
...
> **2026-09-11 대체**: A = 가동 ÷ 생산가능(벽시계), P 표준치 = 14일 **중앙** CT, 고장 = 행 전체, MTTR → 고장 사이클 평균 시간 — [doc/28](28_TWO_RULE_STOP_MODEL_PLAN.md) §2 참조. 아래는 이력.
