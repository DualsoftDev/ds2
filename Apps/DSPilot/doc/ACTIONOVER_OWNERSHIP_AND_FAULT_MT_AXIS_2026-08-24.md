...
**기동 직후 오탐 문제는 원인만 규명됐고 수정은 미적용이다(§5 최우선).**
...
|---|---|
| 컴파일 | ✓ 0 error |
| 테스트 | ✓ 298/298 (신규 10건 포함) |
| DSPilot ActionOver 실발화 | ✓ 확인 — `01:47:56Z 가공/2nd_stp.RET 6620ms` (재기동 후 발행분) |
| 완료대기 시계 실발화 | **✗ 미검증** — 발행 이력 0건. 발행값이 `MaxMs+1`이 아닌 실측 경과면 이 경로 |
...
### ★ P0 — 기동 직후 오탐이 아직 살아 있다

§1-5 원인만 규명, **수정 미적용**. 현재도 재기동/재접속마다 오탐이 난다.

합의된 방향:
1. **엔진 워치독 경로 차단** — `SetMaxMeasured(_ => true)` 제거 + `AbnormalDetected` 구독 해제. 판정을 완료대기 시계 단일화. 워치독이 잡는 케이스는 시계가 전부 잡고(상위집합), 시계는 실제 관측한 OUT 상승에서만 시작해 baseline 오염이 구조적으로 불가능
2. **블랙아웃·재동기 시 `_overClock` 전량 폐기**
3. (선택) 기동 후 15~30초 절대 유예
