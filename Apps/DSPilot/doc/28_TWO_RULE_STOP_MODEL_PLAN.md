...
>
> ⚠ **부분 폐기(2026-09-18)** — 이 문서의 **수동 전환 축**(§2.6 사용자 라벨 우선, §2.8 확인 필요 대기열·전환 UX·되돌리기,
> classify/reclassify/bulk-* API)은 전부 제거됐다. 길이가 비생산 경계 이상인 행은 사람이 되돌리는 대상이 아니라 그냥 비생산이다
> (doc/30 §12-⑧ 비생산 우선 · §11.1 원인 어휘 금지 · §13 2.5차). 두 규칙 판정 자체(MT·WT 축, 배수 2개)는 그대로 살아 있다.
...
- **표본 게이트**: 14일 창의 완료 사이클이 **10건 미만**이면 기준선은 만들되 ①②를 적용하지 않는다. ~~(전부 정상)~~ → **판정 불가 = 분모·분자 밖**(2026-09-14, 미계측·진행 중과 같은 자리). 종전 "전부 정상"은 그 flow 의 모든 행(긴 정지 포함)을 가동으로 계상해, 원칙 "측정 안 되는 설비를 만점으로 표시하지 않는다"와 모순이었다(실측 교정지그: 가동 105.6h 중 98h 가 정지, 라인 분모의 23% 점유 → 라인 A 81%↔94% 단차). 제외분은 `UnjudgedWallMs`/커버리지로 반드시 보고한다 — 빼는 것과 뺐다고 말하는 것이 한 세트. 화면 '잠정' 배지 기준 5→10건. 0건 = 종전대로 '—'.
