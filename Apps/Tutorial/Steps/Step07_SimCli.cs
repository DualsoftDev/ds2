...
static class Step07_SimCli
{
    ...
    public static void Run(TutorialContext ctx, bool silent = false)
    {
        ...
        store.Works[ctx.W1Id].SetSimulationProperties(new SimulationWorkProperties { Duration = TimeSpan.FromMilliseconds(500) });
        store.Works[ctx.W2Id].SetSimulationProperties(new SimulationWorkProperties { Duration = TimeSpan.FromMilliseconds(800) });
        store.Works[ctx.W3Id].SetSimulationProperties(new SimulationWorkProperties { Duration = TimeSpan.FromMilliseconds(500) });
