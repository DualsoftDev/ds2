...
/// <summary>
/// 업로드 대상 선택 — macOS Finder 컬럼 뷰(사이트 › 단말 › 상세). 로그인 토큰으로
...
public partial class PvTargetDialog : Window
{
    ...
    private PvTargetDialog(IReadOnlyList<PvSite> sites)
    {
        InitializeComponent();
    ...
    public static (PvSite Site, PvEdge Edge)? Show(IPvClient client, string token, Window? owner = null)
    {
        ...
        var dlg = new PvTargetDialog(r.Sites) { Owner = owner ?? Application.Current?.MainWindow };
    ...
    private void EdgeList_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        UploadButton.IsEnabled = SelectedEdge is not null;
