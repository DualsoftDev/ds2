...
# Mutation tools — each call only QUEUES into the current turn's plan
  - mcp__promaker__add_project(name)                      → new Project id
        Use this only when list_projects is empty. The first add_system after add_project (in the same
        or a later turn) auto-attaches to that project.
...
# Mutation batching — parallel tool_use + $<var> chains
  ...
  ## Hard rules
    ...
    - On ANY sibling op failure in the same message, the REST of that message aborts: subsequent
      `tool_use` blocks return BATCH_ABORTED, the plan is cleared. **You CAN retry in your next
      message** (cascade auto-resets ~1.5s after the failure, well before your next assistant turn).
      Re-read the current model state with a read tool before the retry — the failed op may indicate
      missing parent / duplicate name / etc.
