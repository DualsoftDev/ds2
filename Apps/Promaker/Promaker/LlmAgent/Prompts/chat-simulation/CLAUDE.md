...
3. `../3.tooling.md` — MCP 도구 사용 규약 (`apply_operations` 우선, helper 4종, 운영 규칙 7개)
...
**(b) MCP 호출 명세** — 실제로 발행될 도구 호출. JSON code fence 로:

````
```mcp
TOOL: mcp__promaker__apply_operations
ARGS:
{
  "operations": [
    {"op": "add_project",       "ref": "p",   "args": {"name": "M1"}},
    {"op": "add_cylinder",      "ref": "cyl", "args": {"name": "Cyl", "apiDef1Ref": "cylAdv", "apiDef2Ref": "cylRet"}},
    ...
    {"op": "add_call",          "args": {"workId": "@adv", "apiDefId": "@cylAdv"}},
    {"op": "add_call",          "args": {"workId": "@ret", "apiDefId": "@cylRet"}},
...
- `TOOL:` 한 줄 + `ARGS:` 다음에 JSON. JSON 은 `apply_operations` 의 인자 그대로.
- 단일 mutation (legacy) 호출이라도 동일 형식 — `TOOL: mcp__promaker__add_project` + `ARGS: {"name": "M1"}`.
- read 도구 시뮬레이션도 동일 — `TOOL: mcp__promaker__describe_subtree` + `ARGS: {"rootId": "<placeholder>", "depth": 2}`.

### 2.3 ref 와 GUID

실제 MCP 호출이 없으므로 GUID 는 발급되지 않습니다. **GUID 가 필요한 자리는 그대로 placeholder** (`@<ref>`
또는 `<guid:adv>` 같은 명시적 placeholder) 로 두십시오 — 임의 UUID 를 지어내지 마십시오. `apply_operations`
의 sub-ref (`@cyl` / `@cylAdv` 등) 는 production 과 동일하게 사용 가능하며 출력 명세에서도 그대로 유지.

기존 entity 재사용 시나리오 (예: "기존 Cyl1 에 새 Active Work 추가") 는 사용자가 GUID 를 알려주지 않는
한 — `<guid:cyl1>` 같은 placeholder 로 두고 사용자에게 "실제 운영에서는 `find_by_name` / `list_systems`
로 조회한 GUID 가 들어갑니다" 라고 1줄 안내.
...
- 한 turn 당 한 번의 `apply_operations` 명세 (또는 1 회 명확화 질문) 가 원칙. 다중 turn 에 걸친 누적
  모델링은 **시뮬레이터 안에서는 store 가 없으므로** — 사용자가 "이전 turn 에서 만든 X 를 …" 이라 하면
  placeholder GUID 를 가정하고 진행, 매번 시작 상태가 비어 있다고 가정하지 마십시오.
