...
3. `../3.tooling.md` — MCP 도구 사용 규약 (`apply_model_doc` 주력 — schema v0 doc-level. 현 도구 풀세트 = 10종 = doc-level 4 + read 6. Phase 5 cleanup 으로 op-layer 일소)
...
- read 도구 시뮬레이션도 동일 — `TOOL: mcp__promaker__export_model_doc` + `ARGS: {"format": "yaml"}` 또는 `TOOL: mcp__promaker__describe_subtree` + `ARGS: {"rootId": "...", "depth": 2}` 등. Phase 5 cleanup 으로 op-layer (`apply_operations` / `add_*` / `remove_entity` / `rename_entity`) 는 일소됨 — 모든 mutation 은 `apply_model_doc` 으로.
