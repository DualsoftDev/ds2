...
- read 도구 시뮬레이션도 동일 — `TOOL: mcp__promaker__export_model_doc` + `ARGS: {"format": "yaml"}` (전체) 또는 `+ ARGS: {"format": "yaml", "path": ".Proj.SysA", "depth": 1}` (scope 좁힘) 등. Phase 5 cleanup 으로 op-layer (`apply_operations` / `add_*` / `remove_entity` / `rename_entity`) 는 일소됨 — 모든 mutation 은 `apply_model_doc` 으로. Phase 6 cleanup 으로 list/describe read (`list_projects` / `list_systems` / `describe_system` / `describe_subtree`) 는 일소됨 — 모든 read scope 는 `export_model_doc(path?, depth?)` 의 인자로.
