...
/// <summary>
/// Ds2.Core.Condition (ApiCall-바운드 leaves + IsOR/IsRising 플래그) ↔ ExprNode.
///
/// 매핑 규칙:
///   • Condition(IsOR=false): ExprKind.And, children = leaves(Var with RefKey) ++ children(recursive)
///   • Condition(IsOR=true) : ExprKind.Or
///   • IsRising=true            : 각 leaf 변수를 ExprKind.Rising 으로 변환 (legacy 의미)
///
/// 역변환은 의미 손실 가능 — 트리 편집기에서 NegVar/Falling/Raw 같이 Condition 에 직접 매핑 안 되는
/// 노드를 만들면 Raw 식 또는 무시. 사용자에게 사전 안내 필요.
/// </summary>
public static class ConditionConverter
{
    /// <summary>ConditionPanelItem (Editor 의 view-type) → ExprNode 트리.</summary>
    public static ExprNode FromCallCondition(ConditionPanelItem cc, DsStore store)
    {
    ...
    // 역변환 (ExprNode → Condition) 은 store API 기반 rebuild 가 필요 — 별도 마이그레이션 작업으로 분리.
    // Condition 은 internal 생성자를 가지며 ApiCall 객체 참조 무결성 유지가 까다로워, 본 변환에서는
