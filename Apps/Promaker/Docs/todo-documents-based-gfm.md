...
| r4 | 2026-05-28 | N15 박제 — `<folder>/guide/*.md` 사용자 작성 가이드 자동 박제 정책. `F:/Git/dualsoft/secrets/KBSamples/core/guide/` 의 4 파일 (01-promaker-yaml-mapping / 02-204-key-jig-example / 03-callcondition-fallback / 04-tool-use) 시연 박제. documents-based-gfm.md §8.1 / §8.2 / §8.5.4 갱신 — `_user-guide-*.md` 박제 흐름 사실 SSOT 박제. |
| r5 | 2026-05-28 | PR-N15 (`da1e0eb0`) + PR-N8 (`d4eb1417`) commit 완료. N16 박제 — R3a caption cache lookup. `.lighthouse-kb/` wipe 직전 ImageCache sweep + `<source>/.lighthouse-caption-cache.json` 박제 + caption 생성 step cache lookup → Anthropic VLM API call skip path. |
...
| **N3** | caption 두 모델 모두 실패 시 fallback | PR-I6 진입 시 | **Opus → Sonnet → SkippedCaption + caption=NULL** — 외부 todo `done-lighthouse-indexer-claude-caption.md` §2 #10 의 per-image max 2 attempts 정합. retry queue 별도 박제 안 함 (다음 색인 시 NULL row 자동 재시도) |
...
| **N15** | `<folder>/guide/*.md` 사용자 작성 가이드 자동 박제 — `/indexer <folder>` 시 `guide/` 디렉토리 (재귀) sweep + 머리말 6행 자동 prepend + `.lighthouse-kb/summary/_user-guide-<relative-path>.md` 박제. specialized digest cache breakpoint 3 에 흡수 → KB 활성/비활성 토글과 정합. lib embedded resource 안 (이전 검토 안) 은 보류 — 사용자 손 SSOT 단일. | P2 hand-off 직후 별 PR (PR-I6 / N8 wiring 과 평행 가능, 코드 영역 완전 직교) | **별 PR 1건** — (a) `Solutions/Core/Ds2.LightHouse/UserGuideImporter.fs` 신규 (`<folder>/guide/` 재귀 sweep + 머리말 6행 prepend + `.lighthouse-kb/summary/_user-guide-<basename>.md` 박제 — subdir 시 `_user-guide-<sub>-<basename>.md`, ~80~100 line). (b) `Classifier.fs` 의 source 결정 단계에 `<folder>/guide/` filter 추가 (~5 line, 이중 박제 회피 — (A) keyword digest / (B) text dump / (C) chunk 모두 skip). (c) `Ds2.LightHouseService/Indexer 흐름` 에 UserGuideImporter 진입점 hook (~10 line). (d) `SpecializedDigestBuilder.fs` 의 `_user-guide-*` 우선 ordering 명시 (~5 line, 기본 alphabetical 도 자동 동작이라 선택). (e) `UserGuideImporterTests.fs` 신규 (sweep / 머리말 prepend / 중복 회피 / 빈 디렉토리 / 재귀 sweep 회귀, ~150 line). 머리말 = §8.5.5 6행 SSOT 정합 — `<!-- source: guide/<relative-path> (cross-ref-hash: <sha256>) -->` 박제 → stale 감지. 재색인 trigger = 수동 (`/indexer <folder>` 재호출), file watcher 도입은 P3 backlog (N7 정합). 시연 자료 박제 (2026-05-28 turn) = `F:/Git/dualsoft/secrets/KBSamples/core/guide/` 의 4 파일 (`01-promaker-yaml-mapping.md` / `02-204-key-jig-example.md` / `03-callcondition-fallback.md` / `04-tool-use.md`). |
| **N16** | image caption cache lookup (R3a) — `.lighthouse-kb/` wipe 직전 ImageCache (ImageHash, CaptionText, CaptionModel, CaptionAt) sweep → `<source>/.lighthouse-caption-cache.json` 박제. caption 생성 step (Vlm.buildCaptionGen closure) 진입 직전 cache lookup → hit 시 Anthropic VLM API call skip + cache 값 (Captioned) 반환. miss 시 정상 Anthropic call. cache file 보존 → 다음 색인 시점 incremental 재사용. cache fail (부재/corrupt/version mismatch) → 정상 흐름 fallback (모든 image Anthropic call). | 본 turn 즉시 별 PR 진입 (사용자 결정 1a, auto commit #3) | **별 PR 1건** — (a) `Solutions/Core/Ds2.LightHouse/CaptionCache.fs` 신규 (~80 line) — `saveFromDb : sourceFolder -> conn -> unit` (ImageStore sweep + JSON 박제) + `tryLookup : sourceFolder -> hash -> CaptionResult option` (lookup hit/miss). schema = `{version: 1, savedAt: <ISO8601>, captions: {<sha256-hex>: {captionText, captionModel, capturedAt}}}`. 위치 = `<source>/.lighthouse-caption-cache.json` (KB-local, .lighthouse-kb/ wipe 와 무관 경로). (b) `Solutions/Tools/Ds2.LightHouse.Cli/Packager.fs` 의 `resetKbDir` 호출 직전 또는 `runIngest` 진입 직전 `CaptionCache.saveFromDb` hook (~10 line). (c) `Solutions/Core/Ds2.LightHouse/Vlm.fs` (또는 `CaptionGenerator.fs`) 의 `buildCaptionGen` 가 반환하는 closure 진입 직전 `CaptionCache.tryLookup sourceFolder hash` hook — hit 시 `Captioned (text, model)` 반환 + Anthropic API call skip, miss 시 기존 Anthropic call (~15 line). closure signature 의 sourceFolder 인자 전파 의무 (build site 갱신). (d) `Solutions/Tests/Ds2.LightHouse.Tests/CaptionCacheTests.fs` 신규 (~120 line) — save/load happy path / hash hit / hash miss / 빈 cache file / 부재 cache file / corrupt JSON / version mismatch / 멱등 회귀. cache 손실 = fail-safe (정상 색인 흐름 fallback). cache hit 시 Anthropic call 진입 0 검증 (mock captionGen 호출 횟수). |
| **PR-I8** | **IoListStrategy v2 — 광명2 IO List 박제 결함 해소** (orchestrator: `done-lighthouse-iolist-v2.md`). Phase 0 (root cause 진단) + Phase 1 (`IoListStrategy.reshapeRowToBlocks` 의 22-col 6-col block 정합 fix — `blockSize=5` + leading 빈 col skip `i=1` 시작 + trailing 빈 col 자동 흡수) + Phase 3 (27-col RB 자동 흡수 — 5 block × 6-col layout) + Phase 4 (`MarkdownCapPolicy.applyCapFor` device-aware sampling — IoList 전용 `applyIoListSampling` + Direction 분포 박제) + Phase 5 (XlsxStrategiesTests fixture 22-col 정합 갱신 + IoListStrategyTests 신설 6건 + 검열 Major fix 4건 박제). | 2026-05-28 commit (branch `light-house`) — Phase 1: `adf4970b` / Phase 4: `3c8d99df` / Phase 5: (본 PR commit). | **별 PR 1건 (완료)** — (a) `Solutions/Core/Ds2.LightHouse/Extractors/XlsxStrategies/IoListStrategy.fs` (Phase 1 `reshapeRowToBlocks` fix + signature `hasFourBlockLayout` threshold 22) — Direction Input/Output 박제율 0% → 100% 복구. (b) `Solutions/Core/Ds2.LightHouse/MarkdownCapPolicy.fs` (Phase 4 `applyCapFor` strategyName dispatch + Phase 5 검열 Major fix: elide marker row 의 정확 elidedCnt 박제 / `deviceOnly` head/tail overlap 흡수 / fallback 주석 정정). (c) `Solutions/Tests/Ds2.LightHouse.Tests/XlsxStrategiesTests.fs` (fixture 22-col 정합 갱신 — 기존 baseline 실패 test 1건 통과). (d) `Solutions/Tests/Ds2.LightHouse.Tests/IoListStrategyTests.fs` 신규 (6 test — 22-col reshape / 27-col RB / device base token edge case / cap policy device-aware sampling / default cap byte-equal 회귀 / 51-col COVER skip). 8-col WRS 변형 (시트 4개 ~846 tag) 은 본 PR 범위 외 (Phase 2 skip, 후속 PR backlog). |
...
- **caption prompt SSOT 갱신** — `done-lighthouse-indexer-claude-caption.md` 의
...
- `done-lighthouse-indexer-claude-caption.md` — `/indexer` skill subagent caption
