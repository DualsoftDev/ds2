...
현재 1 요청당 RT 분해: `ToolSearch` (R1) → `export_model_doc(depth=0)` (R2 — Phase 6 이전 `list_projects` 와 동등) → `ToolSearch` (R3) → `apply_model_doc` (R4 — Phase 5 이전 `apply_operations` 와 동등) = 4. snapshot 자동 첨부로 R2 제거, schema preload 로 R1/R3 제거, R4 만 남기는 것이 목표.
...
# Store snapshot (자동 첨부)
  ...
  - 사용자가 명시적으로 현재 상태 확인을 요청하지 않는 한 read 도구 호출 금지.
  - mutation 은 `apply_model_doc` (전체 doc) 또는 `apply_model_doc` 의 `patch:` 키 (점진).
```

**6.2. 기존 greenfield 룰 수정** (`3.tooling.md`)

변경 전 (Phase 5/6 이전):
> "`list_projects` 가 비어 있으면 같은 turn 에 add_project 1회 호출 허용"

변경 후 (Phase 6 정렬 — `export_model_doc(depth=0)` 으로 cold-start 표면 통일):
> "**snapshot 의 `projects` 가 비어 있으면** 같은 turn 에 `apply_model_doc` 의 `project:` 키 포함 발행 허용. snapshot 이 첨부되지 않은 turn 에서는 직전 transcript 의 snapshot 을 참조. snapshot 도 없는 첫 turn 에 한해 `export_model_doc(depth=0)` 1회 호출 허용 (cold start)."

→ 룰 충돌 시 LLM 이 보수적으로 read 호출 → R2 제거 의미 상실.
...
| R7 (§J3) | `Promaker.csproj NoWarn=CS9057` 회수 시도 — 실측 결과 회수 불가 확인 (OllamaSharp 5.4.25 분석기가 Roslyn 5.0 요구, 현 SDK 9.0.301 = Roslyn 4.14). 회수 시점은 **VS 2026 (18.0) / .NET 10 SDK** (Roslyn 5.0.0 의 첫 SDK), 또는 OllamaSharp 분석기 downgrade. 코멘트에 회수 조건 명시 | `Promaker.csproj` |
...
- `apply_model_doc` 1회 호출 = batch N op = `WithTransaction` 1회 = `Revision++` 1회 (자동 보장 — Phase 5 이전 `apply_operations` 와 동등)
- snapshot 매 turn 자동 첨부 (delta-only, revision 변경 시만 새 snapshot — sticky 유지) → read 도구 호출 제거 (Phase 6 이후 잔존 read = `find_by_name` + `validate_model` + `export_model_doc` 의 scope 인자)
...
9. `3.tooling.md` 의 cold-start 권장 문구를 §6.2 로 교체 (Phase 6 정렬: `list_projects` → `export_model_doc(depth=0)`)

### Step 4 — Cache control (in-process AnthropicApi only) — partial DONE
10. **DONE** — `ApiChatProvider.SendImpl` (firstTurn 분기) 에서 provider label "Anthropic" 인 경우 system prompt 의 TextContent 에 `cache_control: ephemeral` 부착. **Anthropic 공식 .NET SDK (`<PackageReference Include="Anthropic" />`) 가 `WithCacheControl(this AIContent, Anthropic.Models.Messages.CacheControlEphemeral)` extension 으로 정식 지원** (`Microsoft.Extensions.AI` namespace 가 아님 — 2026-05-11 cross-check 정정). `IChatClient` 추상 위 `AIContent` 에 부착하면 `AnthropicClient.AsIChatClient` 어댑터가 wire 직전 `cache_control` 로 변환. system + tool schema 까지 cache prefix.
...
4. **기대**: 단일 turn 안에 `apply_model_doc` 1회 호출 + 응답 종료. 중간에 read 도구 호출, 추가 `ToolSearch` 호출 없음.
...
| Stale snapshot 의심 시 | system prompt 룰 + 첫 turn 한정 `export_model_doc(depth=0)` cold-start 허용 (§6.2 — Phase 6 정렬, 옛 `list_projects`) |
...
- **v5.4** — R2/R3 (운영 안전망 일부) 구현 반영
  ...
  - **R2 cross-check 보강** (2026-05-11) — "외부 제어 완전 불가" 는 부분 정정. **turn 단위 keepalive / max-history / history-limit 같은 직접 제어는 여전히 불가**하지만, 환경변수 **`CLAUDE_AUTOCOMPACT_PCT_OVERRIDE`** (1~100, 기본 ~83.5%) 로 compaction threshold 만 조정 가능. subprocess 띄우기 전 95~99% 로 상향 시 prefix-stable 윈도우 연장 효과. `settings.json` 의 `autoCompactEnabled` / `compactionThreshold` 같은 키는 docs 노출은 있어도 현 schema 에서 **silently ignored** (anthropics/claude-code issue #38483 등). 본 R2 의 rolling history 정책은 in-process API provider 한정 — Claude CLI provider 측은 `CLAUDE_AUTOCOMPACT_PCT_OVERRIDE` 환경변수 주입이 보조 우회 수단
- **v1** — 초기 작성
