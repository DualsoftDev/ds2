...
### 진행 상태
- **현재 rev**: **r14 (s6-r25 sweep 동기화)** — server-side `todo-lighthouse-kb-server.md` s6-r25 묶음에서 본 doc §3.15 sweep 잔여 3개소 정정 (line 200 §3.0 표 / line 790 Phase 2 헤더 / line 818 Phase 3 검토 — cached lazy → eager VLM caption, s6-r18 D-2-2 정합). line 7 historical / line 581 mr4 (취소선 정합 적용 완료) 변경 0. Phase 1 본체는 r10/r11 결정 그대로 유지 (lib 152 Fact). lib + server 분담은 server.md §0 + §7.1 본 turn row 참조.
- **후속 phase 별도 추적**: `todo-lighthouse-kb-server.md` (**s6-r25**, backlog 묶음 17건 처리 완료 + 자가 검열 review Major-1 lock 변경 + 누적 530 Fact). 다음 권장 = Phase S7 (mTLS / SSE / multi-service routing) 또는 Phase 2 task F (OCR / embedding) — 사용자 우선순위 결정 시 진입.
