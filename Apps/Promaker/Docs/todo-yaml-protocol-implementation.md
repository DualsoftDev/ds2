...
| Phase 4 (UI YAML preview/apply) | ⏳ 대기 — **현 진입 후보** |
...
**D. todo 자체 organizational cleanup** (소작업, 30분) — 본 todo 의 §3.2/§3.3/§3.4 절이 *옛 진입 가이드 + Phase 1 결정 본문* 으로 ✅ 완료 상태인데 본문이 길게 잔존. cross-ref (line 179 / line 304 의 "§3.2 참조") 가 line 150 *Phase 2 자가 검열 잔여 우려* 절을 가리키므로 번호 재정렬은 위험 — *본문만* 축약 + 결정 본문은 SSOT (`yaml-protocol-v0.md §1.7`) 로 redirect.
...
**측정 baseline** — Phase 3 효과 검증은 `Promaker` GUI 빌드/실행 + 동일 사양 (`ds2.log20260512` turn #2 = 3 zone × N cyl + Pusher Punch) 재현이 필요 (사용자 환경). baseline `$0.61 / 62 op / 1 재시도` 대비 기대 `≤ $0.20 / 1 op`. **LLM 측 자가 측정 불가** — 사용자가 측정 후 본 todo §3.0 측정 결과 절에 회신 권장.

A (Phase 2.5 refactor backlog), B (Phase 3 prompt 마이그레이션) 모두 완료 — §3.1 / §7.3 참조.

**보류된 후속 cycle 후보** (corpus / trigger 등장 시):
...
### 3.2 Phase 3 — prompt 마이그레이션 (✅ 완료, 참조용)

진입 가이드는 **§7.3 으로 통합** — 실제 적용 결과 + 자가 검열 + 5인 외부 review meta 모두 §7.3 참조.

핵심 prompt 룰 6가지 (SSOT §1.7 ↔ `3.tooling.md` 의 "핵심 룰" 절) 는 모두 prompt 본문에 반영됨. 측정 (baseline `$0.61 / 62 op / 1 재시도` 대비 기대 `≤ $0.20 / 1 op`) 은 사용자 환경에서 Promaker GUI 재현 필요 — 본 항목은 §3.0 의 "측정 baseline" 으로 이관.
...
### 7.4 Phase 0 직전 별개 작업 (commit `44cf62f`)
