...
**현 working tree 상태**: Phase 2.5 작업 unstaged. `bKwak` branch, local only (upstream 없음). 자동 생성 untracked = `Apps/Promaker/Promaker.kwak.sln` + `Apps/Promaker/Docs/ds2.log*` (제외). **315 테스트 통과** (Phase 1 baseline 289 + Phase 2 신규 26). 다음 세션 진입 시 *clean working tree* 가정 (Phase 2.5 commit 후).
...
### 3.0 우선순위 가이드 (Phase 2.5 이후)

**B. Phase 3 prompt 마이그레이션** (필수, 큰 작업) — todo §6 룰 #10 *독립 PR 필수*. `3.tooling.md` 50-70% 재작성 + `2.modeling.md` 검토. **현 진입 후보**.

A (Phase 2.5 refactor backlog) 는 cycle1 + cycle2 (5인 외부 review meta) 모두 완료 — §3.1 참조.

### 3.1 Phase 2.5 — refactor backlog (✅ 완료, cycle1 + cycle2)

**진행 결과**:
- **cycle1** (1인 sub-agent review): M5 → M4 → m1/m3/m4/m5/m6/m7 묶음 cycle 순으로 적용. m2/M3 는 보류. 315 통과 무회귀.
- **cycle2** (5인 외부 review meta): C1 → M1 → M2 → M3 → M4 → M5 → m1/m5 순. m2/m3/m4/m6/m7/m8/o1 은 반론 보류 (사유 §3.1 자가 검열 결과에 기술). 322 통과 (cycle2 신규 7).

| ID | 항목 | 결과 |
|---|---|---|
| **M5** | `Queries.tryResolveCallTargetSystem` 신설 — Call→ApiCall.tryHead→ApiDefId→ApiDef→ParentSystem chain public helper. `ModelProtocol.exportToJson` 의 7줄 chain + `Queries.findConflictingDeviceSystemType` 의 10줄 chain → helper 호출 1줄로 통합. | ✅ 적용 |
| **M4** | `KnownSugars.fs` 신규 파일 (ToolOperations.fs 앞). `KnownSugarSpec` record + `cylinder/clamp/robot` SSOT entry + `tryMatchFingerprint`. `ToolOperations.queueAddCylinder/Clamp/Robot` (default), `ModelProtocol.deviceDefaults` (DeviceLiteral 매핑), `ModelProtocol.exportToJson` fingerprint 역추정 모두 표 lookup 으로 통합. literal `["ADV";"RET"]` / `["CLP";"UNCLP"]` 4 곳 제거. | ✅ 적용 |
| **m1** | RelaxedShape / captureRelaxed 를 `Helpers/ModelEquivalence` 로 이동 (정보 중복 제거). | ✅ 적용 |
| **m3** | `ModelEquivalence.roundTripRelaxed` 신설. §3.1 #1 테스트의 5줄 pattern → 1줄. | ✅ 적용 |
| **m4** | `scalarToJsonLiteral` 의 null literal hardcode → `yamlNullTokens.Contains` 통합. | ✅ 적용 |
| **m5** | `yamlReservedTokens = Set.unionMany [yaml12Bools; yaml11OnlyBools; yamlNullTokens]` 단일 캐시. `jsonToYamlNode` 의 3중 Or → 단일 lookup. | ✅ 적용 |
| **m6** | `ModelEquivalence.captureShape` 의 callArrows O(N×M) → `allArrowCalls` 1회 호출 + ParentId 별 그룹 캐싱. | ✅ 적용 |
| **m7** | `ModelProtocol.formatArrowType` private → public. `ModelEquivalence.captureRelaxed` 의 `%A` → `formatArrowType` 통일. | ✅ 적용 |
| **m2** | `processOneArrow` / `processOneFlowArrow` 25줄 중복 — todo 권고대로 보류 (가독성 trade-off). | ⏸ 보류 |
| **M3** | ApiCalls=[] fallback 직접 회귀 테스트 — helper 정상 경로로 시뮬레이션 불가. logger forensic 으로 간접 보장 (Phase 2 cycle3 기 적용). | ⏸ 보류 |

**자가 검열 결과** (Phase 2.5):
- **cycle1** (1인 sub-agent): 7 파일 (+155/-118). Critical 0 / Major 0 / Minor 4 (KnownSugars 확장 주의 1건만 주석 보강 적용).
- **cycle2** (5인 외부 review meta): 8 파일 (+248/-179). Critical 1 + Major 5 + Minor 8 + Outlier 1.
  - **Critical C1** (defaultOpposing SSOT 미통합): 적용. `defaultOpposing` 함수 제거 + fingerprint 매칭에서 `spec.DefaultOpposing` 직접 사용 (3-tuple 추출). SSOT 통합 완성.
  - **Major M1** (Custom 하드코드 → robot 우연 일치 silent drift 위험): 적용. `KnownSugars.customDefaultApis / customDefaultOpposing / customDefaultDuration` 모듈 상수 추출.
  - **Major M2** (queueAddCylinder/Clamp 25줄 골격 중복): 적용. `queueAddPairedSugar` private helper 추출 — cylinder/clamp 두 함수 본문 1줄 위임. (queueAddRobot 은 quota check 등 별개 — 미흡수.)
  - **Major M3** (roundTripRelaxed 주석 "5건 흡수" vs 실제 1건): 적용. `roundTripWith : (DsStore → 'T) → DsStore → 'T * 'T` generic helper + `roundTripShape` / `roundTripRelaxed` 두 specialization. 3 round-trip pattern (§3.1 단일 cylinder / §3.1 #3 multi-sugar / §3.1 #5b custom Unit) 모두 helper 호출로 치환. 주석은 "projection 함수 인자로 받아" 로 자연스럽게 정정.
  - **Major M4** (tryMatchFingerprint 단위 테스트 부재): 적용. 6 case 신규 (Unit+[ADV;RET], Unit+[RET;ADV], Unit+[CLP;UNCLP], Robot+arbitrary, Unit+[FOO]→None, Conveyor+[]→None).
  - **Major M5** (formatArrowType enum 전수 cover 부재): 적용. `Enum.GetValues<ArrowType>()` 순회 + SSOT 명시 6 케이스 subset 검증 + `Unknown(<n>)` fallback 진입 시 fail.
  - **Minor m1** (sort vs set 의미 차이): 적용. `List.sort` → `Set.ofList` 비교로 의도 명료화.
  - **Minor m5** (placeholder 주석 잔존): 적용. 2줄 → 1줄 축약.
  - **반론 / 보류**:
    - **m2** (null 가드 손실): 5인 review 자체 결론도 "F# Set/TryParse/Serializer 가 null 안전 — 실 regression 없음". 명시적 가드 복원은 paranoid — 미적용.
    - **m3** (failwithf vs Assert.False): `failwithf` 가 stack trace 풍부 + helper 외부에서 호출자 fixture 이름 별도 인자로 받을 필요 없음 — 의도된 선택. 미적용.
    - **m4** (formatArrowType private→public 우려): 5인 review 자체 결론도 "현재 정도는 수용". `InternalsVisibleTo` 도입은 over-engineering — 미적용.
    - **m6** (if/elif → match 가독성): 4단 if/elif 가 fingerprint *우선순위* (SystemType 매칭 → DefaultApis 빈 fallback → set 비교 → 미매칭) 를 *순차* 표현 — match (cond1, cond2) 는 cartesian 의미가 더 강함. 미적용.
    - **m7** (DefaultApis=[] wildcard fence): 현재 영향 없음 + KnownSugars.fs 의 확장 주의 주석으로 이미 fence — 미적용.
    - **m8** (callArrowsByParent 캐싱 1줄 주석): 기존 "Phase 2.5 m6" 주석 (`work 마다 전체 enumerate (O(N×M)) 회피`) 이 이미 적절 — 미적용.
    - **o1** (ModelEquivalence 책임 확장 — workflow runner 분리): 5인 review 자체 결론도 "PoC scope 수용 가능". helper 응집도 변화는 인지하나 별도 파일 분리는 over-engineering — 미적용.
- **빌드/외부 호환성**: 회귀 없음.
- **잔여 우려**: 없음. C1 해소로 KnownSugars SSOT 통합 완성.

### 3.2 Phase 3 — prompt 마이그레이션 (독립 PR 필수)

**진입 조건**: Phase 2 commit 완료. 본 PR 과 분리 (todo §6 룰 #11).

**대상 파일**:
- `Apps/Promaker/Promaker/LlmAgent/Prompts/3.tooling.md` (264 라인) — *주력 진입점을 `apply_model_doc` 로 교체*. 기존 op-layer 도구 21개 정의는 *escape hatch 섹션으로 강등*. schema 예시는 YAML 표기로 제공 (사람·LLM 모두 가독), LLM 은 의미 동형의 JSON object 로 직접 생성. **50-70% 재작성**.
- `2.modeling.md` (476 라인) — cascade 규칙 / canonical 형상 / arrow 결정 트리는 그대로 유지, 그러나 *예시* 는 op-level → doc-level 표기 변경.
- `1.entities.md` — 변경 없음 (entity 모델 자체는 동일).
- `4.attachments.md` — 영향 검토 (사용자 첨부 처리 룰 — apply_model schema 와 충돌 가능성).
- `Apps/Promaker/Promaker/LlmAgent/Prompts/CLAUDE.md` (Prompts 폴더) — facts.txt 흡수 규칙 등 — 영향 검토.

**Phase 3 핵심 prompt 룰** (SSOT §1.7 항목 반영 필수):
1. **GUID 노출 0** — apply_model_doc 응답의 `refs` 필드도 prompt 에서 *이름만 사용* 명시.
2. **`project:` 키 자동 처리** — store 에 이미 project 열려있으면 LLM 이 *snapshot 으로 인지* + 동일 이름 사용. 새 project 요청 시 GUI 닫기 안내.
3. **alias 무시** — `calls: [Cyl1.ADV]` 의 `Cyl1` 은 *systemName* (DevicesAlias 아님).
4. **kind: 명시 강제** — Active/Passive 항상 명시.
5. **device sugar 3종** — cylinder/clamp/robot 만 short-form. 그 외 `device: custom(<Type>), apis: [...]` long-form.
6. **YAML 표기 예시** — 사람·LLM 모두 가독. 실제 wire 는 JSON object (escape 0).

**측정**: Phase 3 후 동일 사양 (chat-samples.txt turn #2 — 3 zone × N cylinder + Pusher Punch) 으로 turn 비용 측정. baseline = `$0.61 / 62 op / 1 재시도` (turn #2 ds2.log20260512). 기대 = ≤ $0.20 / 1 op (apply_model_doc 단발).

### 3.4 Phase 2 — `export_model_doc` 정교화 + round-trip SSOT (완료, 참조용)
...
**Phase 2 잔여 작업** (체크리스트):
1. [x] **WithCyl.json round-trip 테스트** — load (Ds2.Serialization.JsonConverter.deserialize) → exportToJson → apply (새 store) → 완화 shape (RelaxedShape) 동등. SSOT §1.7 alias→systemName 정정 정책 도입으로 통과.
2. [x] **WithCyl.json 의 store 로딩 경로** — `Ds2.Serialization.JsonConverter.deserialize<DsStore>(json)` (또는 `loadFromFile<DsStore>`).
3. [x] **export 의 cascade 자식 emit 정책** — short-form 유지. multi-sugar deterministic round-trip 테스트 lock-in (#3 / #3b / #3c). long-form fallback 은 후속 cycle.
4. [x] **format=yaml 출력 안정화** — `jsonToYamlNode` 의 string 분기에 `plainScalarSafeRegex` (`^[A-Za-z_][A-Za-z0-9_\-]*$`) + reserved (yaml12Bools / yaml11OnlyBools + y/Y/n/N / yamlNullTokens) 체크. ASCII identifier 면 Plain, 아니면 DoubleQuoted.
5. [x] **device 추정 로직 정교화** — SystemType=None fail-safe `custom(Unknown)` + logWarn. Unit/Robot/Custom 분기 lock-in 테스트 (#5 / #5b). workDuration/opposing 은 override 키로 처리 (현재 동작 유지) — full long-form fallback 은 후속.
6. [x] **drift test sanity 확인** — 25 그대로 유지.
...
- **Phase 2.5 commit (예정)**: **322 통과** (cycle1 8건 무회귀 + cycle2 C1/M1/M2/M3 refactor 무회귀 + cycle2 M4 6 신규 + cycle2 M5 1 신규)
- Phase 3 진입 시 *이 baseline 위에서 시작*
