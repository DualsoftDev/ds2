...
**의존성 / Layer 룰**:
- `YamlDotNet` NuGet 추가 — F# 프로젝트 `Ds2.LlmAgent.fsproj` 에 `<PackageReference>` 한 줄. 기존 솔루션의 NuGet 버전 정책 미상 — `Directory.Packages.props` 확인 후 일관 버전 사용.
- **YAML 의존은 LlmAgent layer 안에서만**. Ds2.Core / Ds2.Editor 는 YAML 미인지 — wire 가 JSON object 이므로 자연 분리.

**구현 단계** (체크리스트 — SSOT 의 §2/§3 참조):
1. [ ] `YamlDotNet` NuGet 추가 + 변환 helper 1쌍 (`yamlToJson` / `jsonToYaml`).
2. [ ] `JsonElement` walker — SSOT §2 schema 의 각 키 별 dispatcher.
3. [ ] 이름 → entity 테이블 1-pass 빌드 + dotted-path resolver (단 §3.3 의 path 충돌 결정 선행).
4. [ ] device DU literal parse — regex 권고: `^([A-Za-z][\w]*)(?:\((.+)\))?$` (Major M6: greedy 매칭 / ASCII-only 보강 검토).
5. [ ] dispatcher — `queueAdd*` 호출 (mapping 표 = SSOT §5).
6. [ ] validate 룰 6개 (SSOT §2.7) + Levenshtein 후보 제안.
7. [ ] patch DSL 4종 (add/arrows/rename/remove) — 단 §3.3 의 add+remove 제약 결정 선행.
...
**Phase 1 작업**:
- `Solutions/Core/Ds2.LlmAgent/ToolOperations.fs:307` 의 `hasCallNameClash` *완화* — 같은 Work 안 같은 `{system}.{api}` Call 의 중복 허용 (invalidOp → pass).
- 단 이 완화가 기존 op-layer (`add_call` direct) 에도 적용되는지 검토 — 기존 LLM workflow 영향 확인 후 결정. 영향 있으면 *YAML protocol dispatcher 안에서만* 완화 (별도 entry function).
...
- `Apps/Promaker/Promaker/LlmAgent/PromakerToolNames.cs` — `All` allowlist 에 신규 5종 추가 (drift test 통과 필수)
- `Solutions/Tests/Ds2.LlmAgent.Tests/PromakerToolNamesDriftTests.fs:77` — sanity count `21` → `26` (신규 5종 반영)
