...
**현 working tree 상태**: Phase 2 (cycle1+2+3) 작업 commit 완료 가정 시점. `bKwak` branch, local only (upstream 없음). 자동 생성 untracked = `Apps/Promaker/Promaker.kwak.sln` + `Apps/Promaker/Docs/ds2.log*` (제외). **315 테스트 통과** (Phase 1 baseline 289 + Phase 2 신규 26). 다음 세션 진입 시 *clean working tree* 가정.
...
### 3.0 우선순위 가이드 (Phase 2 이후)

**A. Phase 2.5 refactor backlog** (선택, 가벼움) — 외부 review 5명 종합에서 deferred 된 구조 개선. 모듈 간 의존성/Queries 확장 영향으로 Phase 2 범위 밖. 본격 작업 전 단발 cycle 로 정리 권장.

**B. Phase 3 prompt 마이그레이션** (필수, 큰 작업) — todo §6 룰 #11 *독립 PR 필수*. `3.tooling.md` 50-70% 재작성 + `2.modeling.md` 검토.

**C. 통합 cycle (B 와 결합 가능)** — Phase 2.5 의 일부를 Phase 3 의 prompt 자료 갱신과 함께 정리.

권장 순서: **A → B**. 또는 **B 직행** (A 보류, 후속 cycle 분리).

### 3.1 Phase 2.5 — refactor backlog (외부 review 잔여, 본 PR 후속)

**적용 시점**: Phase 2 commit 후 단발 cycle (또는 Phase 3 동행).

| ID | 항목 | 위치 | 분류 | 비용 |
|---|---|---|---|---|
| **M4** | **knownSugars 단일 SSOT 테이블** — `deviceDefaults` (ModelProtocol.fs:356) ↔ export fingerprint (ModelProtocol.fs:1157-) ↔ ToolOperations helper default (`queueAddCylinder/Clamp` 등) 의 `["ADV";"RET"]` / `["CLP";"UNCLP"]` literal 3 곳 중복. → `knownSugars : (deviceCase * systemType * defaultApis * defaultOpposing) list` 단일 테이블 → apply/export/helper 가 lookup. CLAUDE.md "신규 함수 < 기존 재활용" 직격. | 3 파일 cross-module | M | mid |
| **M5** | **`Queries.resolveCallTargetSystemName` 신설** — Call → ApiCall.tryHead → ApiDefId → ApiDef → ParentSystem chain 을 `Queries.fs` 의 public helper 로 추출. 현재 `ModelProtocol.fs:1098-1110` (export emit) + `Queries.fs:250-259` (validateDeviceSystemTypeChange) + `Queries.fs:321-326` (callDeviceDurationMs) 3중복. | `Queries.fs` 확장 | M | small |
| **m1** | RelaxedShape (ModelProtocolTests.fs) ↔ `Helpers/ModelEquivalence.StoreShape` 정보 중복. → ModelEquivalence 에 *relax projection* helper 추가 후 RelaxedShape 삭제. | 1 파일 (테스트) | m | small |
| **m2** | `processOneArrow` / `processOneFlowArrow` (ModelProtocol.fs) 25줄 중복 (closure `resolveCallId`/`resolveWorkId` 차이). generic 파라미터 추출 시 가독성 저하 trade-off. | ModelProtocol.fs | m | mid |
| **m3** | `roundTrip helper` — 테스트 5건의 5줄 patten (parseApplyCommit → exportToJson → apply → captureShape × 2 → diff). | Helpers/ 신설 | m | small |
| **m4** | yamlNullTokens (`null`/`Null`/...) 와 ModelProtocolYaml `scalarToJsonLiteral` 의 null literal hardcode 이중화. | ModelProtocol.Yaml.fs | m | xs |
| **m5** | yaml12Bools ∪ yaml11OnlyBools ∪ yamlNullTokens 단일 `yamlReservedTokens` set 캐시. | ModelProtocol.Yaml.fs | m | xs |
| **m6** | `ModelEquivalence.allArrowCalls` O(N×M) — work 마다 전체 enumerate. N≤20 PoC 영향 0. helper 내 1회 캐싱. | Helpers/ | m | xs |
| **m7** | `arrowType` 테스트 key 의 `%A` 직렬화 → `formatArrowType` 재활용. | ModelProtocolTests.fs | m | xs |

**M3** (외부 review): ApiCalls=[] fallback 직접 회귀 테스트 — helper 정상 경로로 시뮬레이션 불가, logger forensic 으로 *간접* 보장. 별도 cycle 도 보류 권장.
...
- **Phase 2 commit (예정)**: **315 통과** (신규 26 — cycle1 3 + cycle2 18 + cycle3 5)
- Phase 2.5 또는 Phase 3 진입 시 *이 baseline 위에서 시작*
