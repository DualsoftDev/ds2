...
| 10 | top-level unknown 키 (Phase 7 §4.2 C-6) | `ERROR: top-level 키 '<key>' 인식 불가. 허용 = protocol / project / author / version / systems / patch / view / summary / level.` (`author` / `version` row 는 §2.1 top-level 키 enum 표에 신규 추가. `level` 은 §2.7 룰 #29 / Phase 7 §10.2 #31 — apply 가 parse 하는 top-level 키. 코드가 SSOT) |
