...
최상단 mapping 의 허용 키 = 정확히 다음 5개. unknown 키 발견 시 validate 에러 (`ERROR: top-level 키 '<key>' 인식 불가. 허용 = protocol / project / systems / patch / view.`). 본 enum 은 wire JSON 과 disk YAML 양쪽에 동일 적용 (schema 1:1 동형 — §1.5 / §8.3 참조).
...
`patch` 와 `view` 는 시점 분리 — `patch` 는 apply 입력 wire 에만, `view` 는 export 결과 wire 에만. 양쪽 동시 등장은 schema 위반 (mutation 결과 dump 자체가 미정의 의미).
...
`exportToJson` walk 진입 시 `let truncated = ref false` 선언. 다음 시점 중 1개 이상 발생 시 `truncated := true`:
- `path` 인자로 root entity 가 store 의 부분에 한정됨 (path 외부 entity skip)
- `depth` 인자로 자식 절단 발생 (실제 자식이 있는데 depth limit 으로 emit 안 됨)
- 50 entity budget overflow (describe_subtree 의 기존 상한 — `path` 또는 `depth` 명시 시에만 적용)

walk 종료 후:
- `truncated.Value` = `true` → `view: partial` emit
- `truncated.Value` = `false` → `view: full` emit (입력 `depth=999` 라도 실제 절단 0건이면 `full` 정확 emit — 의미 정확성)

**전체 export 의 무제한 정책**: `path` 미지정 + `depth` 미지정 → 50 entity budget 미적용 (round-trip 정합 필수). 결과는 `view: full`.
...
Phase 5 mutation 일소 + Phase 6 read GUID-free 정렬 후 **LLM 노출 풀세트 = 6종** (§3.3 todo-read-surface-guid-cleanup.md 참조). 주력 진입점:
