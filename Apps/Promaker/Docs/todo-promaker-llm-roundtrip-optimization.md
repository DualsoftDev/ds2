...
| R5 | snapshot token 비용 측정 script (heuristic + optional Anthropic count_tokens) — medium 사이즈 ≈ 925 token 확인 | `Apps/Promaker/Promaker/Tools/measure-snapshot-tokens/measure-snapshot-tokens.fsx` (신규) |
...
| R5b | snapshot token 실측 (Anthropic count_tokens) — R5 script 에 ANTHROPIC_API_KEY 설정 후 재실행 | Step 5 | 사용자 직접 측정 | heuristic 추정치 검증 |
...
| Promaker | `Apps/Promaker/Promaker/Tools/measure-snapshot-tokens/measure-snapshot-tokens.fsx` | **R5 신규** — 단발 F# script (heuristic + optional Anthropic count_tokens) |
