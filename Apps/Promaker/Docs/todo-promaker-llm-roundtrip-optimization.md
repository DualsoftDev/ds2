...
### 완료 (DONE) — Step 0 ~ Step 4, Step 6 부분
...
| ~~R2~~ | ~~Compaction keepalive~~ → **DONE**: rolling history (`MaxHistoryDataMessages=40`, ApiChatProvider 한정. CLI 는 CLI 자체 history 라 rolling 불가, 파일 변경 시 `ClearSession()` 으로 안전 영역 유지) | Step 6 | DONE | doc 의 "N=20 turn" 표현을 사용자 협의로 "max 40 message rolling" 으로 재정의 |
| ~~R3~~ | ~~Cache 적중률 모니터링~~ → **DONE** (`ApiChatProvider.LogUsage`) | Step 6 | DONE | INFO 한 줄: `input=N cached=M hit=p% output=K` + provider AdditionalCounts |
