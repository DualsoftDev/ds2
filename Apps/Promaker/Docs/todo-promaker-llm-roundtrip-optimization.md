...
> **v5.6** (2026-05-11, commits `0cdf88e` Tier A + `8d17c50` Tier B 반영). 1 RT 목표 구현 완료 + 회귀 방어 39 테스트 + 자동 측정 PoC (steady 91.3% hit ratio 입증, R10 follow-up 우선순위 격상). 남은 manual: 시나리오 6 외부 mtime UI smoke. **새 세션 진입 절차**: §새 세션 진입 시 권장 시작점 (line 386 부근) 부터 읽고 § Revision History v5.6 entry 확인. 변경 이력 전체는 문서 끝 §Revision History.
...
### 완료 (DONE) — Step 0 ~ Step 4, Step 6, Step 8 부분
...
| R10 (격상) | snapshot block 별도 cache breakpoint **+ `_history` 안에도 multi-content user (snapshot 포함) 누적** (Step 4 deferred) | Step 4 | 중간 | **재현 가능 PoC** — `Apps/Promaker/Docs/Poc/e2e-cache-hit.fsx` 의 옵션 A 패턴 (history multi-content + 마지막 user 의 snapshot 만 cache_control) 에서 **steady-state 91.3% hit ratio 측정** (2026-05-11, claude-haiku-4-5, medium snapshot, 10 turn — 캡처: `Docs/Poc/e2e-cache-hit-run.md`). 재현: `dotnet fsi Apps/Promaker/Docs/Poc/e2e-cache-hit.fsx 10 medium`. hit ratio 정의 = `cache_read / (input + cache_creation + cache_read)`. 반면 현 `ApiChatProvider` 는 history 에 plain user 만 누적 → snapshot 위치가 매 turn 변동 → prompt cache 의 정확 prefix-match 깨져 snapshot 의 ~1K token 이 매 turn cache miss. **우선순위 낮음 → 우선순위 중간 격상**: snapshot 자체의 cache 효과 회복으로 turn-by-turn cost 추가 절감 가능 (의미 있는 ROI) |

### 새 세션 진입 시 권장 시작점 (2026-05-11 / commit `8d17c50` 기준)

**현재 상태 한 줄 요약**: 1 RT 목표는 commit `7be3f1f` ~ `e197ac3` 에서 구현 완료. Tier A 회귀 방어 39 테스트 (commit `0cdf88e`) + Tier B 자동 측정 PoC 가 steady-state 91.3% hit ratio 입증 (commit `8d17c50`). 남은 일은 (a) 시나리오 6 manual UI smoke + (b) follow-up PR 후보 4종 (R8 ~ R10 + Mj 류) 결정.

**진입 절차**:
1. **최근 commit 3 개 git log** (`7be3f1f` snapshot delta-only / `0cdf88e` Tier A / `8d17c50` Tier B) 확인하여 본 PR 의 범위 파악
2. **§완료 (DONE) 표** 정독 — Step 0 / 1 / 2 / 3 / 4 / 6 + Tier A / B 모두 완료
3. **§남은 일 표** 정독 — R1 (자동화 완료분) + R1 (수동 잔여 = 시나리오 6) + R5b (자동화 완료) + R8 / R9 / R10 (격상)
4. **§E2E 시나리오** 의 1~5 는 Tier A 의 39 테스트로 회귀 방어, 3 은 추가로 `Docs/Poc/e2e-cache-hit.fsx` 로 재현 가능
5. **사용자 결정 받을 항목**:
   - 시나리오 6 manual UI smoke 진행 여부
   - follow-up PR 우선순위 (R10 효과 입증치 91.3% 가장 큼 → 최우선 후보)
...
dotnet test Apps/Promaker/Promaker.Tests/Promaker.Tests.csproj --filter "FullyQualifiedName~ApiTurnContentBuilderTests"             # 16
```

**Follow-up PR 후보 우선순위**:
- **R10 (격상)** ★ snapshot block 별도 cache breakpoint + `_history` 안에도 multi-content user 누적. PoC 91.3% 입증치 → ApiChatProvider 변경 ROI 명확. R10 이 단독 PR.
- **R8** `DsSystem.SystemType` SSOT (option<string> → enum/DU) — 도메인 영역, 별도 PR
- **Mj4** cache 부착 DRY (`WithOptionalCache`) — `ApiChatProvider.cs:159-160` + `ApiTurnContentBuilder.cs:60-63` 의 3-line 패턴 흡수
...
| Promaker | `Apps/Promaker/Promaker/LlmAgent/Api/ApiTurnContentBuilder.cs` | **Tier A 신규** — sticky/multi-content 빌드 helper (`UpdateStickySnapshot` / `BuildPromptForHistory` / `BuildMultiContents`) |
| Tests | `Apps/Promaker/Promaker.Tests/ApiTurnContentBuilderTests.cs` | **Tier A 신규** — helper 단위 테스트 16건 (sticky 5 / prompt 3 / multi 8) |
