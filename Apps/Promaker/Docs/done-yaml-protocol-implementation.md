...
| Phase 6 (read surface GUID-free 정렬) | ⏳ **진행 중** — Phase 5 후 잔존 read 6종 중 4종이 GUID 입출력 → done §6 #5 invariant 충돌 청산. `list_projects` / `list_systems` / `describe_system` / `describe_subtree` 4종을 `export_model_doc(path?, depth?)` 으로 흡수, `find_by_name` 출력 = `[{kind, path}]` 목록, `validate_model` 의 'global' literal + GUID scope 폐기. 풀세트 10 → **6종** (-40%). SSOT = `Apps/Promaker/Docs/todo-read-surface-guid-cleanup.md` (closure 5건 + 부속 1건 v4 결정 완료). 2-단계 commit: #1 SSOT-only (`yaml-protocol-v0.md` §2.1 top-level 키 enum +`view` / §2.5.1 path resolver 절 / §2.7 룰 #7,#8 / §2.8 partial export view-only spec / §4 도구 시그니처 / §1.7 결정 표 7행 추가) → #2 코드/prompt/test 일괄 |
