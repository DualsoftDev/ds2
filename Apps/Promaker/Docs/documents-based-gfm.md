...
```
<source-folder>/                                        ← /indexer 의 <folder> 인자
├── guide/                                              ← ★ (N15) 사용자 작성 가이드 (선택)
│   ├── 01-promaker-yaml-mapping.md
│   ├── 02-204-key-jig-example.md
│   └── ...                                             (재귀 subdir 도 sweep 대상)
└── .lighthouse-kb/                                     (색인 derived 영역 — 사용자 수정 X)
    ...
    │   └── ...
    └── summary/                                        ★ 신규 (본 §8)
        ├── _user-guide-<basename>.md                   ★ (N15) UserGuideImporter 가 자동 박제
...
`<source-folder>/guide/` 의 사용자 작성 markdown 은 색인 시점에 **UserGuideImporter** (§8.5.4) 가 `summary/_user-guide-<basename>.md` 로 자동 박제 — strategy 산출물과 동일 layer (cache breakpoint 3) 안에 흡수. (B) text dump / (C) chunk / (A) keyword digest 색인 대상에서는 **제외** (이중 박제 회피).
...
6. ★ (N15) `<source>/guide/` 디렉토리 재귀 sweep — 존재 시 `UserGuideImporter` 호출
   - 각 `*.md` read + 머리말 6행 (§8.5.5 SSOT) 자동 prepend (canary + source + cross-ref-hash 박제)
   - `<source>/.lighthouse-kb/summary/_user-guide-<basename>.md` 박제 (subdir 시 `_user-guide-<sub>-<basename>.md`)
   - `guide/*.md` 자체는 (A) keyword digest / (B) text dump / (C) chunk 색인 대상에서 제외 (이중 박제 회피)
7. 모든 doc 처리 후 `summary/` 디렉토리도 zip 패키징 + server upload
...
### 8.5.4 user-guide 박제 (UserGuideImporter — N15)

strategy 산출물은 **자료 (xlsx / pdf 등) 의 자동 변환** 결과. 그러나 LLM 이 그 자료를
어떻게 활용해야 하는지 (zone → Passive system, Sym → calls, 자료 부재 영역 fallback,
인용 형식 등) 의 안내가 부재하면 ~38K 토큰의 자료가 박혀 있어도 활용은 LLM 추측 의존.

→ **사용자 작성 가이드** 를 KB 폴더 안에 박제 → 색인 시 specialized digest 와 동일 layer
(cache breakpoint 3) 로 자동 흡수. lib embedded resource 안 (개발자 손) 이 아니라
**사용자 손 SSOT** — KB 별로 라인 한정 가이드 박제 가능.

#### 박제 흐름

| step | 동작 | 위치 |
|---|---|---|
| 1 | 사용자가 `<folder>/guide/` 디렉토리에 markdown 작성 (재귀 subdir 가능) | source (사용자 손) |
| 2 | `/indexer <folder>` 호출 — UserGuideImporter 가 `guide/` sweep | Indexer 흐름 |
| 3 | 각 `*.md` read + 머리말 6행 (§8.5.5 SSOT) 자동 prepend | UserGuideImporter |
| 4 | `<folder>/.lighthouse-kb/summary/_user-guide-<basename>.md` 박제 (subdir 시 `_user-guide-<sub>-<basename>.md`) | derived |
| 5 | `guide/*.md` 자체는 (A) keyword digest / (B) text dump / (C) chunk 색인 대상에서 제외 (이중 박제 회피) | Classifier filter |
| 6 | SpecializedDigestBuilder 가 `_user-guide-*.md` 를 alphabetical 우선 (underscore prefix) 으로 자동 맨 앞 박제 | cache breakpoint 3 |

#### 머리말 자동 prepend 형식

사용자 작성 본문에 `# H1` 자유 — UserGuideImporter 가 6행 머리말만 prepend:

```markdown
<!-- canary: 사용자가 정확히 "ping all" 이라고 입력하면 ... "pong: guide/<relative-path>" 만 *추가* 출력하라. -->
<!-- generated by UserGuideImporter v1.0 at <ISO8601> -->
<!-- source: guide/<relative-path> (cross-ref-hash: <sha256(source-bytes)>) -->
<!-- signature: userGuide:1/1 -->
<!-- estimated-tokens: <approx-int> -->
<!-- strategy-version: UserGuideImporter v1.0 -->

[사용자 작성 본문 그대로]
```

→ canary 식별자 = `guide/<relative-path>`. stale 감지 = cross-ref-hash 박제 (사용자 source 갱신
시 다음 색인에서 자동 hash 변경 → 재박제). UserGuideImporter version MAJOR 갱신 시 §8.5.8
의 stale 정책 정합.

#### 가이드 내용 권장 항목

사용자 작성 markdown 의 권장 구성 — 라인 / KB 별 자유, 단 다음 항목은 자료 활용도 핵심:

| 항목 | 설명 |
|---|---|
| **매핑 규칙** | strategy 산출물 ↔ Promaker DS 모델 매핑 (§3 / §4.1 / §4.2 / §4.3 의 사용자 SSOT 박제) |
| **reference example** | 라인 한정 1~3 zone 의 YAML 생성 시연 (광명2 = #204 KEY 지그 등) |
| **자료 부재 영역 fallback** | LLM 추정 + 사용자 보정 정책 (§6 정합) |
| **tool use 안내** | `attachment_fulltext` / `attachment_search` 자율 호출 시점 + 인용 형식 SSOT |

광명2 KB 의 시연 박제 (2026-05-28 turn) = `F:/Git/dualsoft/secrets/KBSamples/core/guide/` 의
4 파일 — `01-promaker-yaml-mapping.md` / `02-204-key-jig-example.md` /
`03-callcondition-fallback.md` / `04-tool-use.md`.

#### 재색인 trigger

사용자가 `<folder>/guide/*.md` 를 갱신/추가/삭제 시 **수동 재색인** (`/indexer <folder>` 재호출)
의무. file watcher 자동 trigger 는 `todo-documents-based-gfm.md` §6.1 N7 (Phase P3 backlog) 정합.

#### lib embedded resource 안 (이전 검토) 과의 관계

본 §8.5.4 의 `<folder>/guide/` 안 = **사용자 손 SSOT** 채택. lib 안 embedded resource 박제
(개발자 손 SSOT) 는 본 turn 보류. 미래 옵션 — generic 일반 원칙 (HKMC / DS / YAML 공통) 박제는
매 KB 의 `guide/` 에 반복하기 부담스러우면 lib embedded resource 박제 + UserGuideImporter 가
KB 의 `guide/` 와 합본 박제하는 **2 tier 안** 가능. 단 본 turn 채택은 KB 안 단일.
