...
| (본 commit) | s6-r20-review | **외부 --inspect-diff 3 reviewer (Generalist + Logic + Design) 종합 review 처리 — Critical 1 + Major 6 + Minor 2 즉시 적용 (총 9건)**. 변경 = 6 파일 + 신규 1 파일.
...
| (본 commit) | s6-r20 | **Phase 2 task D-iii + D-iv + E-i 본격 코드 진입 + 5MB SSOT 정정 (m-r19-1) + 자가 검열 즉시 적용 4건**. 변경 = lib 4 + service 1 + Promaker 4 + cli 2 + Tests 4 = 15 파일, +801/-26 line.
...
| (본 commit) | s6-r22 | **Phase 2 잔여 묶음 — mn7 + mn1 + task 5 (table cell) + mn3 (schema NOT NULL + IndexerVersion 1.3.0 bump) + task 4 (oversize image fact) + 자가 검열 C1 즉시 적용**. 변경 = lib 4 + service 1 + Tests 6 (11 파일, +248/-100 line). 회귀 0 + 누적 **513 Fact** (s6-r21 509 → +4).
...
**처리 완료 (s6-r22, 본 turn)**: Phase 2 잔여 묶음 5건 + 자가 검열 C1 즉시 적용.
...
**다음 권장 (s6-r22 이후)** — Phase 2 잔여 (실 image fixture Fact 묶음 / parent §3.15 sweep) + Phase S7 진입:
...
- **(s6-r22 자가 검열 미적용 backlog — 별 turn)**:
  ...
  - **m1 (SamplePng cross-project sha256 expected 상수 fact)** — 두 sibling module 의 byte literal drift 시 명시 fail. 우선순위 낮음 (`computeSha256` e2e fact 가 간접 detection).
  - **m2 (extractImagesAtRefLocator ↔ extractImagesFromOpenXmlPart helper 통합)** — header/footer 별 분기 와 paragraph/table 분기가 동일 패턴. refactor 여지 — 별 turn 박제.
...
- **(s6-r20 Fact 잔여)** — ~~실 image fixture 기반 attachment_read 의 includeImages 분기~~ → **s6-r21 docx fixture 4 Fact 박제 완료**, ~~size 정책 강등 분기 Fact (단일 image > MaxSingleImageBytes)~~ → **s6-r22 task 4 oversize Fact 박제 완료**. **잔여**: PdfExtractor 산물 기반 fixture (현재 OoxmlExtractor 산물 docx 기반만) / cost gate hard cap 시 captionGen mock 으로 SkippedCaption 반환 Fact (BuildCaptionGen 의 mock 어려움 — Promaker.Tests 의 HttpClient handler stub 별 turn) / LIGHTHOUSE_VLM_API_KEY env var fallback 의 cli 측 e2e Fact.
