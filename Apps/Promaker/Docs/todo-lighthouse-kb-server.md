...
| (본 commit) | s6-r24 | **3 작업 묶음 — Phase 2 cosmetic 정리 + MJ1 multi-instance race 본질 해결 + parent §3.15 sweep**. 변경 = lib 1 + Promaker 2 + Tests 4 (수정 4 + 신규 2) + doc 2 = 9 파일, +342/-56 line. 회귀 0 + 누적 **529 Fact** (s6-r23 522 → +7).
...
- **(s6-r24 자가 검열 미적용 backlog — 별 turn)**:
  - **M1 (LlmConfig lock file 영구 잔존)** — `<ConfigPath>.lock` 가 production user-visible artifact. 다음 호출이 reuse 라 기능 결함 0이나 app uninstall / 사용자 정리 시 cleanup 의무. AppData cleanup script 도입 별 turn.
  - **M2 (parent §3.15 sweep 잔여 5개소)** — 본 turn scope §3.15.1~5 한정. 잔여 = line 7 historical reference (변경 불필요) / line 200 §3.0 표 ("cached lazy" 단어) / line 581 mr4 (취소선 정합) / line 790 Phase 2 헤더 / line 818 Phase 3 검토. 사용자 별 turn 의무.
  - **m2 (ModifyWithLock backoff cap 재검토)** — max 3.1s 가 CI 부하 시 4×50 trips flaky 위험. 부하 측정 후 cap ↑ 또는 retry 횟수 ↑ 검토.
  - **m3 (TestConfigPathOverride process-static 정합)** — 다른 fact assembly 에서 LlmConfig 사용 시 collection 직렬화 누락 위험. internal 가시성 정합 박제 의무.
  - **m4 (CliVlmTests 세 번째 fact 의 외부 network 의존)** — offline CI 시 SkippedCaption 가능. SSOT (`"no caption gen"` 미반환) 정합이나 fact name 정정 의무 — "FailedCaption 또는 다른 SkippedCaption" 으로 통합.
- **(s6-r20-review 미적용 backlog — 잔여)**:
  - **M9 (6장+ 전량 caption_only 강등) — 반론 정합**: D-2-3 SSOT 박제 (응답 일관성 우선 = partial result 차단 의도). per-image granularity 변경 시 D-2-3 정정 의무.
  - **자가 검열 s6-r20 잔여 (M1/m2)**: M1 `VisionCostGate.Consume` race — single-thread Indexer 가정, Phase 4 perf 진입 시 `Interlocked.Add` 의무 / m2 `StringBuilder marker` 패턴 helper 추출 — borderline.
  - **Minor 9건 (s6-r20-review)**: m2 StringBuilder marker / m3 `Status` getter side-effect / m5 MCP schema default 마킹 / m6 `[<Literal>]` cross-assembly drift / m7 `GetVlmApiKey` OS 가드 / m8 catch Exception 좁힘 / m9 `VlmResetCostGate` semantic / m11 "0 입력" UI 안내.
