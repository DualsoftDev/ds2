...
**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 → s6-r6 208 (s6-r7~r11 회귀 0)
- IntegrationTests: S5a 1 → S5e 7 → s5f 8 → s6 12 → s6-r1 19 → s6-r2 20 → s6-r5 22 → s6-r7 24 → s6-r10 25 (s6-r8/r9/r11 회귀 0)
- service Tests: 111 → s6-r6 115 → s6-r10 116 (s6-r7/r8/r9/r11 회귀 0)
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 (s6-r9/r10 회귀 0)
- **누적 472 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 123). s6-r11 = +14 Fact (Phase 2 task B `ImageStore.fs` + 자가 검열 M1/m3-b/c 즉시 적용).
...
**처리 완료 (s6-r11, 본 turn)**: Phase 2 task B 본격 진입 — `ImageStore.fs` 신설. (i) **신규 module** = sha256 + blob 저장 + ImageCache/ImageReferences upsert (parent §3.15.5 MR1/MR2 정합, per-collection dedup). 9 함수 surface + literal SSOT. (ii) **14 Fact** = 결정성 / SSOT path / idempotent (saveBlob/upsert/addRef) / cross-document 공유 / FK 위반 차단 / ON DELETE CASCADE + 자가 검열 즉시 적용 3건 (M1 cross-drift Theory + m3-b/c). (iii) 자가 검열 Minor 5건 (m1 transaction / m2 record vs tuple / m3-a/d 커버리지 / m5 MimeType NULL — 모두 backlog 또는 N/A). 누적 **472 Fact**.

**다음 권장 (s6-r11 이후)** — **즉시 진입 = P3 Phase 2 task C (PdfExtractor / OoxmlExtractor 이미지 추출 + ImageStore 호출)** 또는 dist 본격 실행 (N1) 또는 N5/N6/N7 위치 미정:
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task C) ← 다음 진입 후보**: PdfExtractor (PdfPig) / OoxmlExtractor (DocumentFormat.OpenXml) 가 image raw 추출 + `ImageStore.computeSha256` + `ImageStore.saveBlob` + `ImageStore.upsertImageCache` + `ImageStore.addImageReference` dispatch. PdfPig DCT/JPX/JBIG2 decode 추가 NuGet 필요 시 진입 시점에 확인 (현 `PdfPig` 기본 패키지로 inline image extract 어디까지 가능한지 검증). 규모 ~200 line + Fact 5-7건 (extractor 별).
