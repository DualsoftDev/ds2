...
| 단계 | 범위 | 진입 시점 |
|---|---|---|
| **D-S7-3a (s6-r29)** | schema 확장 + migration + caller 임시 path | 본 turn 완료 |
| **D-S7-3b** | Holder 재설계 (multi-instance dict + SSE 다중) + LlmChatViewModel 의 N 개 session 발급 + MCP config multi-server entry | 별 turn |
| **D-S7-3c** | UI (ApplicationSettingsDialog DataGrid + KbManagerDialog TabControl) + KbCollection ServiceId 재매핑 dialog | 별 turn |
...
- **(P4 Phase S7)** mTLS / SSE / multi-service routing — D-S7-1~5 사전 박제 완료, 사용자 우선순위 결정 시 본격 진입. Phase 2 와 병렬 진입 가능.
  ...
  - ~~D-S7-3a (multi-service routing schema + migration)~~ → **처리 완료 (s6-r29 본 turn)** — §3.16 SSOT.
  - **D-S7-3b (별 turn)** — Holder multi-instance dict + SSE 다중 + LlmChatViewModel N session + MCP multi-server config. Holder closure stale serviceId race (자가 검열 Minor-3) 자연 해소.
  - **D-S7-3c (별 turn)** — UI (ApplicationSettingsDialog DataGrid + KbManagerDialog TabControl). `ClearActiveService` 의 KbCollections.ServiceId dangling reference (자가 검열 Minor-2) 반드시 해소 의무 — 옵션 (a) 재입력 시 dialog 로 collection 재매핑, (b) ClearActiveService 시 KbCollections 동시 purge, (c) 빈 ServiceId 의 collection "orphan" 표시.
