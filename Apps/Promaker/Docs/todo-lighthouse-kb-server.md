...
| **D-S7-3b (s6-r30)** | Holder 재설계 (multi-instance dict + SSE 다중) + LlmChatViewModel 의 N 개 session 발급 + MCP config multi-server entry | ✓ 완료 (본 turn, §3.16.7) |
| **D-S7-3c** | UI (ApplicationSettingsDialog DataGrid + KbManagerDialog TabControl) + KbCollection ServiceId 재매핑 dialog + displayName uniqueness 검증 + orphan 정리 dialog | 별 turn |
...
- **(P4 Phase S7)** mTLS / SSE / multi-service routing — D-S7-1~5 사전 박제 완료, 사용자 우선순위 결정 시 본격 진입. Phase 2 와 병렬 진입 가능.
  ...
  - ~~D-S7-3b (Holder multi-instance + N session + MCP multi-server)~~ → **처리 완료 (s6-r30 본 turn)** — §3.16.7 SSOT. D-S7-3a 의 Holder closure stale serviceId race (자가 검열 Minor-3) 자연 해소.
  - **D-S7-3c (별 turn)** — UI (ApplicationSettingsDialog DataGrid + KbManagerDialog TabControl). `ClearActiveService` 의 KbCollections.ServiceId dangling reference (D-S7-3a Minor-2) + s6-r30 review m-4 (test coverage 누락 — MCP 충돌 fact / SSE ServiceId tagging e2e / 부분 활성화 fact) + s6-r30 review Major-3 의 orphan dialog (KbManagerDialog 에 "정리" 버튼) + displayName uniqueness 검증 (ApplicationSettingsDialog DataGrid validation) — 옵션 (a) 재입력 시 dialog 로 collection 재매핑, (b) ClearActiveService 시 KbCollections 동시 purge, (c) 빈 ServiceId 의 collection "orphan" 표시.
