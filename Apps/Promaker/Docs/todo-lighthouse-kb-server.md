...
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 (s6-r9/r10 회귀 0)
- **누적 477 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 128). s6-r12 = +5 Fact (Phase 2 task C1 `ingestImagesIntoStore` dispatch helper).
...
**다음 권장 (s6-r12 이후)** — Phase 2 본격 image 추출 (C2/C3) 진입 가능:
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task C2) ← 다음 진입 후보**: `PdfExtractor.fs` 가 PdfPig `page.GetImages()` + `IPdfImage.TryGetPng(out bytes)` 로 image bytes 추출 후 `ExtractedDocument.Images` 박제. 화이트리스트 외 image (JPX / JBIG2 decode 미지원) 는 skip 또는 PNG re-encode. **본격 진입 전 결론 의무**: (a) M2 transaction partial failure 정책 (per-image try/catch + log skip vs ingestFile outer txn 도입) / (b) m6 empty Bytes 가드 박제 위치. 규모 ~80 line + Fact 4-6건.
