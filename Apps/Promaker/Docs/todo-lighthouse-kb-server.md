...
**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 → s6-r6 208 → s6-r20 216 (s6-r7~r19 회귀 0, s6-r20 +8 VLM/cost gate)
- IntegrationTests: S5a 1 → S5e 7 → s5f 8 → s6 12 → s6-r1 19 → s6-r2 20 → s6-r5 22 → s6-r7 24 → s6-r10 25 (s6-r8/r9/r11/r12/r20 회귀 0)
- service Tests: 111 → s6-r6 115 → s6-r10 116 → s6-r20 118 (s6-r7/r8/r9/r11/r12 회귀 0, s6-r20 +2 attachment_read mode)
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 → s6-r14 131 → s6-r15 135 → s6-r16 136 → s6-r19 143 → s6-r20 145 → s6-r20-review 144 (M7 dead Fact 제거 -1)
- **누적 503 Fact** (Promaker 216 + service 118 + IntegrationTests 25 + lib 144). s6-r20-review = -1 (M7 dead code 제거).
...
- **(s6-r20-review 미적용 backlog)**: 외부 reviewer 의 Major 잔여 4건 + Minor 9건. 별 turn 박제 의무:
  - **M3 (Searcher mime 빈 row fallback)** — `ImageCache.MimeType` NULL row 시 빈 mime 으로 `ImageContentBlock` 박제 → MCP spec 위반 가능. legacy / 외부 zip import 시 risk. caller (`AttachmentTools.attachment_read`) 가 skip 또는 확장자/시그니처 fallback 추론 결정 의무.
  - **M4 (multi-instance LlmConfig.Save() race)** — `_saveLock` 이 instance lock 이라 KbManager + Settings 동시 사용 시 last-write-wins. process-wide static lock 또는 UI flow 제약 (indexer 진입 시 settings 닫힘). UX 결정 의무.
  - **M5 (VlmProvider silent downgrade)** — `VlmProviderBox` ComboBox 가 `anthropic/none` 만이라 JSON 의 `openai` 등 미지원 값이 silent `none` 으로 강등. 향후 multi-provider 확장 시 데이터 손실. unknown provider 보존 가드 박제 의무.
  - **M9 (6장+ 시 전량 caption_only 강등) — 반론 가능**: D-2-3 SSOT 박제 정합 (응답 일관성 우선 = partial result 차단 의도). per-image granularity (5장 동봉 + 잉여 caption_only) 변경 시 D-2-3 박제 정정 의무.
