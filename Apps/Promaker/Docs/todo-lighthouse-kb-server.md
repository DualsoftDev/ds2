...
**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 → s6-r6 208 (s6-r7~r12 회귀 0)
- IntegrationTests: S5a 1 → S5e 7 → s5f 8 → s6 12 → s6-r1 19 → s6-r2 20 → s6-r5 22 → s6-r7 24 → s6-r10 25 (s6-r8/r9/r11/r12 회귀 0)
- service Tests: 111 → s6-r6 115 → s6-r10 116 (s6-r7/r8/r9/r11/r12 회귀 0)
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 → s6-r14 131 → s6-r15 135 → s6-r16 136 → s6-r19 143 (s6-r9/r10/r17/r18 회귀 0)
- **누적 492 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 143). s6-r19 = +7 Fact (Phase 2 task D partial — ImageStore caption 4 + Indexer caption 3).
...
**다음 권장 (s6-r19 partial 이후)** — Phase 2 잔여 진입 (D-iii/D-iv/E-i 묶음 / s6-r16 backlog):
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task D + E 본격 진입 잔여 — s6-r20 후보)**: 7건 결정 박제 (§0 **D-2-1~7** SSOT). s6-r19 partial 완료 marker:
  - ✓ **(D-i) CaptionGenerator.fs 신설 완료** (s6-r19, 176 line — HttpClient + System.Text.Json + noop FSharpFunc)
  - ✓ **(D-ii) Indexer.ingestImagesIntoStore eager caption 채움 완료** (s6-r19, +44/-7 — captionGen 인자 + getCaption pre-check + Captioned/Skipped/Failed 분기)
  - ✓ **(lib 보강) ImageStore.getCaption + updateCaption + IndexerVersion 1.1.0→1.2.0** (s6-r19)
  - 진입 잔여 작업 묶음:
  - (D-iii) `LlmConfig.VlmProvider` 박제 (~80 line) — C# field 신설 `VlmProvider` (= "anthropic" default) + `VlmModel` (= "claude-sonnet-4-6" default) + DPAPI EncryptedKeys 재활용 + `ApplicationSettingsDialog` 노출 (ComboBox + tooltip "vision 사용 모델") + `lighthouse-cli` 의 `LIGHTHOUSE_VLM_API_KEY` env var fallback (Phase S6 P5 정합) + AttachmentIngestService.cs / Cli/Program.fs / Cli/Packager.fs 의 noop → 실 captionGen (CaptionGenerator.callAnthropic 부분 적용) 치환 [D-2-1 / 2-5 정합]
  - (D-iv) `Solutions/Tools/Ds2.LightHouseService/AttachmentTools.fs:222` 의 `attachment_read` 시그니처 **확장** (~80 line) — `(fileId, ref, includeImages?, caption_only?) : ContentBlock[]` (signature breaking, 기존 caller grep 0 가정) + caption_only mode (text block 만 + `CaptionText` 텍스트) / includeImages mode (text block + image block 분리 base64 — MCP 표준) + size 정책 가드 (단일 ≤5MB / 응답 ≤5장 / 초과 자동 caption_only 강등 + `oversize_image_count` text block 박제) + ImageStore.getImageCache 확장 (caption 필드 포함 반환) [D-2-3 / 2-7 정합]
  - (E-i) `LlmConfig.VisionCostGate` 박제 (~50 line) — `DailyVisionTokenCap` (default 10K) + `LastResetUtc` + `TokensUsedToday` (DPAPI 평문 — 시각화 가능) + soft warning 80% 도달 시 `KbManagerDialog` chip 안내 + hard cutoff 시 caption 생성 skip (NULL 유지, 다음 day 재시도) + 색인 진입 전 사용자 confirm dialog (예상 token = image 수 × 평균 300 token 산정) [MR4 정합]
  - (Fact 잔여) ~120 line — CaptionGenerator mock (HttpClient handler stub) Fact 3건 / attachment_read mode 별 분기 4건 / size 정책 강등 3건 / cost gate trigger 3건 / LlmConfig DPAPI round-trip 2건. s6-r19 의 lib infra Fact 7건 (ImageStore caption 4 + Indexer caption 3) = 별 layer 라 본 잔여 외.
  - **s6-r19 backlog 박제 정합 의무** (자가 검열 m-r19-1): D-iv 진입 시 5MB 한도 SSOT 정정 — CaptionGenerator.callAnthropic 의 사전 검증 기준 (원본 vs base64) 정합 + D-2-3 박제 표현 보강 (현재 "단일 image ≤5MB" 의 base 정정)
  - **규모 합산 잔여 추정 ~310 line** (LlmConfig + Settings UI 80 + AttachmentTools 확장 80 + cost gate 50 + Fact 120). 영역 2개 동시 변경 (service / Promaker — lib 변경 0). 자가 검열 trigger 충족 예상 (시그니처 변경 1+ / 신규 함수 2+ / 신규 control flow / public API 갱신)
- **(s6-r16 backlog → s6-r20 후보)**: image-only paragraph 박제 분기 분리 (산업 매뉴얼 docx caption 시나리오) / table cell 단위 매핑 / header/footer/comments Drawing 커버 — 모두 OoxmlExtractor 강화 별 task. **D+E 묶음 turn (s6-r19) 종료 후 별 turn 진입 권장** (영역 분리되어 있어 자가 검열 분리 합리).
