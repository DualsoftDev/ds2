...
- [x] **D-S7-2b (Promaker client subscribe, s6-r28)** — `LightHouseClient.OpenEventsStreamAsync` + `ServerEventDto` + `LightHouseClientHolder.EventReceived` + `StartSseLoopLocked` 무한 reconnect (5s backoff, 좁힌 catch) + `KbManagerDialog.OnSseEventReceived` (Dispatcher.BeginInvoke + IsLoaded 가드 + RefreshAsync). Promaker.Tests +5 Fact. **D-S7-2c 잔여 (별 turn)**: Phase 2 vision caption 진행률 client → server upload (caption-progress event 추가) + reconnect exponential backoff + magic string SSOT.
...
- **(P4 Phase S7)** mTLS / SSE / multi-service routing — D-S7-1~5 사전 박제 완료, 사용자 우선순위 결정 시 본격 진입. Phase 2 와 병렬 진입 가능.
  ...
  - **D-S7-2c (별 turn)** — caption-progress publish (Phase 2 vision indexing) / reconnect exponential backoff / magic string SSOT (`ServerEventNames`) / `using var resp` 의도 docstring.
