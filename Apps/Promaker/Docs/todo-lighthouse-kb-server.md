...
| (대기) | s5e-r0 | 본격 IntegrationTests round-trip (MA22) 박제 해제 — Program.configureApp export + ServiceFixture (in-memory cert + Kestrel localhost:0) + 6 e2e Fact |

**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203
- IntegrationTests: S5a 1 (scaffold sentinel) → S5e 7 (sentinel 1 + e2e 6)
- service Tests: 111 (S1 24 + S2 41 + S3 27 + S4 19, s5e-r0 회귀 0)
- lib Tests: 100 (s5e-r0 회귀 0)
- **누적 421 Fact** (Promaker 203 + service 111 + IntegrationTests 7 + lib 100)
...
### 7.4 다음 세션 진입 후 권장 작업 (우선순위) — s5e-r0 transfer 시점 갱신

**즉시 진입 가능 — Phase S5 100% 종결 + dist 진입 준비 (paired-release drift detector) + 본격 e2e (MA22) 모두 완료**. 다음 세션은 아래 N1~N5 중 사용자 의사 확인 후 진입.

**N1. dist 본격 실행** (가장 자연스러운 다음 단계) — `/dist` skill 호출. 본 turn 의 모든 변경 (paired-release drift detector + IntegrationTests 7 Fact + 자가 검열 통과) 이 dist 직전 안전망 제공. `/dist` 가 Step 3 (빌드) → Step 3.5 (paired-release drift) → Step 4 (pull) → ... → Step 14 (push) 워크플로 실행. 단 P1 (사용자 UI 검증) 후 진입 권장.

**N2. s5e-r0 잔여 follow-up** (IntegrationTests 본격 e2e 확장):
- (s5e-M2) Fact 6 finally `.Result.Dispose()` → `let!` + 명시 dispose refactor
- (s5e-m5) `buildMinimalZip` 의 `Indexer.ingest` 결과 `Ingested` 분기 검증 강화
- (s5e-I) HTTPS-only 검증 Fact (`http://` scheme → connection refused) 추가
- 추가 fact (L3 자동 회복 / IndexerVersion gate 415 / zip bomb 거부 / IPv4 vs IPv6 localhost / multipart 누락 등) — 별 PR
- 규모 ~150-300 line + 5-10 Fact

**N3. parent r11 보류 confirm** (`todo-lighthouse-kb-index.md` r11 의 사용자 confirm 항목):
- 본 todo 파일 git mv — `Apps/Promaker/Docs/` → `Solutions/Core/Ds2.LightHouse/doc/` (§6.14)
- `Apps/Promaker/Directory.Packages.props` 에 OpenXml 3.5.1 명시 (MSB3277 충돌 해소)
- r10 자가 검열 잔여 (M3 docstring + m2/m3/m6/m7 등) — Phase 2 이연

**N4. Phase S6** (CLI 도구 옵션, 회사 IT 운영용) — `Ds2.LightHouse.Cli` 무인 batch 색인 + upload. §4.2 Phase S6 / §4.3 미확정 표 참조.

**N5. Phase 2** (이미지 인프라 — OCR / embedding / page render) — parent `todo-lighthouse-kb-index.md` §3.15.
