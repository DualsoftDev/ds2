...
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 → s6-r14 131 → s6-r15 135 (s6-r9/r10 회귀 0)
- **누적 484 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 135). s6-r15 = +4 Fact (Phase 2 task C4 ChunkId 매핑 + ImageCount post-update).
...
**처리 완료 (s6-r15)**: Phase 2 task **C4** — ChunkId 매핑 (Q1 옵션 1 = 첫 chunk MIN(Id)) + `Chunks.ImageCount` post-update (Q3 옵션 X). `SqliteStore.lookupChunkIdsByDocument` + `updateChunkImageCounts` 신설. `ingestImagesIntoStore` 시그니처 breaking (refToChunkId 인자 추가, 외부 caller 0). lib Tests 131→135 (+4 신규 C4 fact + 1 강화). 자가 검열 Critical 0 / Major 0 / Minor 3 (1건 즉시 적용). **Q2 OoxmlExtractor paragraph 매핑 강화** = 별 turn 의무 (현재 docx image ChunkId 항상 None).

**다음 권장 (s6-r15 이후)** — Phase 2 잔여 진입 (C4-Q2 / D / E):
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task C4-Q2) ← 다음 진입 후보**: `OoxmlExtractor` paragraph 매핑 — `Body.Descendants<Drawing>()` iter + relationship id → ImagePart 매핑. image 의 RefLocator 를 `"body"` → `"p=%d"` (paragraph ordinal) 로 강화. PdfExtractor scheme 통일 + ChunkId 매핑 활성화 (현재 docx image ChunkId 항상 None 결함 해소). 규모 ~30 line + Fact 2-3건.
