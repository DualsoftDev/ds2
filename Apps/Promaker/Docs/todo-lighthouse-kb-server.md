...
- **모드**: **Phase S6 scaffold 완료 + s5e follow-up 3건 + N1~N5 묶음 — commit 대기** (s5f-r0). `lighthouse-cli index` 색인만 진입 (upload follow-up). IntegrationTests 7→8 (HTTPS-only +1). 누적 **422 Fact** (lib 100 + service 111 + IntegrationTests 8 + Promaker 203).
- **이전 모드** (참고): **Phase S5e — 본격 IntegrationTests round-trip (MA22) 박제 해제 + 자가 검열 0 차단 — commit 대기** (s5e-r0). `Program.configureApp` pure 함수 export + `ServiceFixture` (in-memory self-signed cert + Kestrel localhost:0) + 6 Fact 본격 e2e (실 Kestrel HTTPS round-trip). 누적 **421 Fact** (Promaker 203 + service 111 + IntegrationTests 7 + lib 100, +6 vs s5d-r0). s5c-r0 (vii) 의 이연 박제 해소 — 우회 mechanism = C 안 (configureApp export + in-memory cert + 신뢰 우회 HttpClient).
...
| (본 commit) | s5f-r0 | Phase S6 scaffold (`Ds2.LightHouse.Cli`) + s5e follow-up 3건 (M2/m5/I — 8 Fact) + N3-A MSB3277 해소 + N5 parent §3.15.5 흡수 |

**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 (s5f 회귀 0)
- IntegrationTests: S5a 1 (scaffold sentinel) → S5e 7 (sentinel 1 + e2e 6) → s5f 8 (+ HTTPS-only Fact 1)
- service Tests: 111 (S1 24 + S2 41 + S3 27 + S4 19, s5f 회귀 0)
- lib Tests: 100 (s5f 회귀 0)
- **누적 422 Fact** (Promaker 203 + service 111 + IntegrationTests 8 + lib 100, +1 vs s5e-r0)
