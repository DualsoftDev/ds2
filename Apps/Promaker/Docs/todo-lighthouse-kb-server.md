...
**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 → s6-r6 208 (s6-r7~r10 회귀 0)
- IntegrationTests: S5a 1 → S5e 7 → s5f 8 → s6 12 → s6-r1 19 → s6-r2 20 → s6-r5 22 → s6-r7 24 → s6-r10 25 (s6-r8/r9 회귀 0)
- service Tests: 111 → s6-r6 115 → s6-r10 116 (s6-r7/r8/r9 회귀 0)
- lib Tests: 100 → s6-r5 102 → s6-r8 109 (s6-r9/r10 회귀 0)
- **누적 458 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 109). s6-r10 = +2 Fact (K1 + K2 회귀 차단).
...
**처리 완료 (s6-r10, 본 turn)**: s6-r9 의 외부 reviewer 의무 2건 follow-up — K1 + K2 회귀 차단 Fact 박제. (i) **K1**: `ZipImport.makeBackupPath` public helper 추출 + `BackupSuffixHexLen = 12` SSOT 상수 + unit Fact 1건 (200회 unique suffix + regex 형식 검증). race 시뮬레이션은 swap catch 분기의 over-aggressive cleanup 결함으로 deferred (별 turn). (ii) **K2**: `ServiceFixture.Services` accessor 신설 + E2eRoundTripTests Fact 1건 (IOptions<FormOptions> 추출 후 cfg 정합 박제). (iii) 자가 검열 Minor 4건 (m1 suffix 길이 SSOT 즉시 적용 / m2-m4 backlog). 누적 **458 Fact**.

**다음 권장 (s6-r10 이후)** — **즉시 진입 = P3 Phase 2 task B (`ImageStore.fs`)** (사용자 confirm 박제, s6-r10 commit 직후 본격 진입):
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task B) ← 다음 진입 대상**: `ImageStore.fs` — sha256 + blob 저장 + ImageCache/ImageReferences upsert (cross-document 공유, 단일 collection 안). 본 phase 의 schema 확장 (task A, s6-r8) 위에 build. 규모 ~150 line + Fact 5-7건. lib (`Solutions/Core/Ds2.LightHouse`) 안에 박제 — `SqliteStore.fs` 의 schema 위에 layer.
