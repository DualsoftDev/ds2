...
| (본 commit) | s6-r23 | **Phase 2 잔여 묶음 4건 — §7.1 hash 갱신 + sha256 drift detection + OoxmlExtractor m2 helper 통합 + PdfExtractor 정상 fixture 도입 + AttachmentIngestService BuildCaptionGenStatic refactor + cost gate mock fact**. 변경 = lib 1 + service 0 (m2 영향 없음) + Promaker 1 + Tests 6 (수정 6 + 신규 1, 11 파일, +200/-70 line). 회귀 0 + 누적 **522 Fact** (s6-r22 513 → +9).
...
**다음 권장 (s6-r23 이후)** — Phase 2 잔여 (parent §3.15 sweep) + Phase S7 진입:
...
- **(s6-r21 미적용 backlog)**:
  - **MJ1 (multi-instance read-modify-write race 본질 해결)** — KbManagerDialog 의 cost gate merge-save 가 cap 덮어쓰기 1 시나리오만 해결. Load→modify→Save 사이 lock 미보호 race 잔존 (`TokensUsedToday` 누적 일부 손실). 본질 해결 = file lock (`FileStream` + `FileShare.None` + retry) 또는 LlmConfig 전체 transaction. 별 turn design.
...
- **(s6-r23 자가 검열 미적용 backlog — 별 turn)**:
  - **m1 (OoxmlExtractor Warn 메시지 통합)** — header/footer 의 별 prefix 가 통합 후 단일 "Drawing image 추출 실패" 로 흡수. log grep 시 `refLocator` (`header=N` / `footer=N`) 로 식별은 가능. 실 회귀 영향 0, log audit sweep 별 turn.
  - **m2 (PdfExtractor fact 의 bytes 결정성 강화)** — `Assert.True(img.Bytes.Length > 0)` 약한 assertion (PdfPig zlib 비결정성). PNG signature head 박제 + ImageStore.computeSha256 결정값 박제로 강화 가능. backlog.
...
- **(parent §3.15 단원 sweep)** **별 turn 의무 (deferred)** — s6-r18 정정 marker 박제 완료, 단원 본문 (lazy 흐름 → eager 흐름) 재작성 sweep 은 별 turn. 우선순위 낮음 (server doc D-2-2 가 SSOT, parent doc 의 stale 박제는 cross-ref 로 흡수 정합).
