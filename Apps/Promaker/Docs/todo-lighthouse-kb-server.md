...
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 → s6-r14 131 (s6-r9/r10 회귀 0)
- **누적 480 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 131). s6-r14 = +2 Fact (Phase 2 task C3 OoxmlExtractor PNG 매칭 + BMP skip).
...
**다음 권장 (s6-r14 이후)** — Phase 2 본격 진입 (C4 / D / E):
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task C4) ← 다음 진입 후보**: `Chunks.ImageCount` 갱신 + `ExtractedImage.ChunkIndex` (또는 RefLocator → ChunkId 직접 매핑) 박제. `ingestImagesIntoStore` 시그니처에 ChunkId Some 채움. segment → chunk 결정 후 image 의 ChunkId 박제. 동시 OoxmlExtractor 의 paragraph 매핑 (image 가 inline Drawing 박힌 paragraph 의 `p=%d` 로 RefLocator 통일) 강화 — 옵션 B 의 단순화 trade-off 흡수. 규모 ~70 line + Fact 4-5건.
