...
**테스트 누적**:
- Promaker.Tests: S4 158 → S5a 180 → S5b 192 → S5c 203 → s6-r6 208 → s6-r20 216 (s6-r21 회귀 0)
- IntegrationTests: S5a 1 → S5e 7 → s5f 8 → s6 12 → s6-r1 19 → s6-r2 20 → s6-r5 22 → s6-r7 24 → s6-r10 25 (s6-r21 회귀 0)
- service Tests: 111 → s6-r6 115 → s6-r10 116 → s6-r20 118 → s6-r21 122 (s6-r21 +4 image fixture e2e)
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 → s6-r14 131 → s6-r15 135 → s6-r16 136 → s6-r19 143 → s6-r20 145 → s6-r20-review 144 → s6-r21 146 (s6-r21 +2 image-only paragraph + header image)
- **누적 509 Fact** (Promaker 216 + service 122 + IntegrationTests 25 + lib 146). s6-r21 = +6 (image fixture e2e 4 + OoxmlExtractor 강화 2).
...
**다음 권장 (s6-r20 이후)** — Phase 2 잔여 (실 image fixture Fact 묶음 / OoxmlExtractor 강화 / parent §3.15 sweep) + Phase S7 진입:
...
- **(s6-r21 미적용 backlog)**:
  ...
  - **mn3 (mime fallback dead code 위험)** — `inferMimeFromPath` production caller 부재 (정상 색인 path 는 mime 박제 항상). 본질 해결 = `ImageCache.MimeType NOT NULL DEFAULT 'application/octet-stream'` schema 제약. mn3 fallback path 의 e2e fact 도 별 turn.
  - **mn1 (OoxmlExtractor paragraph/table 분기 통합)** — `text>0` vs `image-only` if/elif 분기 → 단일 `if isText || hasImg then` 통합 (segment add 는 `if isText`). 6 line 감소.
  - **mn7 (PNG sample bytes 공통 fixture module)** — `samplePngBytes` 가 ImageStoreTests / OoxmlExtractorTests / AttachmentToolsTests 3곳 중복. `Ds2.LightHouse.Tests.Fixtures.SamplePng` 류 module 추출.
  - **mn5/mn6 (테스트 스타일)**: OpenXml header API 정석 (Header property 객체 할당) / fixture 메시지 한영 혼재 정리.
  - **table cell 단위 매핑 (s6-r16 잔여)** — `tbl.Descendants<Blip>()` cell scheme 확장 (`p=%d.cell=%d.p=%d`). 별 turn 의무.
...
- **(s6-r20 Fact 잔여)** — 실 image fixture (PdfExtractor 산물) 기반 attachment_read 의 includeImages 분기 / size 정책 강등 분기 Fact (현재 image 0개 fact 만 박제). cost gate hard cap 시 captionGen mock 으로 SkippedCaption 반환 Fact (BuildCaptionGen 의 mock 어려움 — Promaker.Tests 의 HttpClient handler stub 별 turn). LIGHTHOUSE_VLM_API_KEY env var fallback 의 cli 측 e2e Fact.
