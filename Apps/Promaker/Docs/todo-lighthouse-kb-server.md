...
- lib Tests: 100 → s6-r5 102 → s6-r8 109 → s6-r11 123 → s6-r12 128 → s6-r13 129 (s6-r9/r10 회귀 0)
- **누적 478 Fact** (Promaker 208 + service 116 + IntegrationTests 25 + lib 129). s6-r13 = +1 Fact (Phase 2 task C2 PdfExtractor image 추출 + M2/m6 mix scenario).
...
**다음 권장 (s6-r13 이후)** — Phase 2 본격 image 추출 (C3) 진입 가능:
- **(N1 재진입)** dist 본격 실행 — `/dist` skill **사용자 직접 호출**. 본 세션 자체 호출 안 함.
- **(P3 Phase 2 task C3) ← 다음 진입 후보**: `OoxmlExtractor.fs` (docx) 가 `MainDocumentPart.ImageParts` 로 image 추출 (parts iter + `ImagePart.ContentType` 화이트리스트 매칭 + `GetStream()` empty 가드 + PNG/JPEG/GIF/WEBP 화이트리스트 외 skip). 규모 ~60 line + Fact 3-4건. **본격 진입 전 결론 의무 (s6-r13 자가 검열 M-r13-1)**: RefLocator 설계 결정 = `p=%d#img=%d` (현 PdfExtractor 패턴) 유지 vs `p=%d` 만 + Ordinal 로 page 안 순번 표시 (Models.fs §108 SSOT 원형). 결정에 따라 PdfExtractor 의 RefLocator 도 paired 갱신 (가능하면 C3 commit 하나에 묶음).
