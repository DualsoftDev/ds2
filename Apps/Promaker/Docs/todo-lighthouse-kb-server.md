...
- [x] **D-S7-2a (server-side, s6-r27) SSE `/events`** — payload schema 박제 (`event/collectionId?/progress?/message?/timestamp` ndjson, text/event-stream + Cache-Control no-cache + 30s keepalive). EventBus in-memory pub-sub + CollectionEndpoints 3 publish point + IT 3 Fact. **D-S7-2b 잔여 (별 turn)**: Promaker `LightHouseClient.OpenEventsStreamAsync` + `KbManagerDialog` 자동 collection 갱신 (cross-instance 인지) + Phase 2 vision caption 진행률 client → server upload (D-S7-2c).
