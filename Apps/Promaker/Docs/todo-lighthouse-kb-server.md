...
- **모드**: **Phase S5 완전 종결 (100%) + dist 진입 준비 완료 (paired-release drift detector 신설) + 자가 검열 1건 즉시 적용 — commit 대기** (s5d-r0). 신설 `Apps/Promaker/scripts/check-paired-release.ps1` 가 lib `IndexerVersion.Current` literal ↔ service `indexerVersionRange` 정합을 dist Step 3.5 에서 강제. **s1-r0 박제 정정 (Errata)**: paired-release SSOT = AssemblyVersion 비교 (의미 없음) → IndexerVersion.Current literal source regex 추출 + range 정합. **§4.2 Phase S5 100%** (PromakerToolNames.cs + Drift test fresh 확인 완료). 본격 IntegrationTests round-trip (MA22) 는 여전히 Phase 2 / dist 직전 이연 (별 task).
- **이전 모드** (참고): Phase S5c (commit `79ee30b` s5c-r0) → s5c-r1 (commit `83cd464` UI 테마 정합 + Makefile dev install/run + howto 문서 + .ps1 UTF-8 BOM). 203/203 Promaker.Tests.
