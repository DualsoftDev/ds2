...
- **S3b apply patch-merge 본구현** (본 turn `--gc` 대기) — Major-1 해소. `ApplyContext.Level: ExportLevel ref` field + apply 진입부 `wireLevel` 확정 후 `ctx.Level := lvl` set. **5 dispatch helper lookup-first 분기**: `dispatchActiveSystem` (`findSystemByName` + kind 충돌 검증) / `dispatchPassiveSystem` (lookup + device cascade skip + `Queries.apiDefsOf` 로 `entry.ApiDefIds` 누적 + kind/device mismatch 검증 — M-2 sub-agent review) / `dispatchActiveFlows` Flow (`Queries.flowsOf`) / `dispatchWork` Work (`Queries.worksOf`) / `dispatchWork` Call (`Queries.callsOf` + `ApiCalls[0].ApiDefId` 매칭). **Arrow lookup-first 2 site** (T8 멱등성 회귀 발견 후 추가): `arrowWorkExists` / `arrowCallExists` helper 신설 + processOneFlowArrow / processOneArrow 의 modeling 분기. **callObjOpt mutate**: `tryFindCallInPlan` → `lookupCallById` (store + plan 양쪽). **helper 위치 이동**: `findSystemByName` / `findFlowByPath` 가 line 1478 → line 783 (dispatchPassiveSystem 직전, forward-ref 회피).
- **S4 SSOT 본문 + 테스트** (본 turn `--gc` 대기) — yaml-protocol-v0.md 7개 절 갱신 (§1.7 신규 row / §2.1 top-level 키 enum 8→9 / §2.4.1 신규 row 2건 `level` + `Category` 사전 / §2.6 patch DSL × modeling footnote / §2.7 룰 #29~#30 신설 / §2.8 view × level 직교 4 case 표 / §4 도구 시그니처 / §6.3 default fallback 표 확장). `ModelProtocolTests.fs` 신규 11건 (#31 S4-T1~T11): T1 modeling export wire 검증 (B/C/D 부재 + level: modeling) / T2 callHasEnhancement modeling 분기 (B/C/D-only Call string scalar) / T3 silent destructive 차단 / T4 B/C/D 키 거부 (룰 #30) / T5 level unknown 거부 (룰 #29) / T6 view: partial + level: modeling 시 view 우선 (C2) / T7 Full default level 키 부재 (회귀 가드) / T8 멱등성 / **T9 entity kind 변경 금지** / **T10 apiDetails.actionType 변경 (A_Modeling)** / **T11 Work.tokenRole 변경 (A_Modeling)** — sub-agent review M-3 산출.
- **sub-agent review S4 처리** — Critical 0 / Major 3 (M-1 defer + #32 등록 / M-2 device mismatch 진단 + UnknownSugar 진단 (사용자 review Minor #1) / M-3 테스트 T9-T11 추가) / Minor 6 (m-1~m-5 defer 또는 정합 검증 / m-6 ModelingCategory.fs visibility docstring 정정). 사용자 review Minor #1~#4 (UnknownSugar 진단 1건 처리 + 나머지 가성비 검토로 현 상태 유지).
- **자가 검열 trigger ②③④⑤ 4건 동시 충족 — sub-agent review 2회 (S1 + S4) 완료**.

**테스트**: ModelProtocolTests **122/122 통과** (기존 111 + S4 신규 11). 전체 `Ds2.LlmAgent.Tests` **388/388 통과**. 회귀 0건.
