...
D. UX / robustness — 본 turn (2026-05-27) 사용자 보고 사고 기반 별 backlog:
  - **D1 install-service.ps1 stop+delete+create 비롤백 chain** — 사용자가 "로컬 LightHouse 서비스 활성화" 버튼 재클릭 시 기존 service stop → delete → create 흐름 중 create 가 실패 (marked-for-deletion 잔존 / binary lock 등) 하면 service 가 *삭제만* 된 채 종료. 권장 fix: (1) create 실패 시 stop+delete 자체를 retry, (2) 또는 기존 service 의 임시 backup 후 create 실패 시 rollback. install-service.ps1:188-214 영역.
  - **D2 DialogHelpers.Warn 의 large message 무스크롤** — LightHouseLocalInstaller 가 enable-ai.ps1 실패 시 `TryReadTail(logOut, 4096)` 의 4 KB log tail 을 DialogHelpers.Warn (= Windows MessageBox) 에 그대로 dump. MessageBox 는 스크롤바 없음 → 사용자 시야 화면 외 잘림 + Enter 외 선택지 없음. 권장 fix: (a) ScrollViewer + readonly TextBox + "로그 열기" 버튼이 박힌 별 사용자 정의 dialog (재사용 가능), (b) 또는 Warn 본문은 안내만 + "로그 열기" 버튼 (notepad spawn). 본 turn (Local 버튼 2개 분리) 으로 본 dialog 호출이 사라졌지만 다른 large error path 에서 재현 가능.
  - **D3 LightHouseServiceConfig INotifyPropertyChanged** — DataGrid cell 의 BaseUrl 편집이 PropertyChanged 미발화 → ApplicationSettingsDialog 의 "Local 항목 추가" 버튼 IsEnabled 가 cell 편집 후 즉시 갱신 안 됨 (CollectionChanged hook 만). 현재 click handler 의 IsSameLocalEndpoint 재검사 safety net 으로 보완하지만, UX 개선 의도 시 LightHouseServiceConfig 에 INotifyPropertyChanged 적용 + CellEditEnding hook 권장.
  - **D4 ResolveLocalBaseUrl IPv6 unspecified 처리** — config.json 의 listenUrl 이 `[::]` 등 IPv6 unspecified 일 경우 정규화 미적용 (Uri.Host 가 full-expand "0000:..." 형태로 정규화되어 단순 비교 안 통함). 현재 분기는 IPv4 `0.0.0.0` 만 처리. install-service.ps1 default 가 IPv4 loopback 이라 사용자 시나리오 가능성 낮지만, 사내 LAN IPv6 only 환경 위해 별 turn 박제. `Uri.HostNameType == UriHostNameType.IPv6 + IPAddress.IPv6Any 비교` 추가 의무.
  - **D5 NODE_EXTRA_CA_CERTS PEM 의무 (해결 박제, 2026-05-27)** — Node 의 OpenSSL 은 `NODE_EXTRA_CA_CERTS` 에 **PEM 형식만** read. enable-ai.ps1 초기 박제가 `X509ContentType.Cert` (DER binary) 로 export 하여 `error:10000002:SSL routines:OPENSSL_internal:system library` + "ignoring extra certs" warning → cert 신뢰 실패. 본 turn 정정 — `RawData → Convert.ToBase64String (InsertLineBreaks) + BEGIN/END CERTIFICATE wrapper` PEM 박제. 외부 자료: Node.js `--ca-certs` doc / OpenSSL `SSL_CTX_load_verify_locations` PEM only.
