...
D. UX / robustness — 본 turn (2026-05-27) 사용자 보고 사고 기반 별 backlog:
  ...
  - ~~**D5 NODE_EXTRA_CA_CERTS PEM 의무**~~ (**완료 + 후속 무용 — 2026-05-27 m9 메타리뷰**) — Node OpenSSL PEM 강제 fix 완료 후 추가 진단에서 Anthropic Claude CLI 의 mcp HTTP/SSE transport 자체가 OAuth 2.1 강제 (`_authThenStart`) 라 단순 Bearer PSK 와 fundamental mismatch 확정 → Plan B (Promaker wrapper) 진입 → `NODE_EXTRA_CA_CERTS` 박제 path 자체 폐기 (`ClaudeCliProvider.fs`). D5 history 만 의미, 현 운영 path 와 무관.
