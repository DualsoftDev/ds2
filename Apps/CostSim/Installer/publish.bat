...
echo ========================================
echo   CostSim Release Publish
echo ========================================
echo.

set "SCRIPT_DIR=%~dp0"
set "REPO_DIR=%SCRIPT_DIR%..\..\.."
set "PROJECT_FILE=%REPO_DIR%\Apps\CostSim\CostSim.csproj"
...
set "PUBLISH_DIR=%REPO_DIR%\Apps\CostSim\bin\Release\net9.0-windows\win-x64\%PUBLISH_SUBDIR%"
...
echo [5/5] Zip...
powershell -Command "$exe = (Get-ChildItem '%OUTPUT_DIR%\CostSim_Setup_*%OUTPUT_SUFFIX%.exe' | Sort-Object LastWriteTime -Descending | Select-Object -First 1); $zip = $exe.FullName -replace '\.exe$','.zip'; Compress-Archive -Path $exe.FullName -DestinationPath $zip -Force; Write-Host ('  ' + $zip)"
if errorlevel 1 (
...
dir /b "%OUTPUT_DIR%\CostSim_Setup_*%OUTPUT_SUFFIX%.*" 2>nul
