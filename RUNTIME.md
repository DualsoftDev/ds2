...
[사용자 입력] — 키보드 / 마우스 / 메뉴
      ...
      │  (조회/투영은 store.BuildTrees() / store.CanvasContentForTabUi() 등)
      │
      ▼  F# — DsStore Extension Method
...
각 엔티티 Add는 DsStore Extension Method에서 `WithTransaction` + Track 헬퍼로 실행합니다.
...
| `ConnectSelectionInOrderUi` | WithTransaction 1건 | 선택 순서 기반 Work-Work 또는 Call-Call 화살표 N개 일괄 생성. 2-노드(단일 화살표) 포함 모두 이 진입점 사용. |
| `AddApiDef` | Track 1건 | |
...
| 엔티티 이동(단/다중) | `MoveEntitiesUi` | 동일 위치 항목은 Track 미생성, 각 이동이 하나의 WithTransaction에 묶임 |
...
```
Call 선택
  → GetCallConditionsForPanelUi(callId)
...
| 동작 | DsStore Extension 메서드 | 반환 false 조건 |
|------|------------------------|----------------|
| 조건 추가 | `AddCallConditionUi(callId, type)` | Call 미존재 |
...
주요 구현: `Geometry/ArrowPathCalculator.fs`, `EditorCanvas.Connect.cs`, `Queries/ConnectionQueries.fs`
...
주요 구현: `Ds2.Core/JsonConverter.fs` (namespace `Ds2.Serialization`), `Core/DsStore.fs`
