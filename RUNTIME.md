...
| ApiCall I/O 태그 일괄변경 | `UpdateApiCallIOTagsBatch` | (apiCallId, inTag: IOTag, outTag: IOTag) 목록 → 단일 Undo 트랜잭션 |
...
원본 ApiCall (store.ApiCalls 내 존재)
  ...
  ▼  AddApiCallsToConditionBatch
     1. Queries.getApiCall(sourceApiCallId, store) — store 전역 조회 (각 ID별)
...
[F# — Ds2.Editor/Store/Panel/Panel/Batch.fs]
 UpdateWorkDurationsBatch(changes: seq<struct(Guid * int)>)
  ...
  └─ TrackMutate per work → work.Properties.Duration 변경
...
[C# — IoBatchCommands.cs]
 OpenIoBatchDialog()
  ...
  └─ _store.UpdateApiCallIOTagsBatch(changes)
        ...
        ▼
[F# — Ds2.Editor/Store/Panel/Panel/Batch.fs]
 UpdateApiCallIOTagsBatch(changes: seq<struct(Guid * IOTag * IOTag)>)
...
Work에 Call이 없는 경우 (leaf):
  duration = userDurationMs (사용자 설정값)

Work에 Call이 있는 경우:
  deviceCriticalPathMs = Queries.tryGetDeviceDurationMs(workId, store)
    → ArrowBetweenCalls 토폴로지 기반 최장 경로 (DAG Critical Path)
    → longestPath(c) = duration(c) + max(longestPath(pred))
  duration = max(userDurationMs, deviceCriticalPathMs)
```

**Critical Path 알고리즘**: Call 간 Start/StartReset 화살표로 DAG를 구성하고, 각 Call의 device duration(ApiDef → RxWork → Duration)을 가중치로 최장 경로를 메모이제이션으로 계산합니다.
...
속성 패널에서 Work 선택 시 `Queries.tryGetDeviceDurationMs`로 예상 소요 시간을 계산하여 표시합니다.

```
PropertyPanelState.Refresh():
  Work 선택 시:
    Queries.tryGetDeviceDurationMs(workId, store) → int option
